...
export const HASTA_IZOLASYON_ENVANTERI: Record<string, Siniflama> = {
  ...
  'app/api/doktor/pediatri/_kohort.ts': I('kohort: patients .eq(doctor_id) → ids; every source query .eq(doctor_id|doktor_id).in(patient_id, ids); POST ids pass through pediKohortVerisi(doktorId) before any message is written'),
