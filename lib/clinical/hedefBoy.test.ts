...
describe('hedefBoy (mid-parental height)', () => {
  ...
  it('ships 3D cartoon family cutouts for the Araçlar studio', () => {
    ...
    assert.match(manken, /maxWidth: 'none'/)
    assert.match(manken, /hedef-boy-figur/)
    assert.match(manken, /--hedef-boy-h/)
    assert.doesNotMatch(manken, /cm - cmMin/)
    const globals = readFileSync(join(root, 'app/globals.css'), 'utf8')
    assert.match(globals, /img\.hedef-boy-figur/)
    assert.match(globals, /max-width:\s*none\s*!important/)
