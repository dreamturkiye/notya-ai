...
Apply `lib/db/migrations/029_yenidogan_taburcu.sql` on preview/prod (shared Supabase) **before** persistence QA. Do not merge this PR until that audit.
...
- Migration `029_yenidogan_taburcu.sql` — `dogum_olaylari`, `bebek_kartlari`, `taburcu_checklist`, `lohusa_checklist`, `bebek_gorevleri`, `asi_dozlari`; `lab_paneller.panel_type` / `sample_no`; RLS doctor-scoped
- Engine `lib/clinical/yenidogan/*` (gate, calendar, NTP yorum)
- Persist `lib/doktor/yenidoganKayit.ts` (hooked from `POST /api/doktor/gebelik` action `sonlandir`)
