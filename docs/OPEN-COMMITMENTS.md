...
| ID | Status | What |
|---|---|---|
| NOTYA-ASI-LOT-01 | **IN PROGRESS** — part 1 (fields + display, branch `fix/asi-lot`) done; part 2 (backfill from the notes) next | Lot / site written in muayene notes appeared nowhere: the approval sync packed them into `asilar.notlar` ("Lot: … · Uygulama yeri: …"), which no view renders, and the NOTYA-ASI-NOT-02 backfill passed only name / dose / date. Part 1: migration 093 adds `asilar.lot_no` + `asilar.uygulama_yeri` (nullable) and lifts any packed "Lot:" / "Uygulama yeri:" segment out of `notlar` (other text kept, same order; 0 such rows existed in production, applied 2026-09-24). Writers use the columns: approval sync (`notAsiAktarim`), Aşılar form + `POST/PATCH /api/doktor/asilar` (two optional inputs "Lot no", "Uygulama yeri"), voice Ayşe `asi_kaydi_ekle` (optional fields, dropped when guessed). Display: Aşılar list "Lot: X · Yer: Y" under the Uygulandı line only when known; Aşı Karnesi model (`lib/asi/karneBelgesi.ts`) carries `lotNo` / `uygulamaYeri` → Sağlığım screen + print and the single PDF (doctor + portal) get "Lot / Uygulama yeri" (empty cell when unknown). Karne okuma (photo) does not read lot today — not changed. |
