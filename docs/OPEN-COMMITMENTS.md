...
| 2026-09-25 | SEC-RLS-OFF-01: 17 public tables have RLS disabled and are readable/writable with the anon key: all mali_* and avukat tables (musevvekiller, deliller, avukat_sessions, sure_takibi, dilekce_kuyrugu...), plus logs, normalized_transactions, ingestion_log, review_queue. Current exposure is minimal (16 empty, mali_belgeler 3 rows) because Mali/Avukat have not launched. Fix: enable RLS with owner policies (migration 094) before Sprint 3 ships anything. | IN PROGRESS 2026-09-25 - migration lib/db/migrations/094_rls_mali_avukat_kilidi.sql written (enable RLS, no policies); applying it waits on Kaan approving the Supabase migration prompt. Sprint 3 must add owner policies before any browser-side Mali/Avukat access. |
| 2026-09-25 | SCOPE (Kaan): work concentrates on the doktor and Klinik sides only; Mali and Avukat come in the future. Their tables are locked by migration 094 once applied. | STANDING || 2026-09-25 | NOTYA-ILETISIM-01 (Kaan: build all now, Apple-simple): one-tap email + WhatsApp from the doctor's OWN accounts everywhere (mailto / Gmail web / Outlook web links, wa.me), Ayarlar > İletişim card, morning Bugün N mesaj hazır queue, staff (sekreter) access for randevu messages, patient contact consent (intake + profile), contact log, Twilio patient sender + dead Meta route retired, randevu cron fills the queue instead of sending. Claude Code job A, branch feat/iletisim-tek-dokunus, migration 095 (written by the job, applied after review). | IN REVIEW 2026-09-25 - built on feat/iletisim-tek-dokunus, PR #417 (NOT merged; tsc clean except the 4 known, npm test 2753/2753, next build clean): lib/iletisim (templates, wa.me / mailto / Gmail / Outlook link builders capped at 1800 chars, consent gating, queue, OtomatikGonderici slot + README contract for jobs B/C), one send button (GonderDugmesi) on Hatırlatma, hasta bilgi formu, Sağlığım bağlantısı, aşı listesi, randevu screen and the 4 kohort panels that had WhatsApp; Ana Sayfa "Bugün N mesaj hazır" + full-screen flow (secretary: on Randevular); Ayarlar › İletişim; intake consent lines + hasta profili consent/last contacts; randevu cron now enqueues tomorrow (07:00 + 17:00 TRT) instead of Twilio; Resend patient mail → queue item (covers Mesajlar, konsültasyon, aşı and all 30 kohort senders); /api/notes/whatsapp (Meta, unused) and /api/doktor/hatirlatma (Twilio) removed; Twilio kept only for lib/alarm.ts + doctor ping. Migration lib/db/migrations/095_iletisim.sql written, NOT applied — code fails soft until then. Remaining to close: Kaan reviews + merges the PR, applies 095, then a live check (Ana Sayfa card after the 07:00 cron, one WhatsApp + one mailto send from a phone, secretary account sees randevu items only). Decisions to confirm in review: secretary may also send the hasta bilgi formu link (kept from the old intake flow); bookings without a patient record are not messaged (no place to hold consent); PIN never goes in the Sağlığım link message. |
| 2026-09-25 | NOTYA-ILETISIM-02: automatic email from the doctor's own mailbox via one-time Gmail ile bağlan / Outlook ile bağlan (send-only permission; replies and Sent folder stay in his mailbox). Job B, starts after A merges. Needs: Google Cloud OAuth client + Google app verification for the gmail.send scope (weeks; up to 100 test users meanwhile, Dr. Gökhan first) and a Microsoft Entra app registration. | OPEN - waits on Claude (after A) + Kaan (Google Cloud / Microsoft accounts, verification submission) |
| 2026-09-25 | NOTYA-ILETISIM-03: automatic WhatsApp from the doctor's own number via Meta WhatsApp Business coexistence (Business app keeps working on his phone, chats sync; Turkey supported). Job C, starts after A merges. Needs: Meta developer app + Embedded Signup config, Dream Türkiye business verification with Meta (or a partner such as 360dialog), approved utility templates for randevu reminders; per-message Meta fee for API-sent messages. | OPEN - waits on Claude (after A) + Kaan (Meta business verification) |
