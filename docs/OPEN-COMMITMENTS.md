...
|---|---|---|---|
| 2026-09-17 | **Bilinen çocuk hastada baseline branşlarda pediatri sekmeleri** (kardiyoloji, KBB, üroloji, ortopedi… → Büyüme, M-CHAT, Gelişim, Bebek kartı) | Aynı gün yazılmış CHART-TAB-POLICY (`hastaDosyaSekmeleri.ts`) bunu bilerek açık bırakıyor ("baseline/aile mixed care"); Kaan'ın yeni kuralı ("pediatri alanı kardiyoloji hekimine çıkmaz") ise tersini söylüyor. Politikayı sessizce çevirmedim. **Öneri:** `pediatriAracSekmesiUygun`'u `pediatrikBaglamMi`'ye bağla (pediatri/çocuk cerrahisi her zaman, aile yalnız çocukta, diğerleri asla) — tek fonksiyon, testler 30/30 döngüyle hazır. | DONE 2026-09-25 - #409: Kaan ruled it specialty bleed-over. Allow-list: pediatri, crossover aile hekimligi / genel pratisyen / no branch. 3 tests. |
| 2026-09-17 | **"Kadın Sağlığı & Gebelik" sekmesi her branşta** (≥12 yaş kadın hastada tam KD bölümü: NST, risk formu, VTE, gebe kartı) | "Mixed care" politikası olarak belgelenmiş; ama tam KD bölüm UI'sinin göz/derm/kardiyoloji hekimine açılması tam da bu denetimin sızıntı tanımı. Gebelik bilgisi ise her branş için güvenlik bilgisi (isotretinoin, görüntüleme, ilaç). **Öneri:** her branşa salt-okunur "Gebelik durumu" çipi; tam KD bölüm UI'si yalnız KD (±aile) hekimine. Sunucu tarafı (`/api/doktor/gebelik`, `jinekoloji`, `kadin-sagligi`) bugün yalnız hasta sahipliğine bakıyor, branşa değil — karar verilince orada da aynı kapı. | DONE 2026-09-25 - Kaan: do it the Turkish way. Full KD section: kadin dogum + subspecialties and primary care (aile hekimi shares DOB with kadin dogum in the SB system). Every other specialty: header safety chip Gebe · N. hafta only (TITCK KUB isotretinoin pregnancy test, pre-imaging pregnancy query, prescribing). 4 tests. |
...
| 2026-09-25 | NOTYA-RECETE-04: Plan text vs structured Ilaclar list divergence warning (planIlacTutarsizMi + confirm before approval), PR #404. Unit-tested (4 tests incl. the live Umutcan Augmentin/Amoksisilin case), build and 2638/2638 clean. NOT live-verified: the native window.confirm froze the browser automation. PR is on hold, not merged. To close: on the #404 Vercel preview, open a note whose Plan names a drug that is not in the Ilaclar list, press Onayla, confirm the dialog appears and lists the Ilaclar entries; then merge. | SUPERSEDED 2026-09-25 - the native window.confirm was replaced by the in-page card (see the entry below); live check moves to that entry |
| 2026-09-25 | NOTYA-RECETE-04 follow-up (Dr. Gokhan, Umutcan, note 0a57965e): DB confirms he changed ONLY the Plan text at 21:50 (Augmentin ES 10 gun, Calpol); content_ilaclar was never edited, so the recete and hasta_ilaclar correctly still show Amoksisilin 7 gun + Parasetamol. The 198bf41 deactivation fix does not apply (the list never changed), so re-approving alone will NOT fix it. Data fix = doctor edits the Ilaclar list on that note and re-approves (no silent DB write). DECIDED by Kaan 2026-09-25 (Apple-style ease of use): when they diverge, Ayse rebuilds the Ilaclar list from the Plan text and the doctor confirms it in one tap on an in-page card (IlacUyumKarti), on both the note page and Inceleme; implemented in PR #404. Once live, Dr. Gokhan fixes Umutcan by re-approving the note and tapping the update. | DONE 2026-09-25 - live-verified on a local dev server running commit e297adc9, on the kaanari test account with a TEST note reproducing the Umutcan mismatch: card appeared, Ayse extracted Augmentin ES 7.5 ml 10 gun + Calpol 5 ml (no invented duration), one tap saved exactly those two to the note and hasta_ilaclar; re-approving the now-consistent note showed no card. Test rows deleted afterwards. Merged via #404. Remaining for Dr. Gokhan: re-approve the Umutcan note and tap the update. |
| 2026-09-25 | Ayse routing: asked whether Umutcan ever got an antibiotic and which, she answered with the practice-wide last-30-days most-prescribed statistic instead of the patient medication history (answer happened to match, but wrong source and wrong time window for ever). Ledger item, not fixed. | DONE 2026-09-25 - NOTYA-AYSE-HASTA-01: a ranking question that names exactly one patient is answered from that patient file (tekHastaSorusuMu), never the practice-wide statistic. Tested with Dr. Gokhan exact sentence. |
| 2026-09-25 | NOTYA-RECETE-05: close the added-drug gap in the one-tap Ilaclar check. Before: the check ran only when a LISTED drug was missing from the Plan, so a drug ADDED to the Plan (existing ones kept) was never caught. Now it also runs whenever the Plan was edited since the note opened, and Ayse pre-reads the edited Plan in the background (cached per Plan text) so Onayla is usually instant. If Ayse cannot read the Plan and there is no concrete mismatch, approval continues without a card. Branch fix/plan-ilac-eklenen, PR #405 (build clean, 2648/2648). Live check paused 2026-09-25: the Chrome extension disconnected. A TEST note is left on the kaanari test account for it (note dab72ba0, session 7d3ee708; Plan and list both Amoksisilin + Parasetamol). Check: add a new drug line to the Plan, wait a few seconds, press Onayla, expect the card listing 3 drugs; then delete the test rows and merge. | DONE 2026-09-25 - live-verified on a local dev server on the kaanari test account: added c) Ventolin nebul 3x1 5 gun to a Plan whose list had only Amoksisilin + Parasetamol; on Onayla the card appeared within about 1 s (background pre-read) with all 3 drugs; Vazgec saved nothing (DB checked). Test rows deleted. Merged via #405. |
...
| 2026-09-25 | NOTYA-SES-HASTA-01 (Dr. Gokhan): by voice Ayse said there was no record of Ayse Yesil, while text chat found her. Cause: the voice agent sends the whole spoken sentence to hasta_bul; the clinical words (son muayene, ozet) became search filters that excluded the exactly-named patient. Fix in lib/doktor/hastaCozumleyici.ts: an exact full-name (2+ words) match of one patient is final; a partially named patient is returned when the extra words find nothing, except for list questions. Verified against Dr. Gokhan live patient list with 10 phrasings incl. his exact sentence and the Merhaba Ayse, bu hafta atesli hastalarim kimler cohort case; tsc clean; 2902/2902. | DONE 2026-09-25 |
