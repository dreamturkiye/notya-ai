...
- 2026-09-22 — YENİ İSTEK (Dr. Gökhan, Kaan aracılığıyla — NOTYA-RANDEVU-TARIH-01): tüm muayeneler bir randevuyla başlıyor; doktor onay verdiğinde muayenenin zaman damgası şu anki "onay/işlem anı" yerine o randevunun tarih+saatiyle eşleşmeli. Kod incelemesi bulgusu: `app/dashboard/doktor/randevular/page.tsx`'teki "🩺 Muayeneyi Başlat" butonu şu an `/session/new?patientId=...` açıyor, randevunun kendi tarih/saatini HIÇ taşımıyor — kopukluk burada. Çözüm planı (Claude → Kaan, uygulama onay bekliyor): o buton `randevuTarih`/`randevuSaat` (belki `randevuId`) parametrelerini de geçsin; `session/new` sayfası bu değerler varsa NOTYA-GECMIS-MUAYENE-01'de az önce eklenen "Geçmişe dönük tarih" alanını O değerle önceden doldursun (DÜZENLENEBİLİR bırakılır — doktor geç kaldıysa ya da hasta beklenenden erken/geç geldiyse temizleyip "şimdi"ye dönebilsin); randevusuz (walk-in) muayenelerde alan boş kalır, davranış değişmez. Bu, test-only değil TÜM doktorlar için kalıcı varsayılan davranış olmalı (yukarıdaki serbest-tarih alanının aksine) — klinik doğruluk artıyor (kaçıncı vizit, yaş-anlık ölçüm, lab trend sıralaması hep gerçek muayene anına göre olur; AI/onay gecikmesi hiç karıştırmaz). Kaan'ın onayını bekliyor.
