...
| ID | Status | What | Waits on |
|---|---|---|---|
| NOTYA-AYSE-STANDART-01 | **DONE 2026-09-26 — kod** (branch feat/ayse-standart, not pushed) | Event index + statuses at read time, plan→record matching, open loops, 28 synonym groups, router for the 10 questions, evidence blocks + templates, rules block wired into `ayseCevapla.ts` (skips the HIZLI KART for file questions), pediatri parameters, 4 synthetic audit files with golden expectations in `npm test`, live scorer script. No migration, no new UI. | — |
| NOTYA-AYSE-STANDART-02 | OPEN 2026-09-26 | Semantic search (pgvector) for "aynı şikayet" — this build searches approved note text with synonym expansion only. | Kaan (embedding cost + migration decision) |
| NOTYA-AYSE-STANDART-03 | OPEN 2026-09-26 | Adult vaccine schedule parameters (tetanoz-difteri, grip, pnömokok, zona, Hep B; gebelik Tdap/grip) for dahiliye / aile hekimliği / kadın doğum. Until then Soru 4 says "erişkin aşı takvimi parametreleri henüz tanımlı değil". Numbers only from verified TR sources. | Claude (sources) → Kaan |
| NOTYA-AYSE-STANDART-04 | OPEN 2026-09-26 | Chapter question (the "Soru 8" slot) + parameters for the other 29 branşlar. | Kaan (order of branşlar) |
| NOTYA-AYSE-STANDART-05 | OPEN 2026-09-26 — NEXT SPRINT (Kaan, 2026-09-26): after Dr. Gökhan signs off the pediatri İlk 10, (a) Core 100 — daily questions any Notya doctor asks, written once with per-branş parameters (aşı takvimi, referans aralıkları, tarama takvimi, ölçüm trendi), each with answer criterion + golden fixture; (b) per-branş chapter sets for all 30 branşlar + Klinik, sized per specialty (~15–30), wave order KD → dermatoloji → aile hekimliği → most popular first, Turkish references, research-built to 80–90% then specialist revision; (c) harness runs core 100 against every branş and each chapter set against its own after every deploy, per-branş scorecard in docs/denetim/. Gate: pediatri İlk 10 signed off first. | waits on Dr. Gökhan sign-off → Claude |
| NOTYA-AYSE-STANDART-06 | OPEN 2026-09-26 | Live scoring on real files (`scripts/ayse-denetim/canli.mts`) — only run on QA accounts so far; a run on real pediatric files needs Dr. Gökhan's consent and review of the answers. | Dr. Gökhan |
...
## NOTYA-ARSIV — arşivlenen muayene her yerde gizli (Dr. Gökhan via Kaan, 2026-09-23)

**Requirement:** a muayene the doctor archives must not appear on ANY visible surface until it is taken
out of the archive; the record itself stays.

| ID | Status | What |
|---|---|---|
| NOTYA-ARSIV-01 | **DONE 2026-09-23** (branch `fix/arsiv-her-yerde-gizli`) | One rule, one helper: `lib/doktor/arsiv.ts` (`arsivsizSeanslar` / `arsivsizNotlar` — notes via an aliased `sessions!inner(archived_at)` embed). Applied to every display / count / AI-context / export read: dashboard Son notlar + all KPI cards + Bu hafta (raporlar), Ayşe gün özeti (NOTYA-GUN-01) + Bugün programı (NOTYA-GUN-02), İnceleme queue, hasta dosyası Özet (Özet no longer inherits the Arşivlenenler toggle), Ayşe dossier (hastaDosyaDerleyici → konsult / not-konsult / SOAP bağlamı), sessions/end + ses-yukle "ÖNCEKİ VİZİT PLANI" and style examples, Belgeler/Lab "latest note" targeting, Raporlar, epikriz, Sağlığım portal + portal paylaşım özeti, FHIR/HL7/webhook export, all 30 branş kohort routes (→ Fısıltı), büyüme eğrileri, doz önerisi, ilaç uyarısı kilo, hasta arama, "son hastam" çözümleyici, günün notuna ekle, hafıza ritmi, konsültasyon taslağı. Direct open by id (not / yazdır / reçete) stays allowed and shows an **Arşivde** banner. New `POST …/sessions/[sessionId]/arsivden-cikar` + **Arşivden çıkar** button in Arşivlenenler. Guard: `lib/doktor/arsiv.test.ts` (in `npm test`) fails on any new raw notes/sessions read that is not allowlisted with a reason, and proves arşivle → hidden / arşivden çıkar → back on the real routes. No migration (`sessions.archived_at` + partial index already exist, 089). `notes.session_id` is NOT NULL (0 null rows in prod, checked 2026-09-23), so the inner join hides nothing else. |
| NOTYA-ARSIV-02 | **DONE 2026-09-23** — decided by Kaan 2026-09-23 (confirmed by Dr. Gökhan) | An archived muayene's drugs disappear too — hasta dosyası İlaçlar, Sağlığım › İlaçlarım + portal paylaşım özeti, Ayşe dossier / etkileşim / ilaç eylemleri, reçete (medula + e-reçete), gün programı, hasta arama, konsültasyon, dahiliye/nöroloji/psikiyatri/pediatri cards and all kohort routes. **Read filter, no row change:** `arsivsizIlaclar` in `lib/doktor/arsiv.ts` hides a `hasta_ilaclar` row while its `kaynak_note_id` note's muayene is archived; unarchive → back. Hand-added rows (`kaynak_note_id` NULL) always visible. Edge case fixed in `lib/doktor/receteAktarim.ts`: an unchanged re-prescription by a later approved note now moves `kaynak_note_id` to that note (before, the row stayed on the older note, so archiving it would have hidden a drug a live note still prescribes). Production check (read-only, 2026-09-23): 14 rows, 11 hand-added, 0 with an archived source; 1 real stale-source row (Dr. Gökhan, D vitamini damla) → backfilled with one plain `UPDATE … kaynak_note_id` (rollback list kept outside the repo); 2 qa.test rows point at the latest note containing the drug (unapproved) → left as is. Guard in `lib/doktor/arsiv.test.ts` extended to `hasta_ilaclar` reads. |
| — | note | `app/api/doktor/araclar/sgk-rapor` reads `notes.patient_id`, a column that does not exist → the query has always errored and the SGK draft never used notes (falls back to hekimNotu). Archive rule is applied, the filter is left as is: **DONE 2026-09-23 (NOTYA-SGK-RAPOR-02)** — Kaan approved the data flow; query fixed (notes via sessions!inner(patient_id), doctor-scoped, approved, non-archived, newest 3) and the call moved from Groq/xAI to aiCagir (Anthropic, klinik-analiz) so no new health-data processor; pseudonymize + TCKN guard kept. |

## NOTYA-CEK-DOGRULA — "Eksik kalanlar" çek listesi doğru ve canlı (Dr. Gökhan, 2026-09-23)

| ID | Status | What |
|---|---|---|
| NOTYA-CEK-DOGRULA-02 | **DONE 2026-09-23** (branch `fix/cek-dogrula`) | 1-month visit listed D vitamini / işitme taraması / işitme risk / GİDR 6–9 ay / göbek / kalça as missing. Causes: tarama items had their kalem code as their only keyword (`dvit`, `gidr`); `vizitPlani` got no patient records; future windows (`surekli`) were counted as missing; no scope (per-visit vs whole file); panel parsed text written once at note creation and the LLM re-evaluation just copied it. Fix: real Turkish keywords (`TARAMA_ANAHTARLARI`, word-start matching), records loaded with the kohort loader (`pediKohortGirdileri`), only due-now + "her izlemde" items, `kapsam: 'vizit' \| 'dosya'` with new status **✓ önceki kayıtta var** (previous approved notes, intake, hasta_ilaclar, tarama/aşı records; archived excluded). One source of truth `lib/doktor/cekListeSunucu.ts` used by sessions/end, GET notes/[id], approve and the data fix; the note page recomputes the panel from the current fields on every edit; approve rewrites the block server-side; not-konsult strips the block from what the LLM sees and returns. Kalça stays per-visit (Gökhan). Judgment call: "İşitme risk faktörleri" is `dosya` and counts as done when hearing status (screening / audiology) or a risk-factor review is on file. Data: 7 notes of Dr. Gökhan got only the ÇEK block rewritten (rollback JSON outside the repo). Mobile: no live device check. The only new UI is a wrapped text line inside the existing panel, which already stacks on top on phones. |
| — | open, waits on Dr. Gökhan | Umutcan Türkoğlu's patient card has DOB **15.06.2024**, which is the 1-month visit date. His notes show 3 days old on 18.05.2024, so the real DOB is about 15.05.2024. Visits before 15.06.2024 therefore get no age-specific items, and the 1-month visit is treated as the newborn (day 0) band. Not changed: this is patient data only he can correct. |

## NOTYA-ASI-NOT — muayenede yapılan aşı, not onayında aşı kartına (Kaan + Dr. Gökhan, 2026-09-23)

| ID | Status | What |
|---|---|---|
| NOTYA-ASI-NOT-01 | **DONE 2026-09-23** (branch `fix/asi-not`) | SOAP generation (sessions/end + ses-yukle) extracts `asilar` = ONLY vaccines stated as given in this visit; a deterministic backstop (`lib/doktor/notAsilari.ts` → `uygulananAsilariSuz`) drops anything the transcript does not say was given this visit (planned / recommended / next visit / given before / not given / disease mentions), drops unspoken dose / lot / site, and dates every row with the visit day (backdated visits keep their date). Name = the karne / voice-Ayşe normalizer (`asiAdiNormalize`, moved to `lib/asi/karneOkuma.ts`). Unspoken dose → next in series from the card, flagged `doz_hesaplandi` (yellow "Ayşe hesapladı — kontrol edin"). Stored in `notes.content_asilar` (migration 091, applied 2026-09-23). Note form: editable "Bu muayenede uygulanan aşılar" rows (aşı, doz, tarih, lot, yer; add / remove), sent to not-konsult and approve, edits logged in `not_duzenlemeleri`. Approve (first + re-approval) → `lib/doktor/notAsiAktarim.ts` syncs into `asilar` (kaynak `kayit`, hekim_onay_at, kategori by age, doktor_id, patient_id, new `asilar.kaynak_note_id` FK ON DELETE SET NULL, `sonraki_doz_tarihi` from the pediatric `asiPlani` engine for a child on the national schedule). Idempotent per note: updates this note's rows, deletes rows the doctor removed (only `kaynak_note_id` = this note), never duplicates. Dedupe vs other sources: same vaccine + same date → not inserted; same dose on another date, or another dose on the same date → form warning ("Bu hastada Hepatit B 2. doz zaten kayıtlı (…)"), not written until the dose changes; other sources' rows are never modified. Archive: `arsivsizAsilar` on every `asilar` read (aşı kartı, karne / portal / PDF, Ayşe dossier + konsült + eylem mükerrer, kohort / Fısıltı / çek listesi, hatırlatma, search); guard extended in `lib/doktor/arsiv.test.ts`. Checklist: a vaccine in the note satisfies "Aşı durumu". Tests: `lib/doktor/notAsilari.test.ts` (golden + sync + archive) and a real-route approve → archive → unarchive case. |
| NOTYA-ASI-NOT-02 | **OPEN — waits on Kaan** | Backfill WRITE for past approved, non-archived notes. Preview only so far: `npx tsx scripts/asi-not-backfill.mts dr.gokhan@notya.ai` (read-only) → 8 notes, 7 vaccine rows: 3 would-insert, 4 conflicts (all Umutcan T.: his card already has Hepatit B 1. and 2. doz both dated 15.06.2024, the same date as the suspect DOB noted under NOTYA-CEK-DOGRULA, and three of his notes each say a Hepatit B 1st dose was given). No write until Kaan approves the table. |

## NOTYA-ASI-LOT — lot no + uygulama yeri on the Aşılar list and the Aşı Karnesi (Dr. Gökhan, 2026-09-24)

| ID | Status | What |
|---|---|---|
| NOTYA-ASI-LOT-01 | **DONE 2026-09-24** (PR #389 fields + display, branch `fix/asi-lot`; part 2 backfill, branch `fix/asi-lot-backfill`) | Lot / site written in muayene notes appeared nowhere: the approval sync packed them into `asilar.notlar` ("Lot: … · Uygulama yeri: …"), which no view renders, and the NOTYA-ASI-NOT-02 backfill passed only name / dose / date. Part 1: migration 093 adds `asilar.lot_no` + `asilar.uygulama_yeri` (nullable) and lifts any packed "Lot:" / "Uygulama yeri:" segment out of `notlar` (other text kept, same order; 0 such rows existed in production, applied 2026-09-24). Writers use the columns: approval sync (`notAsiAktarim`), Aşılar form + `POST/PATCH /api/doktor/asilar` (two optional inputs "Lot no", "Uygulama yeri"), voice Ayşe `asi_kaydi_ekle` (optional fields, dropped when guessed). Display: Aşılar list "Lot: X · Yer: Y" under the Uygulandı line only when known; Aşı Karnesi model (`lib/asi/karneBelgesi.ts`) carries `lotNo` / `uygulamaYeri` → Sağlığım screen + print and the single PDF (doctor + portal) get "Lot / Uygulama yeri" (empty cell when unknown). Karne okuma (photo) does not read lot today — not changed. Part 2: `asiLotYeriBul` (`lib/doktor/notAsilari.ts`) reads lot ("lot", "lot no", "lot numarası", "seri no" + a code with a digit) and site (IM / SC / ID / oral / ağızdan / intramüsküler / subkutan / deri altı / sağ-sol uyluk / deltoid / omuz / bacak / kol / kalça …) from the vaccine's OWN segment (per-vaccine split, so "KKK 1. doz (sol omuz, lot: MMR12345), Suçiçeği 1. doz (sağ omuz, lot: Rix12345) uygulandı" never mixes lots); given-this-visit sentences win, two different values → empty, no marker → empty. Also the SOAP-generation backstop when the model left lot / site empty. `scripts/asi-lot-backfill.mts <e-mail> <rollback.json> [--uygula]` run for dr.gokhan@notya.ai (Kaan approved): 9 note-sourced rows, 7 filled (lot 6, site 7) + 3 notes' content_asilar — Grip 15.02.2025 Vaxi12345 / IM, MenACWY 15.02.2025 acwy12345 / IM, KKK MMR12345 / sol omuz, Suçiçeği Rix12345 / sağ omuz, MenACWY 2 Men12345 / sol bacak, Hep A Hav12345 / sağ bacak (15.05.2025), Rotavirüs 16.07.2024 site ağızdan; Karma + KPA 16.07.2024 left empty (the note states none). Rollback: `~/notya-ai-asi-lot-rollback-2026-09-24.json`. |

## NOTYA-FISILTI-GIZLE — Fısıltı gizle + aşı "kayıt tutarsız" (Kaan + Dr. Gökhan, 2026-09-24)

| ID | Status | What |
|---|---|---|
| NOTYA-FISILTI-GIZLE-01 | **DONE 2026-09-24** (PR #385 logic, PR #386 Gizle) | **Logic (#385):** pediatri kohort flag `asi_kayit_tutarsiz`: a recorded dose before birth, on the same day as the previous dose, or before the GBP minimum age/interval already in `asiPlan` → "Aşı kaydı tutarsız — <aşı> <doz> tarihi / doğum tarihini kontrol edin" (aşı tab), and that series is no longer called "gecikti". The parent 1-tap reminder skips children whose only flag is this one. Golden: Umutcan (DOB 15.06.2024, Hep B 1+2 on 15.06.2024) → tutarsız; corrected DOB 15.05.2024 + 0-1-6 → no Hep B item. **Gizle (#386):** "Gizle" on the Fısıltı card → "Bu uyarıyı gizle" (until the facts change) or "7 gün sonra hatırlat"; "Gizlenenler (n)" → "Geri getir". Table `fisilti_gizlenen` (migration 092, applied 2026-09-24; RLS doctor-only) stores only the item key + SHA-256 of the item's facts (no name / text), `until`, soft `kaldirildi_at`. A hide only matches while the hash matches, so a new late dose or a corrected date brings the item back. POST looks the item up in the doctor's own freshly-built list (patient id never from the body); hides / restores go to `audit_logs`. Candidate collection moved to `lib/doktor/fisiltiTopla.ts` (shared by `/api/doktor/fisilti` and `/fisilti/gizle`); client fetches are `no-store`, so a dose written by an approved note (NOTYA-ASI-NOT) shows on the next load. No delete option (clinical safety). |
| NOTYA-FISILTI-GIZLE-02 | **OPEN — waits on Kaan + Dr. Gökhan** | Proposed "Gizle" reason picker: *Yapıldı — kaydet* (opens the prefilled aşı card), *Başka kurumda takip ediliyor*, *Yanlış uyarı* (→ feedback). Not built; build only after their decision. No Sil/delete option for Fısıltı (clinical safety). |
...
| 2026-09-16 | **NOTYA-JINE-03 — şiddet tarama + endometriyal örnekleme→Belgeler + KOK yıllık TA/kilo — SHIPPED inside JINE-04** | Şiddet checkbox + sevk görevleri; kok_yillik API; örnekleme bridge remains via aub_guncelle + Belgeler flow. |
...
| 2026-09-02 | **Klinik meslektaş v2 kalemleri** | (a) Sesli asistan (ElevenLabs Ayşe) için dosya erişimi — konsült şu an metin sohbet (Ayşe'ye Danış sekmesi); ses entegrasyonu ayrı iş. (b) Konsült içinde röntgen/EKG görselini inline açma (v1 kayıt listesi + Görüntüleme sekmesine yönlendirme). (c) Eski hastalara e-posta backfill (yeni kayıtlarda zorunlu; eskilerde boş). (d) Cron hatırlatmaların TRT denetimi. (e) Randevu API'sinde e-posta sunucu tarafı zorunluluğu (şu an istemci zorunlu kılıyor; sekreter/asistan çağrıları için sunucu doğrulaması eklenmeli). Claude yapar |
| 2026-09-02 | **Sadelik O6 — Araçlar emekliliği (v2)** | Asistanın karşıladığı araçlar kullanım verisiyle tek tek kaldırılacak: ilac-interaksiyon (Ayşe dosyadan proaktif uyarıyor), hatirlatma (cron+randevu akışı), hasta-portali (intake linkleri). Kullanım birikince Claude yapar |
| 2026-09-02 | **SOAP öğrenme v2 + not PDF** | (a) not_duzenlemeleri farklarını periyodik damıtıp doktor stil profiline çevir ve prompta ekle (v1 few-shot bugün aktif). (b) Onaylı notun PDF/yazdır çıktı. (c) İnceleme Reddet butonuna gerçek yeniden-üretim akışı. Claude yapar |
| 2026-09-02 | **SOAP-03 kapanan kalemler** | Ses dosyası yükle->SOAP (ElevenLabs Scribe, ham ses silinir), plan sürekliliği (önceki vizit planı değerlendirilir), Yazdır/PDF sayfası (attestasyon + düzenleme sayısı + imza alanı). Kalan: ekranda satır-satır versiyon geçmişi UI (log tabloda birikiyor); ambient diarized kayıt v2. Claude yapar |
| 2026-09-03 | **Sesli-Ayşe (ElevenLabs) dosya bilinci + kademe zorlaması** | (a) ElevenLabs ajanına webhook tool eklenip dosya sorguları sunucudan cevaplanacak (endpoint + kısa ömürlü token mimarisi hazır değil; ajan konfigürasyon oturumu gerekli - Kaan ile birlikte). (b) Abonelik lansmanında kademe zorlaması: temel=yazılı sohbet, orta/pro=sesli 1:1 + seans limitleri (ai_kullanim tablosu altyapı olarak hazır). Claude yapar |
...
| 2026-09-05 | **FHIR Gateway P2-P4 (hastane eklentisi devamı)** | P2: PDF DocumentReference + kurum yönetim ekranı + Vercel cron kaydı. P3: ilk gerçek hastane onboarding (MRN eşleme, OAuth, onların test ortamı). P4: muayenehane katmanı için doğrudan Sağlık.Net/USS gönderimi (ayrı ray, sertifikasyon işi). P1 TAMAM: mapper + kuyruk + HAPI kanıtı (PR #107). Claude yapar |
| 2026-09-05 | HBYS satıcı hedef sırası (Kaan zorluk tablosu) | P3 kanal sırası: 1) Sisoft + Probel (olgun API/HL7), 2) Fonet (ortaklık API), 3) Enlil (interface engine), 4) Talya/Kardelen/Meddata (müşteriye özel). SB HBYS.API spesifikasyonu ilk onboardingde çekilecek. İlk mektuplar ödeyen doktorlarla | K+C |
| 2026-09-06 | QA BOT PASS #1 (test hesabı qa.test@notya.ai, sentetik hasta) | SONUÇ: çekirdek döngü UÇTAN UCA YEŞİL - giriş, hasta oluştur, yazılı seans, not üretimi (güven %95, anamnez düzeni tam, vitaller doğru, mg/kg hesap doğru, ayırıcı tanı + kritik bulgular paneli), İncele+Onayla. BULGULAR: (1) /login 404 - küçük, /giris gerçek yol; (2) DÜZELTİLDİ PR #114: manuel hasta ekleme bloklu idi (formda olmayan eposta alanı + TC zorunluluğu adım-1de ateşlenmiş); (3) eposta doğrulaması adım-1de adım-3 alanına bakıyordu - kök neden sıra hatası. AÇIK KARAR (Kaan): veli e-postası zorunlu mu opsiyonel mi kalsın (şu an opsiyonel) | C yaptı |
| 2026-09-06 | QA BOT PASS #2 (derin özellikler) | DÜZELTİLDİ: #1 /login->/giris 308 (PR 116); #4 Muayene Geçmişi HER hastada boştu - sessions sorgusu notes(...) gömme yerine kolon sanıyordu, catch sessizce yutuyordu (PR 117); #5 FHIR worker notes.status kolonu yok, approved_at olmalı (PR 117). DOĞRULANDI YEŞİL: dosya zaman çizelgesi notu gösteriyor + Yazdır/PDF; yazdır sayfası HBYS köprüsü iki düğme (kopyala->Kopyalandı, HTML indir) çalışıyor; Ayşe chart-aware konsült MÜKEMMEL - yazdığımız notu okuyup tanı+ICD+antibiyotik+doz+takip doğru getirdi. AÇIK: sesli Ayşe dosya bilinci (bilinen sınır), kota/tier davranışı test edilmedi (yeni hesap limitsiz)
| 2026-09-06 | QA BOT PASS #3 (yüksek-riskli yüzeyler) | ÜÇÜ DE YEŞİL, sıfır bug. (1) REÇETE/ETKİLEŞİM: varfarin kronik ilaç kayıtlı hastada 2. vizitte antibiyotik önerisi - motor TMP-SMX+varfarin ciddi etkileşimini (CYP2C9, K vit, kanama YÜKSEK) gerçek farmakoloji ile yakaladı, amoksisilin-klavulanatı daha güvenli seçenek önerdi, ibuprofen+varfarin GI kanama için parasetamol önerdi, INR takibi şart dedi - jenerik kutu değil, gerçek ikili-ilaç muhakemesi. (2) PLAN SÜREKLİLİĞİ: 2. vizit notu önceki vizit planına + kronik ilaca (Coumadin) atıf yaptı. (3) BELGE YÜKLEME: type-to-search hasta seçimi, dosya ekle, X Kaldır kontrolü, Kasaya yükle->Kasa(0->1), arşivde Lab Sonucu etiketiyle + inline önizleme + İndir/Kapat. Uçtan uca çalışıyor | C |
...
| 2026-09-17 | **Depolama kovaları büyük olasılıkla HERKESE AÇIK.** `goruntuleme/yukle` `getPublicUrl()` ile, `belgeler/ingest` `/storage/v1/object/public/hasta-belgeler/…` ile URL üretiyor → URL'yi bilen herkes (doktor olsun olmasın) hasta görüntüsünü/belgesini indirebilir. Kova ayarları ve storage politikaları repoda yok (panelde). Kontrol: Supabase › Storage › `hasta-goruntuleme`, `hasta-belgeler`, `ses-kayitlari` → "Public" kapalı mı, politikalar `{auth.uid()}/` önekine mi kısıtlı. Kapatmak, görüntülerin doktor ekranında ve portalda imzalı URL'lerle gösterilmesini gerektirir — görüntüleme akışını değiştiren bir ürün değişikliği, gözetimsiz yapılmadı. Yeni yüklemeler için Belge Kasası (`lib/vault`, şifreli, public URL yok) zaten doğru yol. | DONE - verified 2026-09-25: hasta-belgeler, ses-kayitlari, mali-belgeler are private; only motorlar (engine files, no PHI) is public |
| 2026-09-17 | **Repoda tanımı olmayan tablolar:** `not_duzenlemeleri` (notun önce/sonra klinik metni), `fhir_audit`, `fhir_export_kuyruk`, `kurum_*`, `ai_kullanim`. RLS durumları bilinmiyor; 052 var olanlarını kapatıyor (`not_duzenlemeleri` → doktor politikası). Uygulama sonrası sorguyla teyit edin. | DONE - verified 2026-09-25: RLS enabled on all of them; not_duzenlemeleri has hasta_izolasyon_kendi_satiri |
| 2026-09-17 | **Cron kimlik doğrulaması taklit edilebilir** (izolasyon dışı, ama güvenlik): `cron/*` ve `entegrasyon/fhir/isle` GET, dışarıdan gönderilebilen `x-vercel-cron: 1` başlığına güveniyor → herkes hatırlatma/KVKK imha/FHIR dışa aktarım işlerini tetikleyebilir (yanıtlar PHI içermiyor; işler kendi kurallarıyla çalışıyor). Önerilen: yalnız `Authorization: Bearer $CRON_SECRET` (Vercel bunu `CRON_SECRET` tanımlıysa otomatik gönderir). Cron'ları kırma riski olduğu için gözetimsiz değiştirilmedi — önce Vercel'de `CRON_SECRET` tanımlı mı bakılmalı. | DONE 2026-09-25 - SEC-CRON-01: lib/cronYetki.ts; kvkk-imha, randevu-hatirlatma and fhir/isle accept only Bearer CRON_SECRET (sent by Vercel; CRON_SECRET set in Production) or ?secret=; the x-vercel-cron header is no longer trusted. 3 tests. |
...
|---|---|---|---|
| 2026-09-17 | **Bilinen çocuk hastada baseline branşlarda pediatri sekmeleri** (kardiyoloji, KBB, üroloji, ortopedi… → Büyüme, M-CHAT, Gelişim, Bebek kartı) | Aynı gün yazılmış CHART-TAB-POLICY (`hastaDosyaSekmeleri.ts`) bunu bilerek açık bırakıyor ("baseline/aile mixed care"); Kaan'ın yeni kuralı ("pediatri alanı kardiyoloji hekimine çıkmaz") ise tersini söylüyor. Politikayı sessizce çevirmedim. **Öneri:** `pediatriAracSekmesiUygun`'u `pediatrikBaglamMi`'ye bağla (pediatri/çocuk cerrahisi her zaman, aile yalnız çocukta, diğerleri asla) — tek fonksiyon, testler 30/30 döngüyle hazır. | DONE 2026-09-25 - #409: Kaan ruled it specialty bleed-over. Allow-list: pediatri, crossover aile hekimliği / genel pratisyen / no branch. 3 tests. |
| 2026-09-17 | **"Kadın Sağlığı & Gebelik" sekmesi her branşta** (≥12 yaş kadın hastada tam KD bölümü: NST, risk formu, VTE, gebe kartı) | "Mixed care" politikası olarak belgelenmiş; ama tam KD bölüm UI'sinin göz/derm/kardiyoloji hekimine açılması tam da bu denetimin sızıntı tanımı. Gebelik bilgisi ise her branş için güvenlik bilgisi (isotretinoin, görüntüleme, ilaç). **Öneri:** her branşa salt-okunur "Gebelik durumu" çipi; tam KD bölüm UI'si yalnız KD (±aile) hekimine. Sunucu tarafı (`/api/doktor/gebelik`, `jinekoloji`, `kadin-sagligi`) bugün yalnız hasta sahipliğine bakıyor, branşa değil — karar verilince orada da aynı kapı. | DONE 2026-09-25 - Kaan: do it the Turkish way. Full KD section: kadın doğum + subspecialties and primary care (aile hekimi shares DOB with kadın doğum in the SB system). Every other specialty: header safety chip Gebe · N. hafta only (TİTCK KUB isotretinoin pregnancy test, pre-imaging pregnancy query, prescribing). 4 tests. |
...
| Date | Item | Durum |
|---|---|---|
| 2026-09-24 | notya.io domain owned; migrate once the new website ships (DNS, sender domain for email/SMTP, any hardcoded notya.ai references). | MOSTLY DONE 2026-09-25 - https://www.notya.io is the official address and live (Kaan confirmed). Done: domains on Vercel project notya-ai; Namecheap A @ / A www -> 76.76.21.21; NEXT_PUBLIC_APP_URL + NEXT_PUBLIC_SITE_URL = https://www.notya.io in Production; #413 moved every notya-ai.vercel.app fallback, CORS and login placeholder to notya.io. Supabase Auth URLs not needed today (app uses password login only; add www.notya.io when email-link login is built). OPEN (waits on Kaan): delete the Namecheap URL Redirect Record on @ (parking) - until then the bare notya.io randomly hits the parking page; then Claude sets notya.io -> www redirect in Vercel. OPEN (waits on Kaan decision): mailboxes on notya.io (kvkk@, merhaba@) - the KVKK page and Resend sender still use notya.ai, which we do not own, so KVKK mail currently cannot be received; switch once mailboxes exist. OPEN (decision): FHIR identifier namespace https://notya.ai/fhir/sid -> notya.io. |
...
| Date | Item | Durum |
|---|---|---|
| 2026-09-24 | Sprint 1 (doktor): tüm iş bitti (renk denetimi 252 dosya, Sidebar B, 8 yazdırılabilir belge + Aşı Karnesi, KVKK footer, design-token konsolidasyonu, Sağlığım portal hizalaması — son madde Kaan'ın açık onayıyla eklendi). Tam doğrulama temiz (2617/2617, build clean). Kalan tek şey Kaan'ın son onayı + açık "git" talimatı. | DONE 2026-09-25 - redesign live on main; post-launch fixes #393-#405 |
| 2026-09-24 | Sprint 2 (klinik): Sprint 1 canlıya alındıktan SONRA başlar — ~90 sayfa, aynı şablon (doktor redesign'ı ile aynı boyutta veya daha büyük). | OPEN - unblocked (Sprint 1 live); waits on Kaan to start |
| 2026-09-25 | NOTYA-RECETE-04: Plan text vs structured İlaçlar list divergence warning (planIlacTutarsizMi + confirm before approval), PR #404. Unit-tested (4 tests incl. the live Umutcan Augmentin/Amoksisilin case), build and 2638/2638 clean. NOT live-verified: the native window.confirm froze the browser automation. PR is on hold, not merged. To close: on the #404 Vercel preview, open a note whose Plan names a drug that is not in the İlaçlar list, press Onayla, confirm the dialog appears and lists the İlaçlar entries; then merge. | SUPERSEDED 2026-09-25 - the native window.confirm was replaced by the in-page card (see the entry below); live check moves to that entry |
| 2026-09-25 | NOTYA-RECETE-04 follow-up (Dr. Gökhan, Umutcan, note 0a57965e): DB confirms he changed ONLY the Plan text at 21:50 (Augmentin ES 10 gün, Calpol); content_ilaclar was never edited, so the reçete and hasta_ilaclar correctly still show Amoksisilin 7 gün + Parasetamol. The 198bf41 deactivation fix does not apply (the list never changed), so re-approving alone will NOT fix it. Data fix = doctor edits the İlaçlar list on that note and re-approves (no silent DB write). DECIDED by Kaan 2026-09-25 (Apple-style ease of use): when they diverge, Ayşe rebuilds the İlaçlar list from the Plan text and the doctor confirms it in one tap on an in-page card (IlacUyumKarti), on both the note page and İnceleme; implemented in PR #404. Once live, Dr. Gökhan fixes Umutcan by re-approving the note and tapping the update. | DONE 2026-09-25 - live-verified on a local dev server running commit e297adc9, on the kaanari test account with a TEST note reproducing the Umutcan mismatch: card appeared, Ayşe extracted Augmentin ES 7.5 ml 10 gün + Calpol 5 ml (no invented duration), one tap saved exactly those two to the note and hasta_ilaclar; re-approving the now-consistent note showed no card. Test rows deleted afterwards. Merged via #404. Remaining for Dr. Gökhan: re-approve the Umutcan note and tap the update. |
| 2026-09-25 | Ayşe routing: asked whether Umutcan ever got an antibiotic and which, she answered with the practice-wide last-30-days most-prescribed statistic instead of the patient medication history (answer happened to match, but wrong source and wrong time window for ever). Ledger item, not fixed. | DONE 2026-09-25 - NOTYA-AYSE-HASTA-01: a ranking question that names exactly one patient is answered from that patient file (tekHastaSorusuMu), never the practice-wide statistic. Tested with Dr. Gökhan exact sentence. |
| 2026-09-25 | NOTYA-RECETE-05: close the added-drug gap in the one-tap İlaçlar check. Before: the check ran only when a LISTED drug was missing from the Plan, so a drug ADDED to the Plan (existing ones kept) was never caught. Now it also runs whenever the Plan was edited since the note opened, and Ayşe pre-reads the edited Plan in the background (cached per Plan text) so Onayla is usually instant. If Ayşe cannot read the Plan and there is no concrete mismatch, approval continues without a card. Branch fix/plan-ilac-eklenen, PR #405 (build clean, 2648/2648). Live check paused 2026-09-25: the Chrome extension disconnected. A TEST note is left on the kaanari test account for it (note dab72ba0, session 7d3ee708; Plan and list both Amoksisilin + Parasetamol). Check: add a new drug line to the Plan, wait a few seconds, press Onayla, expect the card listing 3 drugs; then delete the test rows and merge. | DONE 2026-09-25 - live-verified on a local dev server on the kaanari test account: added c) Ventolin nebul 3x1 5 gün to a Plan whose list had only Amoksisilin + Parasetamol; on Onayla the card appeared within about 1 s (background pre-read) with all 3 drugs; Vazgeç saved nothing (DB checked). Test rows deleted. Merged via #405. |

| 2026-09-25 | SEC-RLS-OFF-01: 17 public tables have RLS disabled and are readable/writable with the anon key: all mali_* and avukat tables (musevvekiller, deliller, avukat_sessions, sure_takibi, dilekce_kuyrugu...), plus logs, normalized_transactions, ingestion_log, review_queue. Current exposure is minimal (16 empty, mali_belgeler 3 rows) because Mali/Avukat have not launched. Fix: enable RLS with owner policies (migration 094) before Sprint 3 ships anything. | DONE 2026-09-25 - migration 094 applied; all 311 public tables now have RLS enabled. Sprint 3 must add owner policies before any browser-side Mali/Avukat access. |
| 2026-09-25 | SCOPE (Kaan): work concentrates on the doktor and Klinik sides only; Mali and Avukat come in the future. Their tables are locked by migration 094 once applied. | STANDING || 2026-09-25 | NOTYA-ILETISIM-01 (Kaan: build all now, Apple-simple): one-tap email + WhatsApp from the doctor's OWN accounts everywhere (mailto / Gmail web / Outlook web links, wa.me), Ayarlar > İletişim card, morning Bugün N mesaj hazır queue, staff (sekreter) access for randevu messages, patient contact consent (intake + profile), contact log, Twilio patient sender + dead Meta route retired, randevu cron fills the queue instead of sending. Claude Code job A, branch feat/iletisim-tek-dokunus, migration 095 (written by the job, applied after review). | DONE 2026-09-25 - #417 merged, migration 095 applied, live-checked on the test account (İletişim page, consent, WhatsApp link with the notya.io form link, Gönderildi mi prompt). |
| 2026-09-25 | NOTYA-ILETISIM-02: automatic email from the doctor's own mailbox via one-time Gmail ile bağlan / Outlook ile bağlan (send-only permission; replies and Sent folder stay in his mailbox). Job B, starts after A merges. Needs: Google Cloud OAuth client + Google app verification for the gmail.send scope (weeks; up to 100 test users meanwhile, Dr. Gökhan first) and a Microsoft Entra app registration. | CODE DONE 2026-09-25 (#416 via #420, migration 096 applied). Stays a calm Yakında until Kaan creates the Google Cloud OAuth client + Microsoft Entra app and sets GOOGLE_OAUTH_CLIENT_ID/SECRET, MS_OAUTH_CLIENT_ID/SECRET; then Google review (gmail.send; connections expire every 7 days until approved). Guide: docs/iletisim-kurulum-eposta.md. Waits on Kaan. |
| 2026-09-25 | NOTYA-ILETISIM-03: automatic WhatsApp from the doctor's own number via Meta WhatsApp Business coexistence (Business app keeps working on his phone, chats sync; Turkey supported). Job C, starts after A merges. Needs: Meta developer app + Embedded Signup config, Dream Türkiye business verification with Meta (or a partner such as 360dialog), approved utility templates for randevu reminders; per-message Meta fee for API-sent messages. | CODE DONE 2026-09-25 (#415 via #420, migration 097 applied). Stays Yakında until Kaan creates the Meta app + Embedded Signup config, Dream Türkiye business verification / App Review, webhook, env META_APP_ID, META_APP_SECRET, META_ES_CONFIG_ID, WHATSAPP_WEBHOOK_VERIFY_TOKEN. First live test on a real +90 Business number (Turkey support, number age, price). Guide: docs/iletisim-kurulum-whatsapp.md. Waits on Kaan. |
| 2026-09-25 | NOTYA-GELEN-BELGELER (Kaan): one Gelen Belgeler inbox for documents, images AND sound files arriving to the doctor (lab results, X-ray/ECG photos, consultation replies, WhatsApp voice notes). AI sorts the type, reads it (transcribes Turkish audio, converts HEIC, routes images/labs to the existing readers), suggests the patient with a confidence score, links consultation replies to the open request; the doctor files with one tap (auto-file off at first). Phases: (1) web app now - inbox, drag-and-drop, paste, in-app upload incl. camera and voice recording; (2) forwarding address on belge.notya.io - needs a mail-receiving account (e.g. Postmark) under Kaan, Claude sets DNS; (3) WhatsApp automatic capture with coexistence (NOTYA-ILETISIM-03); (4) native Share -> Notya on iPhone and Android after the App Store / Google Play release (Notya is a home-screen web app today). KVKK: consent line on the intake form, private storage (also fix the public-style URL in belgeler/ingest), unfiled items deleted after 30 days, filing audit log. DECIDED (Kaan, 2026-09-25): incoming documents are doctor-only by default; the doctor can turn the secretarys access (see and file) on or off with one switch in Ayarlar. | OPEN - phase 1 BUILT (Claude, 2026-09-25, PR feat/gelen-belgeler): /dashboard/doktor/gelen-belgeler + Ana Sayfa "N yeni belge" card + sidebar badge; drop anywhere, paste (files + plain text), upload, phone camera, in-app voice note; PDF / Word / Excel / JPEG / iPhone HEIC (server JPEG) / DICOM (browser reader) / sound (.opus/.ogg/.m4a/.mp3/.wav, Turkish transcript via ElevenLabs Scribe, audio kept); filing goes into the encrypted vault (medical_documents) + hasta_belgeler row + audit, lab results start the lab reader, open konsültasyon can be linked (belge_bagla); dedup by sha256; 30-day clean-up in kvkk-imha; secretary switch in Ayarlar (server-enforced); consent line on intake + hasta profili; ingest route fixed (storage reference + signed URL). Entry point for phases 2-3: gelenBelgeEkle (lib/gelenBelgeler/README.md). Waits on Kaan: apply lib/db/migrations/099_gelen_belgeler.sql (until then the inbox stays hidden / answers "kısa süre içinde"). Not done: TC "last digits" matching (only the TC hash is stored, so only a full 11-digit TC can match). Phase 2 waits on Kaan (mail-receiving account); phase 3 on NOTYA-ILETISIM-03; phase 4 waits on the app-store release |

| 2026-09-25 | NOTYA-ILETISIM-04: dispatcher wiring B and C into A - automatic reminders from the doctor own accounts (cron every 10 min, 07-21 TRT; non-clinical types only; consented channels; WhatsApp first then email; claim lock against double-send; falls back to the human queue with a reason). #420 merged, migration 098 applied, tsc clean, 2854/2854, build OK. Not testable live until ILETISIM-02/03 accounts exist. | DONE 2026-09-25 (live test waits on Kaan accounts) |
| 2026-09-25 | NOTYA-BETA-0925 (Dr. Gökhan live test, 4 fixes, branch fix/beta-0925-gokhan): (1) Ayşe answers identity/contact questions (anne/baba adı, veli, telefon, e-posta, adres, doğum yeri/tarihi) on the server without the model, in text chat and voice (values only on screen; voice agent gets a value-free sentence); parent names, birthplace and the demographics JSON no longer reach the model context. (2) Randevu modal: Cep telefonu (pre-filled, required when missing, saved to the patient record), WhatsApp consent checkbox (source randevu), post-save Hasta Bilgi Formu prompt; doctor and sekreter. (3) Intake form: noValidate + Turkish inline messages for every field. (4) sessions/end: one retry on transient SOAP failures, failed + sanitized error_message, single-line error log, failed session reused by the client, stuck processing sessions (incl. 98e6fb44) marked failed on the doctor's next session end. | MERGED as #422 (row was stale). Remaining, freeze lifted 2026-09-26: migration 100_iletisim_izin_randevu.sql applied to live DB and npm run ses:eylem-tools rerun (agents patched) on 2026-09-26; migration 101 verified applied earlier. Real-device mobile check superseded by Cursor's #441-445. E-posta requiredness DECIDED: stays required. |
| 2026-09-25 | NOTYA-SES-HASTA-01 (Dr. Gökhan): by voice Ayşe said there was no record of Ayşe Yeşil, while text chat found her. Cause: the voice agent sends the whole spoken sentence to hasta_bul; the clinical words (son muayene, özet) became search filters that excluded the exactly-named patient. Fix in lib/doktor/hastaCozumleyici.ts: an exact full-name (2+ words) match of one patient is final; a partially named patient is returned when the extra words find nothing, except for list questions. Verified against Dr. Gökhan live patient list with 10 phrasings incl. his exact sentence and the Merhaba Ayşe, bu hafta ateşli hastalarım kimler cohort case; tsc clean; 2902/2902. | DONE 2026-09-25 |
| 2026-09-25 | Intake form e-posta field: Kaan decided to keep it REQUIRED for now (NOTYA-BETA-0925 raised making it optional because many parents have no email). Revisit only if Kaan asks. | DECIDED |
| 2026-09-25 | NOTYA-TEK-BEYIN (Kaan: simplify code, architecture and design): voice Ayse and text Ayse become ONE brain. Today text runs our /api/asistan/chat pipeline while voice runs the ElevenLabs-hosted model with a copied prompt and separate routes (hasta-bul, ses-eylem), so every feature is built twice and drifts (e.g. NOTYA-SES-HASTA-01). Plan: one shared answer function used by text and by an ElevenLabs Custom LLM endpoint (OpenAI-compatible, streamed); ElevenLabs keeps only listening, turn-taking and speaking. One conversation and active patient across voice and typing; answers in two forms (screen / spoken); all 8 specialty agents. Kaan approved: an immediate natural filler line, streaming, and measuring response time on real questions before switching any doctor, with the current voice setup kept as fallback until the numbers are good. Remove duplicate voice routes only after the switch. Job G, starts automatically after Job E. BUILT (feat/tek-beyin PR, not merged, nobody switched): lib/asistan/ayseCevapla.ts is the one brain (text route = thin wrapper, byte-identical responses); /api/asistan/ses-llm/v1/chat/completions is the ElevenLabs Custom LLM endpoint (SSE, server secret + signed per-conversation token, filler line, streamed answer); spoken Evet/Hayir runs outside the model turn through the tap's spine (lib/asistan/sesliOnay.ts); /asistan shows voice turns' screen text and cards via light polling (/api/asistan/ses-ekran) and shares one asistan session with the typed chat. 3 Custom LLM test copies of the 3 base agents (the 8 personas share them) via scripts/tek-beyin-ajanlari.mts; existing agents untouched. Latency (scripts/tek-beyin-olcum.mts, 20 real questions, QA dahiliye): filler at ~1 ms server-side, first real answer median 1.6 s, but model-generated turns 3.6-6.1 s; 16/20 identical screen text, 0 identity values spoken. Switch = env NOTYA_TEK_BEYIN_DOKTORLAR + 2 secrets (steps in the PR). Old voice routes (hasta-bul, ses-eylem, client tools) removed only in the clean-up PR after the switch. | PR OPEN - waits on Kaan review / switch decision |
| 2026-09-25 | GELEN-BELGELER-ROZET: after filing a document the sidebar Gelen Belgeler badge still shows the old count until the page reloads (seen in the live test of #426: filed 1 item, badge stayed at 1). Small fix: refresh the count after filing / deleting. | DONE 2026-09-25 - separate badge-only event (GELEN_SAYI_OLAY) fired right after filing; the list refresh keeps its 7 s delay so the confirmation card stays |
| 2026-09-25 | NOTYA-SES-KART-01 (Dr. Gokhan: voice answers are thinner than text). ElevenLabs transcripts 21:27/21:29 show the voice hasta_bul tool only returned a 563-char card with no examination, plan or follow-up, so voice said the kontrol date was not in the file although the note says Bir hafta sonra kontrol + 60. ay izlemi; it also said Kadin for a 5-year-old and called short-course drugs surekli ilac. Fix: the card now carries the last visit examination and plan/follow-up, kontrol/takip questions are answered from the plan, drugs read as aktif ilac, children as kiz/erkek cocuk. Interim fix for the current voice setup; NOTYA-TEK-BEYIN (Job G) makes voice use the full text pipeline. Checked on the live file; 43/43 card tests, full suite 2968/2968. | DONE 2026-09-25 |
| 2026-09-25 | NOTYA-TEK-BEYIN requirement from Dr. Gokhan (via Kaan): voice answers must be as detailed as text answers. The spoken answer (konusma) reads the same content as the text answer in natural sentences (summary sections, exam, diagnosis, treatment with doses, follow-up) - it is NOT a short version; the screen shows the formatted text at the same time. Only identity/contact values (VELI-YASAL-ONAM) and long numeric tables stay screen-only. Job G brief said short spoken answers; Claude adjusts this when reviewing G PR, before any doctor is switched. Done inside the G PR itself (lib/asistan/konusma.ts): konusma reads every sentence, heading and list item of the screen answer as natural sentences (doses kept, same dose/source locks as the screen); only sentences with phone / e-mail / T.C. no, the (d.t. ...) birth date and table rows stay screen-only, each replaced once by "Iletisim bilgisini / Tabloyu ekraniniza yazdim"; identity questions still say "ekraniniza yazdim". Tests in lib/asistan/tekBeyin.test.ts. | DONE in the NOTYA-TEK-BEYIN PR (awaiting merge) |
| 2026-09-25 | NOTYA-ILAC-SONLANDIR-01 (Kaan / Dr. Gokhan): when the approved note says to stop a medicine (keselim, iptal, sonlandiriyoruz, artik vermiyoruz, vermeyelim), it is ended in the patient Ilaclar list automatically: only at note approval, negation- and condition-aware, brand/generic aware, never a medicine the same note prescribes, sonlandir not delete, reason with the quoted sentence, one calm confirmation line with Geri al. Job H, queued behind Job G. | OPEN - waits on Claude (Job H queued) |
| 2026-09-25 | NOTYA-TEK-BEYIN live test: #432 merged; voice one-brain switched ON for Kaan test account (c4989e29) and Dr. Gokhan (9030fe09) via NOTYA_TEK_BEYIN_DOKTORLAR in Vercel production (redeployed 16:08 EDT); all other doctors on the old voice setup. Test agents point to https://www.notya.io/api/asistan/ses-llm/v1. Rollback for a doctor: remove the id from NOTYA_TEK_BEYIN_DOKTORLAR and redeploy production (~5 min). Known gaps to fix next: active-patient questions like En son ne zaman geldi routed to a cohort search (text + voice); no voice calendar availability check in the new path; 3-6 s wait on long model-written answers; old voice routes removed only after the test; /giris/doktor login page still dark. | OPEN - waits on Dr. Gokhan test feedback + Claude fixes |
| 2026-09-25 | NOTYA-AKTIF-HASTA-01 (Kaan / Dr. Gokhan): with a patient open, an unnamed question (En son ne zaman geldi?, Tansiyon takibini nasil planlarsin?) is answered from THAT patient file, not an all-patients search (0 hasta, Filtre: ...). Only explicit many-patient questions (hastalarim, kac hasta, hangi hastalar, kimler, tum/butun, en cok/en sik, toplam) stay a search; a name matching several patients still asks which. Shared function, so text and voice. lib/asistan/aktifHasta.ts + tests; 3002/3002. | DONE 2026-09-25 |
| 2026-09-25 | NOTYA-SES-DOLGU-01 (Dr. Gokhan, first tek-beyin voice test 23:34 TRT): he said Umutcan, eee, Turkoglunun dosyasina bak... su anda kac yasinda Umutcan? and voice answered Kayitlarda 0 hasta. Causes: the filler between first and last name broke the full-name match, and kac yasinda was treated as a count across all patients. Fix in the shared patient search (text + voice): fillers ignored, a patient is found when every part of the name appears (any order, words in between, split names like Umut Can), a named patient with a non-group question is final, addressing the assistant (Merhaba Ayse,) is never read as a patient name, and with a patient open a single search hit does not replace him. Checked on the live patient list with 11 phrasings; 3005/3005. | DONE 2026-09-25 |
| 2026-09-25 | KURAL — TÜRKÇE (Kaan): Notya bir Türk uygulamasıdır. Uygulamadaki her şey Türkçe olmalı ve Türkçe karakterlerle (ç, ğ, ı, İ, ö, ş, ü) doğru yazılmalı: arayüz metinleri, hata ve doğrulama mesajları, tarayıcı kontrolleri (dosya seçme, form uyarıları), Ayşe'nin yazılı ve sesli cevapları, hastaya giden e-posta ve WhatsApp metinleri, yazdırılan belgeler, PDF'ler. Türkçe kelime ASCII'ye çevrilmez (“Gokhan”, “Kayitlarda”, “kiz cocuk” gibi yazımlar yasak). Tüm Türkçe denetimi Job I olarak Job H'den sonra başlar. | STANDING + OPEN (Job I sırada) |
| 2026-09-25 | NOTYA-ILAC-SONLANDIR-01 implementation, branch feat/ilac-sonlandir-nottan. On approve (note page + Inceleme), after nottanIlacAktar: deterministic pre-check (stop verb + drug name/SGK brand in the same sentence) then one GUCLU call (klinik-analiz, temperature 0, JSON, note text + medicine names only) over this doctor's active, non-archived rows for this patient; every model answer is re-checked in code (id in candidate list, quote verbatim in the note, drug named in the quote, no negation/condition in its clause). Never touched: rows this note wrote, drugs the same note (re)prescribes (name, etken or SGK brand->etken exact), rows undone for this note. Row gets aktif=false, bitis_tarihi=today, notlar prefixed "Muayene notunda sonlandirildi: <quote>". One line + Geri al (POST /api/notes/[id]/ilac-sonlandir-geri-al, isolation-tested). IlacUyumKarti drops a drug the Plan stops. Migration 101_ilac_sonlandirma.sql (audit + undo memory) WRITTEN, NOT APPLIED. Real-model check on 8 synthetic Turkish cases: 8/8. | OPEN - to close: (1) apply migration 101; (2) live check on the kaanari test account: approve a TEST note whose Plan says "Klacid ... keselim" with an active Klacid row, see the line, tap Geri al, re-approve (not ended again), delete test rows; then merge |
| 2026-09-25 | NOTYA-TEK-BEYIN (Kaan: simplify code, architecture and design): voice Ayşe and text Ayşe become ONE brain. Today text runs our /api/asistan/chat pipeline while voice runs the ElevenLabs-hosted model with a copied prompt and separate routes (hasta-bul, ses-eylem), so every feature is built twice and drifts (e.g. NOTYA-SES-HASTA-01). Plan: one shared answer function used by text and by an ElevenLabs Custom LLM endpoint (OpenAI-compatible, streamed); ElevenLabs keeps only listening, turn-taking and speaking. One conversation and active patient across voice and typing; answers in two forms (screen / spoken); all 8 specialty agents. Kaan approved: an immediate natural filler line, streaming, and measuring response time on real questions before switching any doctor, with the current voice setup kept as fallback until the numbers are good. Remove duplicate voice routes only after the switch. Job G, starts automatically after Job E. BUILT (feat/tek-beyin PR, not merged, nobody switched): lib/asistan/ayseCevapla.ts is the one brain (text route = thin wrapper, byte-identical responses); /api/asistan/ses-llm/v1/chat/completions is the ElevenLabs Custom LLM endpoint (SSE, server secret + signed per-conversation token, filler line, streamed answer); spoken Evet/Hayır runs outside the model turn through the tap's spine (lib/asistan/sesliOnay.ts); /asistan shows voice turns' screen text and cards via light polling (/api/asistan/ses-ekran) and shares one asistan session with the typed chat. 3 Custom LLM test copies of the 3 base agents (the 8 personas share them) via scripts/tek-beyin-ajanlari.mts; existing agents untouched. Latency (scripts/tek-beyin-olcum.mts, 20 real questions, QA dahiliye): filler at ~1 ms server-side, first real answer median 1.6 s, but model-generated turns 3.6-6.1 s; 16/20 identical screen text, 0 identity values spoken. Switch = env NOTYA_TEK_BEYIN_DOKTORLAR + 2 secrets (steps in the PR). Old voice routes (hasta-bul, ses-eylem, client tools) removed only in the clean-up PR after the switch. | PR OPEN - waits on Kaan review / switch decision |
| 2026-09-25 | GELEN-BELGELER-ROZET: after filing a document the sidebar Gelen Belgeler badge still shows the old count until the page reloads (seen in the live test of #426: filed 1 item, badge stayed at 1). Small fix: refresh the count after filing / deleting. | OPEN - waits on Claude |
| 2026-09-25 | NOTYA-SES-KART-01 (Dr. Gökhan: voice answers are thinner than text). ElevenLabs transcripts 21:27/21:29 show the voice hasta_bul tool only returned a 563-char card with no examination, plan or follow-up, so voice said the kontrol date was not in the file although the note says Bir hafta sonra kontrol + 60. ay izlemi; it also said Kadın for a 5-year-old and called short-course drugs sürekli ilaç. Fix: the card now carries the last visit examination and plan/follow-up, kontrol/takip questions are answered from the plan, drugs read as aktif ilaç, children as kız/erkek çocuk. Interim fix for the current voice setup; NOTYA-TEK-BEYIN (Job G) makes voice use the full text pipeline. Checked on the live file; 43/43 card tests, full suite 2968/2968. | DONE 2026-09-25 |
| 2026-09-25 | NOTYA-TEK-BEYIN requirement from Dr. Gökhan (via Kaan): voice answers must be as detailed as text answers. The spoken answer (konusma) reads the same content as the text answer in natural sentences (summary sections, exam, diagnosis, treatment with doses, follow-up) - it is NOT a short version; the screen shows the formatted text at the same time. Only identity/contact values (VELI-YASAL-ONAM) and long numeric tables stay screen-only. Job G brief said short spoken answers; Claude adjusts this when reviewing G PR, before any doctor is switched. Done inside the G PR itself (lib/asistan/konusma.ts): konusma reads every sentence, heading and list item of the screen answer as natural sentences (doses kept, same dose/source locks as the screen); only sentences with phone / e-mail / T.C. no, the (d.t. ...) birth date and table rows stay screen-only, each replaced once by "İletişim bilgisini / Tabloyu ekranınıza yazdım"; identity questions still say "ekranınıza yazdım". Tests in lib/asistan/tekBeyin.test.ts. | DONE in the NOTYA-TEK-BEYIN PR (awaiting merge) |
| 2026-09-25 | NOTYA-ILAC-SONLANDIR-01 (Kaan / Dr. Gökhan): when the approved note says to stop a medicine (keselim, iptal, sonlandırıyoruz, artık vermiyoruz, vermeyelim), it is ended in the patient İlaçlar list automatically: only at note approval, negation- and condition-aware, brand/generic aware, never a medicine the same note prescribes, sonlandir not delete, reason with the quoted sentence, one calm confirmation line with Geri al. Job H, queued behind Job G. | OPEN - waits on Claude (Job H queued) |
| 2026-09-25 | NOTYA-TEK-BEYIN live test: #432 merged; voice one-brain switched ON for Kaan test account (c4989e29) and Dr. Gökhan (9030fe09) via NOTYA_TEK_BEYIN_DOKTORLAR in Vercel production (redeployed 16:08 EDT); all other doctors on the old voice setup. Test agents point to https://www.notya.io/api/asistan/ses-llm/v1. Rollback for a doctor: remove the id from NOTYA_TEK_BEYIN_DOKTORLAR and redeploy production (~5 min). Known gaps to fix next: active-patient questions like En son ne zaman geldi routed to a cohort search (text + voice); no voice calendar availability check in the new path; 3-6 s wait on long model-written answers; old voice routes removed only after the test; /giris/doktor login page still dark. | OPEN - waits on Dr. Gökhan test feedback + Claude fixes |
| 2026-09-25 | NOTYA-AKTIF-HASTA-01 (Kaan / Dr. Gökhan): with a patient open, an unnamed question (En son ne zaman geldi?, Tansiyon takibini nasıl planlarsın?) is answered from THAT patient file, not an all-patients search (0 hasta, Filtre: ...). Only explicit many-patient questions (hastalarım, kaç hasta, hangi hastalar, kimler, tüm/bütün, en çok/en sık, toplam) stay a search; a name matching several patients still asks which. Shared function, so text and voice. lib/asistan/aktifHasta.ts + tests; 3002/3002. | DONE 2026-09-25 |
| 2026-09-25 | NOTYA-SES-DOLGU-01 (Dr. Gökhan, first tek-beyin voice test 23:34 TRT): he said Umutcan, eee, Türkoğlu'nun dosyasına bak... şu anda kaç yaşında Umutcan? and voice answered Kayıtlarda 0 hasta. Causes: the filler between first and last name broke the full-name match, and kaç yaşında was treated as a count across all patients. Fix in the shared patient search (text + voice): fillers ignored, a patient is found when every part of the name appears (any order, words in between, split names like Umut Can), a named patient with a non-group question is final, addressing the assistant (Merhaba Ayşe,) is never read as a patient name, and with a patient open a single search hit does not replace him. Checked on the live patient list with 11 phrasings; 3005/3005. | DONE 2026-09-25 |
| 2026-09-25 | KURAL — TÜRKÇE (Kaan): Notya bir Türk uygulamasıdır. Uygulamadaki her şey Türkçe olmalı ve Türkçe karakterlerle (ç, ğ, ı, İ, ö, ş, ü) doğru yazılmalı: arayüz metinleri, hata ve doğrulama mesajları, tarayıcı kontrolleri (dosya seçme, form uyarıları), Ayşe'nin yazılı ve sesli cevapları, hastaya giden e-posta ve WhatsApp metinleri, yazdırılan belgeler, PDF'ler. Türkçe kelime ASCII'ye çevrilmez (“Gokhan”, “Kayitlarda”, “kiz cocuk” gibi yazımlar yasak). Tüm Türkçe denetimi Job I olarak Job H'den sonra başlar. | STANDING; Job I DONE 2026-09-25 |
| 2026-09-25 | NOTYA-TURKCE-01 denetimi (Job I): 315 dosyada 201 ASCII-Türkçe + 100 İngilizce arayüz metni düzeltildi; ortak Dosya Seç düğmesi (DosyaSecDugmesi) ve Türkçe HTML5 doğrulama mesajları eklendi; lib/turkce/turkceDenetim.test.ts nöbetçi testi npm test'e girdi (gerekçeli IZINLI listesi: doz-oner JSON alanı aciklama, KVKK sayfasındaki Google Limited Use beyanı İngilizce kalır). Mali + Avukat mantığı kapsam dışı, yalnız metin. | DONE 2026-09-25 |
| 2026-09-26 | NOTYA-ASISTAN-REHBER-01 (Kaan, ekran görüntüsü): (1) Asistana sor sayfasında asistan balonu ham ** ve tek satırda • zinciri gösteriyordu — balon artık HafifMarkdown ile çiziliyor ve inlineMaddeAyir satır içi • zincirini gerçek madde satırlarına açıyor (YaziliSohbet dahil). (2) Sisteme BİÇİM kuralı: speech gerçek satır sonları, "- " maddeler, **kalın**; • zinciri yasak. (3) UYGULAMA REHBERİ (lib/asistan/uygulamaRehberi.ts) her uzmanın önbelleklenen sabit istemine girdi: sol menü haritası, muayene/onay akışı, yeni kullanıcıya adım adım anlatma kuralı; sesli istem yalnız tek satırlık işaret alır. (4) BRANŞA ÖZEL bloğu: her uzman kendi branşının Araçlar kataloğundan (doktorAraclariGruplu — el yazısı değil, yeni araçla kendiliğinden güncellenir) türetilen yetenek listesini bilir; branş sızması kuralı korunur, başka branş sorulursa şeritten uzman değişimi söylenir; sesli istem yalnız araç adları. Testler: uygulamaRehberi.test.ts (tüm personalar + sızma). | DONE 2026-09-26 — merged + deployed (Kaan'ın "deploy" işareti, 2026-09-26) |
| 2026-09-26 | NOTYA-NOT-SIL-01 (Kaan, ekran görüntüsü): (1) Not sayfası üst başlığı temizlendi — çift Yazdır/PDF bağlantısı ve ikinci Muayene Geçmişi oku kaldırıldı, durum artık rozet (Onay bekliyor kehribar / Onaylı yeşil). (2) Onay bekleyen (taslak) not silinebilir: başlıkta Sil düğmesi (onay penceresiyle), DELETE /api/notes/[id] — sahiplik + onaylıysa 409 (tıbbi kayıt silinmez), gevşek referanslar koparulur, düzenleme günlüğü silinir. Kaan'ın TEST H taslak notu canlı DB'den aynı sırayla silindi ve doğrulandı. | DONE 2026-09-26 |
| 2026-09-26 | NOTYA-AYSE-ACILIS-01 (Kaan, canlı ses seansı): (1) Ayşe açılıştaki “onay bekleyen not” için “hangi hasta?” sorusuna cevap veremeyip “uydurdum, bağlantım yok” diye YANLIŞ itirafta bulundu — gün özeti artık bekleyen notların hasta adlarını da taşır (en çok 3; selamda 2 ad) ve sesli isteme gerçeklik koruması girdi: açılış sayıları sistemden gelir, asla ‘uydurdum / erişimim yok’ deme; bilinmeyen ayrıntıda hasta_bul ya da ekrana yönlendir. (2) ‘Bakıyorum Hocam’ bekletme sözü tamamen kaldırıldı (dolguSec aramada boş döner; istem yasağı da eklendi) — süreye bağlı söyleme güvenilir ölçülemediği için Kaan tam kaldırmayı seçti; sesli onaydaki ‘Kaydediyorum Hocam’ teyidi kalır. | DONE 2026-09-26 |
| 2026-09-26 | NOTYA-ASISTAN-YUZEN-01 (Kaan): doktor asistanla (sesli seans ya da yazılı sohbet) çalışırken başka sayfaya geçince asistan artık kapanmıyor — doktor kendisi kapatana kadar oturum ve konuşma sürer. Oturum durumu (ElevenLabs konuşma nesnesi, mesajlar, persona, tek beyin oturumu + ses-ekran yoklaması, hasta_bul / ses-eylem araçları, süre sayaçları, yazılı sohbet) app/asistan/page.tsx ve YaziliSohbet.tsx içinden components/asistan/AsistanOturumContext.tsx provider'ına taşındı; provider app/layout.tsx'te bütün sayfaları sarar. /asistan artık yalnız görünüm (ikinci oturum kurmaz, ayrılınca oturumu kapatmaz). /asistan dışında components/asistan/AsistanYuzenPanel.tsx sağ altta: balon (persona baş harfi + mikrofon durumu) ↔ ~360px panel (son mesajlar, yazı girişi, mikrofon durumu, onay kartı), Asistana dön + Kapat; 390px telefonda taşmaz. DoktorChrome gezinmesi window.location yerine router.push; yazılı sohbet yönlendirmesi ve EylemKarti ilgili sekme bağlantısı next/link. Endpoint mantığı, onay kartları ve tek beyin ekran biçimi değişmedi. Saf karar lib/asistan/yuzenPanel.ts + yuzenPanel.test.ts (npm test'te). | DONE 2026-09-26 — branch feat/asistan-yuzen, commit (push/merge yok) |
| 2026-09-26 | NOTYA-HASTA-ODAK-01 + NOTYA-SES-SLUR-01 (Dr. Gökhan canlı vaka, Kaan): (1) Doktor Umutcan Türkoğlu'nun aşı karnesini sordu; "Biraz koy. Ayşe, benim spesifik arzum..." cümlesindeki hitap hasta adı sanıldı, çözümleyici Ayşe Yeşil'in (5 yaş) dosyasını açtı ve "Dosyada aşı: kayıtlı aşı yok" dedi — doğru hasta oturum belleğinde (currentPatientId) zaten Umutcan'dı; ardından model "tamamen uydurdum" diye sahte itirafta bulundu. Düzeltme: hitap temizliği 30 persona adının tümüyle ve cümlenin her yerinde (virgül/noktalama/Hocam-Hanım sonrası); tek başına persona adı kısmi eşleşmede hasta seçemez (yan etki: Ayşe adlı hasta yalnız tam adıyla bulunur); her kesin dosya cümlesi hastanın adıyla başlar ("Umutcan Türkoğlu — dosyada aşı: ..."); gerçeklik koruması genelleştirildi (persona kural 13 + ses istemi + dosya bloğu): asla "uydurdum / dayanağı yok / erişimim yok", çelişen kaynaklar adıyla söylenir (aşı tablosu vs vizit notu), aşı/ilaç/lab listesi yalnız aktif dosya bloğundan. (2) Konuşma bozulması (2-3 gündür, tek beyinle başladı): SesAkisi artık yalnız BİTMİŞ cümleyi sese verir (ilk virgülde parça atma kaldırıldı); 3+ maddelik liste okunmaz, "N madde, ekranınızda"; sözlü tur en çok 5 içerik cümlesi, sonra "Devamı ekranınızda". KARAR DEĞİŞİKLİĞİ: 25.09 kuralı "sesli cevap yazılı kadar ayrıntılı" Kaan tarafından 26.09 geri alındı (konusma.ts başlığında da yazılı). Sırada (Kaan'ın sırası): (3) TEK BEYİN ajan kopyalarında TTS ayarı (Flash kontrol / eleven_v3_conversational, latency 0-1, speed 0.92-0.98, stability 0.55-0.65, style 0), (5) telaffuz sözlüğü (mg, ml, mcg, IU, x2, q8h, Nelson, Harriet Lane, ICD), (6) A/B 10 tur aynı senaryo: eski ajan / kopya+cümle tamponu+Flash / kopya+tamponu+v3 — 1-2 ile düzelirse model değişmez. NOTYA-SES-TUR-01 (STT parçası başına çift tur — 12:24:45 / :50 / :50 üç satır) henüz yapılmadı. | DONE 2026-09-26 (kod); (3)(5)(6) OPEN — waits on Claude |
| 2026-09-26 | NOTYA-AYSE-STANDART-01 (Dr. Gökhan standardı, Kaan mimarisi): Ayşe 10 klinik dosya sorusunu olay dizini + durum (planlandı ≠ uygulandı) + açık işler + eşanlam + kanıt bloğu ve cevap şablonuyla cevaplıyor; HIZLI KART yalnız tek bilgilik sorularda. Deterministik denetim `npm test` içinde (4 sentetik dosya). Ertelenenler § NOTYA-AYSE-STANDART (02–06). | DONE 2026-09-26 (kod); 02–06 OPEN |
| 2026-09-26 | NOTYA-AYSE-STANDART-01 live audit on QA synthetic patient (docs/denetim/2026-09-26-qa-sentetik-bebek.md): 59/61 rule checks; misses: Soru 9 no literal "Dikkat / Eksik kayıt / Takip" header, Soru 10 used "ya hiç yapılmadı ya da başka merkezde…" — rule block tightened (fix/standart-kural). Clinical correctness of the answers is Dr. Gökhan's read. | DONE 2026-09-26 |
| 2026-09-26 | NOTYA-SES-ERKEN-01 (Dr. Gökhan canlı vaka, 18:05/18:08 UTC): "Umutcan Türkoğlu'nun dosyasını kısaca özetler misin" — Ayşe konuşmaya başladı, sonra "Bağlantı kurulamadı". ElevenLabs tarafında iki kez `custom_llm_error: LLM Cascade Error: TimeoutError`: Custom LLM akışı ~30 sn'yi geçince görüşmeyi düşürüyor; yeni standart cevapları (özet 33 sn) bundan uzun. Düzeltme: ses turu sözlü sınırda ("Devamı ekranınızda") ya da 22 sn bekçide kapanır ([DONE]); ekran cevabı @vercel/functions waitUntil ile arka planda tamamlanır ve ses-ekran yoklamasıyla gelir. Test: tekBeyin.test.ts ERKEN-01. Açık: ilk cümleye kadar geçen 10-15 sn (özetin kanıt bloğu büyük) — ölçülüp kısaltılacak (waits on Claude). | DONE 2026-09-26 |
| 2026-09-26 | NOTYA-SES-OKU-01 (Dr. Gökhan, 18:08 UTC: "Devamını ekranda görüyorum ama sen bana anlat"): "bana anlat / devamını oku / sesli anlat / oku" isteği ekrandaki son cevabı 5 cümle sınırı OLMADAN okur; model çağrılmaz (metin hazır, zaman aşımı riski yok); 3+ maddelik listeler yine "N madde" olarak geçer. Ekrana "Ekrandaki cevabı sesli okudum Hocam." yazılır. Test: tekBeyin.test.ts OKU-01. | DONE 2026-09-26 |
| 2026-09-26 | NOTYA-SES-DEVAM-01 (Dr. Gökhan: sözlü özet yarıda kesilmesin — tek beyinden önce 30 sn'yi geçip bitiyordu; Kaan onaylı tasarım): otomatik devam. Ses turu yine sınırda kapanır (5 cümle ya da 22 sn bekçi — görüşme düşmez), ama sınır artık SESSİZDİR. `sesLlm.ts` ElevenLabs'e gerçekten giden metni tutar ve `ayseCevapla`'ya `sesDurumu` verir; kesilen turda ekran cevabı bitince söylenmeyen kalan (`konusmaYap(ekran, temizle, {sinirsiz})` cümleleri eksi söylenen cümle öneki; eşleşmezse tamamı; kart okuması dahil) oturum JSON'una yazılır: `active_context.sesDevam = { anahtar (turun zamanı), kalan, olusturma }` — migration yok. `/api/asistan/ses-ekran` `devam` / `devamAnahtar` döner; `/asistan` sayfası (AsistanOturumContext) Ayşe susunca gizli `[devam]` turunu tur başına bir kez yollar (baloncukta görünmez; Ayşe sustuktan sonra doktor konuştuysa yollanmaz). `[devam]` (ya da doktorun "devam / devam et" sözü) kalanı sınırsız, cümle cümle, MODELSİZ okur, sesDevam'ı siler, yeni ekran baloncuğu yazmaz; kalan yoksa gizli işaret sessiz geçer, "devam et" sıradan turdur. Her yeni gerçek doktor turu (ve sesli Evet/Hayır) eski kalanı düşürür. ERKEN-01'in "Devamı ekranınızda Hocam" cümlesi artık yalnız devamın olamadığı yollarda (akışsız `konusmaYap` — arama / kesin dosya cevabı) kalır; OKU-01 ("bana anlat") olduğu gibi çalışır. Test: tekBeyin.test.ts ERKEN-01 + DEVAM-01. Açık: canlı ElevenLabs'te `sendUserMessage('[devam]')` ile gerçek görüşme denemesi (waits on Dr. Gökhan / Kaan). | DONE (kod) 2026-09-26 |
| 2026-09-26 | NOTYA-SES-SESSIZ-01 (Dr. Gökhan, canlı test): mikrofon açıkken alt hanedeki yazılı sohbete yazdı; yazılı cevap geldi ama sesli asistan konuşmaya devam etti ("Buradayım, devam edebiliriz", sessizlik uyarıları — bunlar ElevenLabs ajanının kendi davranışı, kodumuzda değil). Kural (Kaan, yalnız bu bölüm için): yazılı sohbet angaje edildiği andan itibaren (ilk tuş ya da Sor) sesli asistan susar — ElevenLabs oturumu kapanır, sunucu oturumu korunur, durum "Sessizde — yazılı sohbet açık. Devam için mikrofona dokunun"; mikrofona basınca aynı oturuma bağlanır. Sayfalar arası takip (YUZEN-01) değişmedi. | DONE 2026-09-26 |
| 2026-09-26 | NOTYA-SAYFA-HASTA-01 (Dr. Gökhan canlı vaka, Kaan kuralı): oturum Ayşe Yeşil odağında başladı; doktor Umutcan Türkoğlu'nun Büyüme sayfasına geçti, yüzen panel hâlâ "aktif hasta: Ayşe Yeşil" dedi ve Ayşe büyüme sorularını yanlış çocuğun dosyasından cevapladı ("karşılaştıracak ikinci bir nokta yok"). Kural: asistan doktoru takip eder — bir hastanın sayfasını açmak (`/dashboard/doktor/hastalar/<id>[/…]`) hastayı adıyla söylemekle eş değer açık odak sinyalidir; EN SON açık sinyal kazanır: doktor X'i adıyla söyler → X (mevcut); Y'nin sayfasını açar → Y, o gezinmede BİR KEZ (aynı hastanın alt sayfaları arasında tekrar gönderilmez; hasta sayfalarından çıkıp geri girmek yeni gezinmedir); adsız takip mevcut odakta kalır (NOTYA-HASTA-ODAK-01 — "her mesajda sayfa hastası" geri GELMEDİ). Kod: `POST /api/asistan/oturum-hasta` (oturum id + doctor_id → yoksa 404; `hastaSahibiMi` → başka doktorun hastası 403, hiçbir şey yazılmaz) `active_context.currentPatientId / patientName / odakKaynak='sayfa' / odakZaman` + `patient_id` yazar, eski hastanın `sesDevam` kalanını düşürür; `AsistanOturumContext` `usePathname` → `sayfaHastaId` (`lib/asistan/yuzenPanel.ts`) değişince ortak oturum varsa çağırır, panel etiketini dönen adla günceller (sesli duyuru yok); oturum yoksa sayfa hastası ilk `yaziliGonder` / signed-url çağrısına `patientId` olarak gider. `ayseCevapla` tur sürerken gelen sayfa geçişini ezmez (tur başlangıcından yeni `odakKaynak='sayfa'` varsa o korunur); adla söylenen odak `odakKaynak='soz'`. Migration yok. Q3 büyüme doğrulandı: olay dizini onaylı not vitalleri + onaylı cihaz ölçümlerinden kilo/boy/baş çevresi serisini kurar, `specialties/pediatri/sorgu.ts` Neyzi p/Z ile değerlendirir (Büyüme sayfası aynı onaylı not vitallerini çizer). Test: `tekBeyin.test.ts` SAYFA-HASTA-01 (rota 200/403/404/401/400, sayfa geçişi sonrası adsız "kaç kilo" yazı + ses yeni hastadan, adla söylenen kazanır, tur-içi yarış, Q3 büyüme kanıtı), `yuzenPanel.test.ts` `sayfaHastaId`, `hasta-izolasyon.test.ts` iki vaka (yabancı hasta 403, yabancı oturum 404). Not: HASTA-IZOLASYON skill'i yabancı hasta için 404 der; bu rota brief gereği 403 döner (oturum doktorun kendisinin olduğu doğrulandıktan sonra). Açık: canlı Dr. Gökhan denemesi (waits on Dr. Gökhan / Kaan). | DONE (kod) 2026-09-26 |
