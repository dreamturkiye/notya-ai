...
| 2026-09-16 | **DAH-SCORE2-OP** — waits on Claude: SCORE2-OP (≥70) as a separate engine with its own ONAYLI gate + ≥3 published worked examples. Source now in hand (2026-09-17): `ehab312_supplementary_data.zip` → "Supplementary material_20210604_v2.docx" from PMC8248997 (same browser download path as DAH-SCORE2-VERIFY). Not built in the SCORE2-VERIFY PR, on purpose. | OPEN (Claude) |
| 2026-09-16 | **DAH-SCORE2-DIABETES** — waits on Claude: SCORE2-Diabetes (ESC 2023, ehad260) for DM 40–69 as a separate engine with the same ONAYLI gate + published worked examples. Until then DM stays on the TEMD rule (DM = at least high risk). | OPEN (Claude) |
