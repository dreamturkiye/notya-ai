...
| Date | Item | Durum |
|---|---|---|
| 2026-09-24 | notya.io domain owned; migrate once the new website ships (DNS, sender domain for email/SMTP, any hardcoded notya.ai references). | IN PROGRESS 2026-09-25 (Kaan: move off the vercel.app link now). DONE: notya.io + www.notya.io added to Vercel project notya-ai (team getvelacom); Namecheap A @ and A www -> 76.76.21.21, www parking CNAME deleted. BLOCKED: Namecheap parking URL-redirect record on @ still overrides the apex; the connector cannot delete URL records - waits on Kaan (delete it in Namecheap Advanced DNS, or log in so Claude can). THEN (Claude, only after notya.io serves with a valid certificate): set NEXT_PUBLIC_APP_URL=https://notya.io in Vercel Production and redeploy; add notya.io to Supabase Auth Site URL + redirect URLs; replace the 8 hardcoded notya-ai.vercel.app fallbacks and the notya.ai references; decide www -> apex redirect; email sender domain later. |
