...
| ID | Status | What |
|---|---|---|
| NOTYA-FISILTI-GIZLE-01 | **DONE 2026-09-24** (PR #385 logic, PR #PR2 Gizle) | **Logic (#385):** pediatri kohort flag `asi_kayit_tutarsiz`: a recorded dose before birth, on the same day as the previous dose, or before the GBP minimum age/interval already in `asiPlan` → "Aşı kaydı tutarsız — <aşı> <doz> tarihi / doğum tarihini kontrol edin" (aşı tab), and that series is no longer called "gecikti". The parent 1-tap reminder skips children whose only flag is this one. Golden: Umutcan (DOB 15.06.2024, Hep B 1+2 on 15.06.2024) → tutarsız; corrected DOB 15.05.2024 + 0-1-6 → no Hep B item. **Gizle (#PR2):** "Gizle" on the Fısıltı card → "Bu uyarıyı gizle" (until the facts change) or "7 gün sonra hatırlat"; "Gizlenenler (n)" → "Geri getir". Table `fisilti_gizlenen` (migration 092, applied 2026-09-24; RLS doctor-only) stores only the item key + SHA-256 of the item's facts (no name / text), `until`, soft `kaldirildi_at`. A hide only matches while the hash matches, so a new late dose or a corrected date brings the item back. POST looks the item up in the doctor's own freshly-built list (patient id never from the body); hides / restores go to `audit_logs`. Candidate collection moved to `lib/doktor/fisiltiTopla.ts` (shared by `/api/doktor/fisilti` and `/fisilti/gizle`); client fetches are `no-store`, so a dose written by an approved note (NOTYA-ASI-NOT) shows on the next load. No delete option (clinical safety). |
