...
| 2026-09-25 | NOTYA-RECETE-05: close the added-drug gap in the one-tap Ilaclar check. Before: the check ran only when a LISTED drug was missing from the Plan, so a drug ADDED to the Plan (existing ones kept) was never caught. Now it also runs whenever the Plan was edited since the note opened, and Ayse pre-reads the edited Plan in the background (cached per Plan text) so Onayla is usually instant. If Ayse cannot read the Plan and there is no concrete mismatch, approval continues without a card. Branch fix/plan-ilac-eklenen. | OPEN - waits on Claude (live check, then merge) |
