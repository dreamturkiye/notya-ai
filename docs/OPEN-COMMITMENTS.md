...
| 2026-09-16 | **NOTYA-LAB-02 — "Lab değerlendirme" muayene auto-create when the patient has no note today** (spec item) — waits on Claude | Today the approve endpoint refuses with a clear message; needs a session+note creation path that matches the SOAP pipeline's shape. |
| 2026-09-16 | **NOTYA-LAB-03 — vault Belgeler list: "Lab" filter + card "12 parametre · 3 yüksek · 1 düşük" + ALT ↑ chip** (spec UI) — waits on Claude | Page-level summary exists; the list card needs a small join to lab_paneller. |
| 2026-09-16 | **NOTYA-LAB-04 — Kre slope (≥2 priors) for nefroloji, Δ HbA1c line for endokrin, serial troponin for kardiyoloji as explicit computed fields** — waits on Claude | Router emphasis exists; the per-specialty computed lines are the next dependability step. |
