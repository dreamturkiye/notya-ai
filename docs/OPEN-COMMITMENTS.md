...
| 2026-09-18 | **DERM-EXCEPTIONAL-01 A — 054 migration Supabase'e uygulanmadı** | `lib/db/migrations/054_derm_exceptional.sql` yalnız-ekleme (skor `ek` jsonb + SCORAD/IGA/BSA, fototerapi v2 kolonları, `derm_med_kayitlari`, yama ICDRG dereceleri, kozmetik lot/komplikasyon, `hasta_derm` acil/basamak/izlem jsonb, `derm_biyolojik_raporlar`, `derm_dermoskopi_skorlari`; RLS doktor-kendi + 052 restrictive hasta sahipliği). Migration uygulanmadan yeni kartlar boş çalışır (GET yeni tabloları boş döner, POST hata verir). **Öneri:** önizleme/prod aynı Supabase — migration merge ile aynı gün uygulanmalı. | OPEN (Kaan) |
