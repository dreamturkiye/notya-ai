...
| 2026-09-15 | **NOTYA-BELGE-02 — Tier B engine modules** (browser ONNX): txrv-densenet121 first (CXR, Apache-2.0), then hear-icbhi/hear-circor probes (sound), ptbxl-inception1d (ECG), grazpedwri-yolo + rsna-boneage (pediatric bone), fracatlas-yolo — waits on Claude, each with ONNX export + golden set + README_BELGELER.md row before aktif=true | Contract + runner (core/belgeler/tarayiciMotor.ts) and registry rows already exist; onnxruntime-web is added with the first module; models hosted on Supabase Storage bucket 'motorlar/' (sha256-pinned, cached by sw.js). |
...
| 2026-09-15 | **NOTYA-BELGE-04 — DICOM single-slice import in the browser (dicom-parser, tags dropped client-side)** — waits on Claude | Hospital exports arrive as DICOM; V1 accepts JPEG/PNG/PDF/audio. |
| 2026-09-15 | **NOTYA-BELGE-05 — Ayşe/SOAP awareness of approved belge reports + beta checklist item "Asistana raporla"** — waits on Claude after Dr. Gökhan's first real report | The approved block already lands in Objektif; the SOAP prompt should cite it as "belge değerlendirmesi mevcut". |
