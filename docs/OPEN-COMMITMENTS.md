...
| 2026-09-16 | **DAH-WOW-SMOKE** — rubric criterion 5 for all domains: capture one smoke-path screenshot per domain on synthetic non-PHI data (steps in the post-sprint audit). Blocker: qa.test@notya.ai is `pediatri` (Dahiliye tab hidden) and no approved lab rows exist. Needs Kaan: create a dahiliye QA account (or approve switching qa.test), then capture and flip 19 domains to Strong. | OPEN (Kaan → Claude) |
