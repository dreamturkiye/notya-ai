...
| — | note | `app/api/doktor/araclar/sgk-rapor` reads `notes.patient_id`, a column that does not exist → the query has always errored and the SGK draft never used notes (falls back to hekimNotu). Archive rule is applied, the filter is left as is: fixing it would start sending note text to the Groq/xAI model there — Kaan's call (ai-model-politikasi). |
