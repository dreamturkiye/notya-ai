...
export default function RrsBekleyenler(props: { ilacAdlari: string[] }) {
  ...
  return (
    <div style={{ marginBottom: 20, padding: 16, borderRadius: 12, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)' }}>
      <div style={{ fontSize: 12, fontWeight: 600, letterSpacing: 1.5, color: '#F59E0B', marginBottom: 8 }}>RENKLİ REÇETE (RRS)</div>
      {kontrollu.length > 0 && (
        <div style={{ fontSize: 14, marginBottom: 10 }}>
          Kontrole tabi ilaç: {kontrollu.map((x) => `${x.ad} (${RRS_ETIKET[x.renk as RrsRenk]})`).join(', ')} — normal e-reçeteye yazılamaz; <a href={RRS_URL} target="_blank" rel="noopener noreferrer" style={{ color: '#F59E0B' }}>RRS&apos;de</a> düzenlenir.
        </div>
      )}
      {kayitlar.length > 0 && (
        <div style={{ fontSize: 13 }}>
          <div style={{ color: 'rgba(255,255,255,0.6)', marginBottom: 6 }}>RRS numarası bekleyen reçeteler</div>
          {kayitlar.map((k) => (
