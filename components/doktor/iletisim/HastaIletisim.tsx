 ...
 * NOTYA-GELEN-BELGELER: third line — what the patient sends (documents, photos, voice messages) may be filed
 * (kanal gelen_belge; shown once migration 099 is in, until then the line is hidden).
 */
...
import { GELEN_BELGE_IZIN_PROFIL } from '@/lib/iletisim/izinMetni'

type Izin = { whatsapp: boolean | null; eposta: boolean | null; guncelleme: string | null; kaydedilebilir: boolean; gelenBelge?: boolean | null; gelenBelgeKaydedilebilir?: boolean }
type IzinKanali = IletisimKanali | 'gelen_belge'
const IZIN_ANAHTARI = { whatsapp: 'whatsapp', eposta: 'eposta', gelen_belge: 'gelenBelge' } as const
...
export default function HastaIletisim({ patientId }: { patientId: string }) {
  ...
  const [kaydediliyor, setKaydediliyor] = useState<IzinKanali | null>(null)
  ...
  const degistir = async (kanal: IzinKanali, deger: boolean) => {
    ...
    try {
      ...
      setIzin((o) => (o ? { ...o, [IZIN_ANAHTARI[kanal]]: deger, guncelleme: j.guncelleme } : o))
    } catch (e) {
  ...
  const satir = (kanal: IzinKanali, etiket: string, kaydedilebilir = izin.kaydedilebilir) => {
    const deger = izin[IZIN_ANAHTARI[kanal]] ?? null
    return (
      <label style={{ display: 'flex', alignItems: 'center', gap: 12, minHeight: 44, cursor: kaydedilebilir ? 'pointer' : 'default' }}>
        <input
          type="checkbox"
          checked={deger === true}
          disabled={!kaydedilebilir || kaydediliyor === kanal}
  ...
  return (
    <div style={{ background: R.paper, border: `1px solid ${R.border}`, borderRadius: 16, padding: '14px 16px', fontFamily: CHROME_FONT.sans }}>
      ...
      {izin.gelenBelgeKaydedilebilir && satir('gelen_belge', GELEN_BELGE_IZIN_PROFIL, true)}
