    ...
    <div ref={kutu} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 14 }}>
      <div style={{ position: 'relative' }}>
        ...
        {acik && (sonuclar.length > 0 || araniyor) && (
          <div style={dd}>
            {araniyor && sonuclar.length === 0 && <div style={{ ...satir, color: CHROME_RENK.muted, cursor: 'default', borderTop: 'none' }}>Aranıyor…</div>}
            {sonuclar.map((g, i) => (
              <div key={g.marka} onMouseDown={() => markaSec(g)} style={{ ...satir, borderTop: i === 0 ? 'none' : '1px solid #F1F5F9' }}>
                ...
                {g.etkenMadde && <div style={{ fontSize: 12, color: CHROME_RENK.muted }}>{g.etkenMadde}</div>}
              </div>
            ))}
          </div>
        )}
