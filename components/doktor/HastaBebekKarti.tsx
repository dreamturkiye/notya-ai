...
export default function HastaBebekKarti({ patientId }: { patientId: string }) {
  ...
  if (yukleniyor) return <div style={{ padding: 16, color: '#8FA0B5', fontSize: 13 }}>Bebek kartı yükleniyor…</div>
  if (!veri?.bebek) {
    return (
  ...
  return (
    <div style={{ background: '#0D1C33', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: 20 }}>
      ...
      {veri.dogum && (
        <div style={{ fontSize: 13, color: '#C9D4E3', marginBottom: 12 }}>
      ...
      <div style={kutu}>
        <div style={{ fontWeight: 700, color: '#EDF1F7', marginBottom: 8 }}>Zaman çizelgesi</div>
        {[
          ...
          <div key={String(etiket)} style={{ fontSize: 13.5, color: '#EDF1F7', marginBottom: 4 }}>
            <span style={{ color: ok ? '#86EFAC' : '#8FA0B5' }}>{ok ? '[x]' : '[ ]'}</span> {etiket}
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 8 }}>
          {(veri.ntp || []).flatMap((p) => p.satirlar).filter((s) => s.flag && s.flag !== 'unknown' && s.flag !== 'normal').slice(0, 8).map((s, i) => (
            <span key={i} style={{ fontSize: 11, fontWeight: 700, color: FLAG[s.flag] || '#C9D4E3', border: '1px solid currentColor', borderRadius: 999, padding: '2px 8px' }}>
              {(s.canonical_key || s.raw_name).replace('ntp_', '')} {s.flag}
            </span>
          ))}
        </div>
        <div style={{ fontSize: 13, color: '#C9D4E3', marginTop: 10 }}>
      ...
      <div style={kutu}>
        <div style={{ fontWeight: 700, color: '#EDF1F7', marginBottom: 8 }}>NTP sonuç belgeleri (bebek)</div>
        {(veri.ntp || []).length === 0 && <div style={{ fontSize: 13, color: '#8FA0B5' }}>Henüz NTP belgesi yok. Sonuç PDF’ini bebek Belgeler’ine yükleyip Asistana raporla.</div>}
        {(veri.ntp || []).map((p) => (
          <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', gap: 8, fontSize: 13, color: '#C9D4E3', padding: '6px 0', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <div style={kutu}>
        <div style={{ fontWeight: 700, color: '#EDF1F7', marginBottom: 8 }}>Görevler</div>
        {(veri.gorevler || []).filter((g) => g.kind !== 'yeni_bebek').map((g) => (
          <div key={g.id} style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center', fontSize: 13, padding: '6px 0', borderTop: '1px solid rgba(255,255,255,0.06)', borderLeft: `3px solid ${g.status === 'red' ? '#F87171' : URG[g.urgency] || '#475569'}`, paddingLeft: 8 }}>
            <div>
              <div style={{ color: '#EDF1F7' }}>{g.title}</div>
              <div style={{ color: '#8FA0B5', fontSize: 11.5 }}>{new Date(g.due_at).toLocaleDateString('tr-TR')} · {g.status}{g.status === 'red' ? ' (takvim duruyor)' : ''}</div>
            </div>
            {g.status === 'bekliyor' && (
              <span style={{ display: 'flex', gap: 6 }}>
                ...
                }} style={{ background: 'rgba(255,255,255,0.08)', color: '#EDF1F7', border: 'none', borderRadius: 8, padding: '6px 10px', fontSize: 12, cursor: 'pointer' }}>Red</button>
              </span>
            )}
          </div>
        ))}
      ...
      {veri.smsNtp2 && (
        <button type="button" onClick={() => navigator.clipboard.writeText(veri.smsNtp2 || '')} style={{ background: 'rgba(255,255,255,0.08)', color: '#EDF1F7', border: 'none', borderRadius: 8, padding: '9px 14px', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
