...
export default function NorolojiHome({ patientId }: { patientId: string }) {
  ...
  if (!v) return <div style={{ ...toolsCard, color: CHROME_RENK.muted, fontSize: 12 }}>Nöroloji yükleniyor…</div>
  ...
  return (
    <div style={toolsCard} data-chapter="noroloji">
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
        {SEKMELER.map((x) => (
          <button key={x} type="button" onClick={() => setSekme(x)} style={{ ...ghost, borderRadius: 999, background: sekme === x ? 'rgba(124,58,237,0.22)' : 'transparent', color: sekme === x ? '#DDD6FE' : CHROME_RENK.muted }}>{x}</button>
        ))}
      </div>
