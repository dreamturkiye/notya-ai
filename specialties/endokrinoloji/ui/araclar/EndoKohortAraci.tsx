...
import { endoStil, Istatistik, TaslakNotu, Rozet } from './EndoAracKabugu'
import { ENDO_BAYRAK_AD, endoKohortFiltre, type EndoKohortBayrak, type EndoKohortSatir } from '../../engines/kohort'

const S = endoStil
const BAYRAKLAR = Object.keys(ENDO_BAYRAK_AD) as EndoKohortBayrak[]

export default function EndoKohortAraci() {
  const [satirlar, setSatirlar] = useState<EndoKohortSatir[] | null>(null)
  const [toplam, setToplam] = useState(0)
  const [suzgec, setSuzgec] = useState<EndoKohortBayrak[]>([])
  ...
  const yukle = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch('/api/doktor/endokrinoloji/kohort', { headers: { Authorization: `Bearer ${t}` }, cache: 'no-store' })
  ...
  const gorunen = endoKohortFiltre(satirlar || [], suzgec)

  const gonder = async () => {
    setDurum(''); setHata(''); setGonderiliyor(true)
    try {
      const t = await getAccessTokenAsync()
      const r = await fetch('/api/doktor/endokrinoloji/kohort', {
        method: 'POST',
  ...
  return (
    <>
      <div style={S.kutu}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 10 }}>
          {BAYRAKLAR.map((b) => (
            <button key={b} type="button" onClick={() => setSuzgec((p) => (p.includes(b) ? p.filter((x) => x !== b) : [...p, b]))}
              style={{ ...S.ghost, background: suzgec.includes(b) ? 'rgba(168,85,247,0.25)' : 'transparent' }}>
              {ENDO_BAYRAK_AD[b]}
            </button>
          ))}
        </div>
        ...
        {gorunen.map((s) => (
          <label key={s.patientId} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div>
              ...
              <div style={S.kucuk}>
                {s.bayraklar.map((b) => <Rozet key={b} ton={b === 'risk_acik' ? 'kirmizi' : 'uyari'}>{ENDO_BAYRAK_AD[b]}</Rozet>)}
        ...
        <TaslakNotu>TASLAK — Hatırlatma tanı, HbA1c sayısı, ilaç adı ve doz yazılmaz. Açık acil bayrağında “sizi arayacağız” dili kullanılır.</TaslakNotu>
      </div>
    </>
  )
}
