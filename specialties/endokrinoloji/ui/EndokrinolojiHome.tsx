...
export default function EndokrinolojiHome({ patientId }: { patientId: string }) {
  ...
  if (!v) return <div style={{ ...toolsCard, color: CHROME_RENK.muted, fontSize: 12 }}>Endokrinoloji yükleniyor…</div>
  ...
  return (
    <div style={toolsCard} data-chapter="endokrinoloji">
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
        {SEKMELER.map((x) => (
          <button key={x} type="button" onClick={() => setSekme(x)} style={{ ...ghost, borderRadius: 999, background: sekme === x ? 'rgba(168,85,247,0.22)' : 'transparent', color: sekme === x ? '#F3E8FF' : CHROME_RENK.muted }}>{x}</button>
        ))}
      </div>
