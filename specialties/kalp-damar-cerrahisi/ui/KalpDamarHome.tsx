...
export default function KalpDamarHome({ patientId }: { patientId: string }) {
  ...
  if (!v) return <div style={{ ...toolsCard, color: '#8FA0B5', fontSize: 12 }}>Kalp Damar Cerrahisi yükleniyor…</div>
  ...
  return (
    <div style={toolsCard} data-chapter="kalp-damar-cerrahisi">
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
        {SEKMELER.map((x) => (
          <button key={x} type="button" onClick={() => setSekme(x)} style={{ ...ghost, borderRadius: 999, background: sekme === x ? 'rgba(159,18,57,0.22)' : 'transparent', color: sekme === x ? '#FFE4E6' : '#8FA0B5' }}>{x}</button>
        ))}
      </div>
      ...
      {sekme === 'Özet' && (
        <div>
          ...
          <div style={metin}>Pre-op, greft/yara ve antikoagülan vade karar desteğidir. Tanı / doz / OR planı hekimdedir. SCORE2/Kalbim bu bölümde yoktur.</div>
      ...
      {sekme === 'Pre-op' && (
        <div>
          <div style={etiket}>Pre-op risk checklist</div>
          {PREOP_MADDELER.map((m) => (
            <label key={m.kod} style={{ display: 'flex', gap: 8, marginTop: 4, fontSize: 12, color: '#EDF1F7' }}>
      ...
      {sekme === 'Acil' && (
        <div>
          ...
          {v.kutuphane.acilKodlari.map((k) => (
            <label key={k.kod} style={{ display: 'flex', gap: 8, marginTop: 4, fontSize: 12, color: '#EDF1F7' }}>
          ...
          <label style={{ display: 'flex', gap: 8, marginTop: 8, fontSize: 12, color: '#EDF1F7' }}>
