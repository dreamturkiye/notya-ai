'use client'
/**
 * KALP-DAMAR-CERRAHISI-EXCEPTIONAL-01 — Araçlar › Kalp Damar Cerrahisi stüdyoları ortak kabuğu.
 * Yalnız kalp-damar-cerrahisi hekimi açar; kardiyoloji /doktor-tools'a döner.
 */
...
export const KDC_VURGU: AracVurgu = { ana: '#9F1239', anaMetin: '#FFF1F2', yumusak: '#FDA4AF', baslik: '#FB7185' }
export const kdcStil = aracStil(KDC_VURGU)
...
export function KdcHastaSecici({ secili, sec }: { secili: string; sec: (id: string, ad: string) => void }) {
  return <HastaSecici secili={secili} sec={sec} />
}

export default function KdcAracKabugu({ route, baslik, aciklama, children }: { route: string; baslik: string; aciklama: string; children: React.ReactNode }) {
  ...
  return (
    <AracVurguSaglayici vurgu={KDC_VURGU}>
      <div style={{ ...toolsShell, overflowX: 'hidden' }}>
        <DoktorNav />
        <div style={{ maxWidth: 1000, margin: '0 auto', padding: '24px 16px 56px', boxSizing: 'border-box' }}>
          {!izin ? (
            <div style={{ color: '#9BB0C7', fontSize: 15, padding: '12px 0' }}>{izin === null ? 'Yükleniyor…' : 'Bu araç yalnızca kalp damar cerrahisi için.'}</div>
          ) : (
            <>
              <div style={{ marginBottom: 18 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: KDC_VURGU.baslik, letterSpacing: '1.4px', textTransform: 'uppercase', marginBottom: 8 }}>Araçlar · Kalp Damar Cerrahisi</div>
