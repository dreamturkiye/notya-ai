...
export function preopSkorla(secilenHam: unknown, not?: string | null): PreopSonuc {
  const dipnot: Dipnot = { ref: 'TKDCD', not: 'Pre-op kontrol listesi karar desteğidir; tanı ve cerrahi karar hekimin' }
  ...
  return {
    tamamMi: true,
    secilen,
    ozet: `Pre-op risk kontrol listesi (${secilen.length}/${PREOP_MADDELER.length}): ${adlar.join('; ')}. Karar desteğidir; tanı/cerrahi karar hekimin. SCORE2/doz yok.`,
    gorevOnerileri,
    dipnot,
  }
}
