...
import { sevkSkorla, SEVK_HEDEFLER, SEVK_MADDELER, type SevkHedef, type SevkMadde } from '../../engines/sevk'

export default function AtSevkAraci() {
  const [hedef, setHedef] = useState<SevkHedef | null>(null)
  const [maddeler, setMaddeler] = useState<SevkMadde[]>([])
  const [hastaId, setHastaId] = useState('')
  const [durum, setDurum] = useState('')
  const [hata, setHata] = useState('')
  useUrlHasta(setHastaId)

  const sonuc = useMemo(() => sevkSkorla({ hedef, maddeler }), [hedef, maddeler])
  ...
  const kaydet = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/doktor/acil-tip', {
        ...
        body: JSON.stringify({ patientId: hastaId, adim: 'sevk', hedef, maddeler, hekimKilit: true }),
      })
      const j = await r.json().catch(() => ({}))
      if (!r.ok) { setHata(j.error || 'Kaydedilemedi'); return }
      setDurum('Sevk/yatış paketi hasta dosyasına kaydedildi.')
  ...
  return (
    <>
      <div style={atStil.kutu}>
        ...
        <div style={{ ...atStil.etiket, marginTop: 12 }}>Hedef</div>
        <div style={atStil.satir}>
          {SEVK_HEDEFLER.map((h) => (
            <button key={h.kod} type="button" style={{ ...atStil.ghost, background: hedef === h.kod ? 'rgba(249,115,22,0.25)' : 'transparent' }} onClick={() => setHedef(h.kod)}>{h.ad}</button>
          ))}
        </div>
        <div style={{ ...atStil.etiket, marginTop: 12 }}>Paket maddeleri</div>
        {SEVK_MADDELER.map((m) => (
      ...
      <div style={atStil.kutu}>
        ...
        <Istatistik deger={maddeler.length} etiket="Madde" ton="notr" />
