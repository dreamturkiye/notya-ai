...
export default function AcilTipHome({ patientId }: { patientId: string }) {
  ...
  if (!v) return <div style={{ ...toolsCard, color: CHROME_RENK.muted, fontSize: 12 }}>Acil Tıp yükleniyor…</div>
  ...
  return (
    <div style={toolsCard} data-chapter="acil-tip">
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
        {SEKMELER.map((x) => (
          <button key={x} type="button" onClick={() => setSekme(x)} style={{ ...ghost, borderRadius: 999, background: sekme === x ? 'rgba(249,115,22,0.22)' : 'transparent', color: sekme === x ? '#FFEDD5' : CHROME_RENK.muted }}>{x}</button>
        ))}
      </div>
        ...
        <div>
          ...
          <div style={satir}>
            {([1, 2, 3, 4, 5] as EsiSeviye[]).map((s) => (
              <button key={s} type="button" onClick={() => setEsiSeviye(s)} style={{ ...ghost, background: esiSeviye === s ? 'rgba(249,115,22,0.25)' : 'transparent', color: esiSeviye === s ? '#FFEDD5' : CHROME_RENK.muted }}>ESI {s}</button>
            ))}
          </div>
