...
describe('029 maddeler jsonb mapping', () => {
  it('reads taburcu checks from maddeler timestamps/booleans and yenidogan_tarama', async () => {
    const { checksFromMaddeler, mergeTaburcuMaddeler, istisnaFromRow } = await import('../../../lib/doktor/yenidoganKayit')
    assert.deepEqual(checksFromMaddeler({}), { ntp1: false, hepb1: false, vitk: false, isitme: false })
    assert.equal(checksFromMaddeler({ ntp1: true, hepb1_at: '2026-09-10T12:00:00Z', vitk: true, isitme_at: '2026-09-10T12:00:00Z' }).ntp1, true)
    assert.equal(checksFromMaddeler({ ntp1: false }, { ntp1: true }).ntp1, true)
    assert.equal(checksFromMaddeler({ isitme: true, isitme_sonuc: 'yapilmadi' }).isitme, false)
    const merged = mergeTaburcuMaddeler({}, { ntp1AlindiAt: '2026-09-10T12:00', hepb1At: '2026-09-10T12:00', gkdRisk: true, dvitBaslandi: true })
    assert.equal(merged.ntp1, true)
    assert.equal(merged.hepb1, true)
    assert.equal(merged.gkd, true)
    assert.equal(merged.dvit, true)
    const ist = istisnaFromRow({ tur: 'red', aciklama: 'aile reddetti', kaydeden: 'dr-1', at: '2026-09-11T10:00:00Z' })
    assert.equal(ist?.neden, 'redd')
  })
})
