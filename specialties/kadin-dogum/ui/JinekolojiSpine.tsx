...
const dueRenk: Record<string, string> = { gecikti: '#F87171', yaklasiyor: '#FBBF24', planli: '#2DD4BF', uygun_degil: CHROME_RENK.muted };
...
export function JinekolojiSpine({ patientId, ofisModulleri }: { patientId: string; ofisModulleri?: boolean }) {
  ...
  return (
    <div style={{ ...toolsCard, marginBottom: 12 }} data-chapter="jinekoloji-spine">
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>{SEKME.map((x) => <button key={x} type="button" onClick={() => setSekme(x)} style={{ ...ghost, background: sekme === x ? 'rgba(15,155,142,0.2)' : 'transparent', color: sekme === x ? '#2DD4BF' : CHROME_RENK.muted, borderRadius: 999 }}>{x}{x === 'Tarama & Görevler' && v.due.some((d) => d.durum === 'gecikti') ? ' ⚠' : ''}</button>)}</div>
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: -4, marginBottom: 6 }}><button type="button" onClick={() => setKaynakAcik(!kaynakAcik)} style={{ ...ghost, padding: '2px 8px', fontSize: 10, color: kaynakAcik ? '#2DD4BF' : CHROME_RENK.muted }}>{kaynakAcik ? 'Kaynak: açık (yalnız hekim)' : 'Kaynak'}</button></div>
      ...
      {sekme === 'Menopoz / HRT' && (<div>
        ...
        {v.hrt?.degerlendirme && <div style={{ fontSize: 12, marginTop: 6 }}>{v.hrt.degerlendirme.engeller.map((e) => <div key={e} style={{ color: '#F87171' }}>✖ {e}</div>)}{v.hrt.degerlendirme.uyarilar.map((e) => <div key={e} style={{ color: '#FBBF24' }}>⚠ {e}</div>)}{v.hrt.degerlendirme.eksikler.map((e) => <div key={e} style={{ color: CHROME_RENK.muted }}>○ eksik: {e}</div>)}{v.hrt.hrt_basladi && <div style={{ color: '#2DD4BF' }}>✓ HRT {v.hrt.hrt_baslangic}{v.hrt.rejim ? ` · ${v.hrt.rejim}` : ''} · yıllık: {v.kutuphane.hrtYillik.join(', ')}</div>}</div>}
      </div>)}
