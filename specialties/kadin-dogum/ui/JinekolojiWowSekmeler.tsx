...
const chk = (label: string, v: boolean, on: (x: boolean) => void) => (
  <label key={label} style={{ ...kucuk, display: 'flex', gap: 4, alignItems: 'center', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 999, padding: '2px 8px', color: v ? '#2DD4BF' : '#8FA0B5' }}>
  ...
  if (sekme === 'CYBH tedavi') {
    ...
    return (
      <div>
        ...
        {plan?.satirlar?.map((r) => (
          <div key={r.baslik} style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '8px 0', fontSize: 12, color: '#EDF1F7' }}>
            <b>{r.baslik}</b>
  ...
  if (sekme === 'Acil KB / MEC') {
    ...
    return (
      <div>
        ...
        {acil?.oneri && <div style={{ fontSize: 12, color: '#EDF1F7', marginTop: 8 }}><b>{acil.oneri}</b></div>}
        {acil?.secenekler?.map((c) => <div key={c.ad} style={{ fontSize: 12, color: '#EDF1F7', borderTop: '1px solid rgba(255,255,255,0.06)', padding: '4px 0' }}><b>{c.ad}</b> · {c.pencere} · {c.etkinlik}<div style={kucuk}>{c.not}</div></div>)}
        ...
        {acil?.postpartum?.map((p) => <div key={p.yontem} style={{ fontSize: 12, color: '#EDF1F7' }}>{p.yontem}: {p.baslangic} <span style={kucuk}>{p.not}</span></div>)}
      </div>
    )
  }
  ...
  if (sekme === 'Menoraji tedavi') {
    ...
    return (
      <div>
        ...
        {m?.basamaklar?.map((x) => (
          <div key={x.sira} style={{ fontSize: 12, color: x.uygun ? '#EDF1F7' : '#64748B', borderTop: '1px solid rgba(255,255,255,0.06)', padding: '6px 0' }}>
  ...
  if (sekme === 'Anti-D döngü') {
    ...
    return (
      <div>
        ...
        {a?.dozNotu && <div style={{ fontSize: 12, color: '#8FA0B5' }}>{a.dozNotu}</div>}
        {a?.gorevler?.map((g) => <div key={g.ad} style={{ fontSize: 12, color: '#EDF1F7' }}>• {g.ad} <span style={kucuk}>({g.dueHint})</span></div>)}
      </div>
    )
  }
  ...
  if (sekme === 'Paket') {
    ...
    return (
      <div>
        ...
        {p?.satirlar?.map((x) => <div key={x.ad} style={{ fontSize: 12, color: x.asildi ? '#F87171' : '#EDF1F7' }}>{x.ad}: {x.kullanilan}/{x.limit} (kalan {x.kalan})</div>)}
      </div>
    )
  }
  ...
  if (sekme === 'C/S savunma') {
    ...
    return (
      <div>
        ...
        {c?.checklist?.map((x) => <div key={x} style={{ fontSize: 12, color: '#8FA0B5' }}>□ {x}</div>)}
  ...
  if (sekme === 'Ürojine / Onkoloji') {
    ...
    return (
      <div>
        ...
        {u && <div style={{ fontSize: 12, color: '#EDF1F7' }}>Ürojine öncelik: {u.oncelik}{u.gorevler?.map((g) => <div key={g}>• {g}</div>)}<div style={kucuk}>{u.not}</div></div>}
  ...
  if (sekme === 'Şiddet / KOK yıllık') {
    ...
    return (
      <div>
        ...
        {sid && <div style={{ fontSize: 12, color: sid.durum === 'evet' ? '#F87171' : '#8FA0B5' }}>{sid.durum}: {sid.not}{sid.gorevler?.map((g) => <div key={g}>• {g}</div>)}</div>}
        {kok && <div style={{ fontSize: 12, color: '#EDF1F7' }}>KOK yıllık due {kok.due}: {kok.maddeler.join(', ')}</div>}
      </div>
    )
  }
