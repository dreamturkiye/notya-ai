...
export default function GastroenterolojiHome({ patientId }: { patientId: string }) {
  ...
  if (!v) return <div style={{ ...toolsCard, color: '#8FA0B5', fontSize: 12 }}>Gastroenteroloji yükleniyor…</div>
  ...
  return (
    <div style={toolsCard} data-chapter="gastroenteroloji">
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
        {SEKMELER.map((x) => (
          <button key={x} type="button" onClick={() => setSekme(x)} style={{ ...ghost, borderRadius: 999, background: sekme === x ? 'rgba(244,63,94,0.22)' : 'transparent', color: sekme === x ? '#FFE4E6' : '#8FA0B5' }}>{x}</button>
        ))}
      </div>
