...
import { gastroStil, Istatistik, TaslakNotu, Rozet } from './GastroAracKabugu'
import { GASTRO_BAYRAK_AD, gastroKohortFiltre, type GastroKohortBayrak, type GastroKohortSatir } from '../../engines/kohort'

const S = gastroStil
const BAYRAKLAR = Object.keys(GASTRO_BAYRAK_AD) as GastroKohortBayrak[]

export default function GastroKohortAraci() {
  const [satirlar, setSatirlar] = useState<GastroKohortSatir[] | null>(null)
  const [toplam, setToplam] = useState(0)
  const [suzgec, setSuzgec] = useState<GastroKohortBayrak[]>([])
  ...
  const yukle = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch('/api/doktor/gastroenteroloji/kohort', { headers: { Authorization: `Bearer ${t}` }, cache: 'no-store' })
  ...
  const gorunen = gastroKohortFiltre(satirlar || [], suzgec)

  const gonder = async () => {
    setDurum(''); setHata(''); setGonderiliyor(true)
    try {
      const t = await getAccessTokenAsync()
      const r = await fetch('/api/doktor/gastroenteroloji/kohort', {
        method: 'POST',
  ...
  return (
    <>
      <div style={S.kutu}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 10 }}>
          {BAYRAKLAR.map((b) => (
            <button key={b} type="button" onClick={() => setSuzgec((p) => (p.includes(b) ? p.filter((x) => x !== b) : [...p, b]))}
              style={{ ...S.ghost, background: suzgec.includes(b) ? 'rgba(244,63,94,0.25)' : 'transparent' }}>
              {GASTRO_BAYRAK_AD[b]}
            </button>
          ))}
        </div>
        ...
        {gorunen.map((s) => (
          <label key={s.patientId} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div>
              ...
              <div style={S.kucuk}>
                {s.bayraklar.map((b) => <Rozet key={b} ton={b === 'risk_acik' ? 'kirmizi' : 'uyari'}>{GASTRO_BAYRAK_AD[b]}</Rozet>)}
        ...
        <TaslakNotu>TASLAK — Hatırlatma tanı, skor, ilaç adı ve doz yazılmaz. Açık acil bayrağında “sizi arayacağız” dili kullanılır. PPI/biyolojik yalnız tarih bayrağıdır.</TaslakNotu>
      </div>
    </>
  )
}
