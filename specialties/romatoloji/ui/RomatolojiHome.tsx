'use client'
/**
 * ROMATOLOJI-EXCEPTIONAL-01 — Romatoloji bölüm ana ekranı.
 * Sticky şerit + sekmeler: Özet | Skor | Lab/Eklem | SUT | Acil | Görevler.
 */
...
import type { RomaSerit } from '../engines/serit'
import { CHROME_RENK } from '@/lib/doktor/chromeTheme';

type Veri = {
  serit: RomaSerit
  bolum: { nextKontrol: string | null }
  sonSkor: { das28: { toplam: number | null; bant: string | null; tarih: string } | null; basdai: { toplam: number | null; bant: string | null; tarih: string } | null }
  sonLab: { crp: { deger: number | null; bant: string | null; tarih: string } | null }
...
const SEKMELER = ['Özet', 'Skor', 'Lab', 'SUT', 'Acil', 'Görevler'] as const
...
export default function RomatolojiHome({ patientId }: { patientId: string }) {
  ...
  const [labTur, setLabTur] = useState('crp')
  const [labDeger, setLabDeger] = useState('')

  const api = useCallback(async (body: Record<string, unknown>) => {
    const token = await getAccessTokenAsync()
    const r = await fetch('/api/doktor/romatoloji', { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ patientId, ...body }) })
  ...
  const yukle = useCallback(async () => {
    const token = await getAccessTokenAsync()
    const r = await fetch(`/api/doktor/romatoloji?patientId=${encodeURIComponent(patientId)}`, { headers: { Authorization: `Bearer ${token}` }, cache: 'no-store' })
  ...
  if (!v) return <div style={{ ...toolsCard, color: CHROME_RENK.muted, fontSize: 12 }}>Romatoloji yükleniyor…</div>

  return (
    <div style={toolsCard} data-chapter="romatoloji">
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
        {SEKMELER.map((x) => (
          <button key={x} type="button" onClick={() => setSekme(x)} style={{ ...ghost, borderRadius: 999, background: sekme === x ? 'rgba(217,119,6,0.22)' : 'transparent', color: sekme === x ? '#FEF3C7' : CHROME_RENK.muted }}>{x}</button>
        ))}
      </div>
      ...
      {sekme === 'Özet' && (
        <div>
          ...
          <div style={metin}>
            {v.sonSkor.das28
              ? <>Son DAS28 <b>{v.sonSkor.das28.toplam ?? '—'}</b> ({String(v.sonSkor.das28.tarih).slice(0, 10)}) — {v.sonSkor.das28.bant || '—'}. Bant karar desteğidir.</>
              : 'DAS28 kaydı yok — Skor sekmesinden girin.'}
          </div>
      ...
      {sekme === 'Skor' && (
        <div>
          <div style={etiket}>DAS28-CRP (hızlı giriş)</div>
          <div style={satir}>
            <input placeholder="TJC" value={tjc} onChange={(e) => setTjc(e.target.value)} style={{ ...toolsInput, width: 70 }} />
            <input placeholder="SJC" value={sjc} onChange={(e) => setSjc(e.target.value)} style={{ ...toolsInput, width: 70 }} />
            <input placeholder="PGA" value={pga} onChange={(e) => setPga(e.target.value)} style={{ ...toolsInput, width: 70 }} />
            <input placeholder="CRP" value={crp} onChange={(e) => setCrp(e.target.value)} style={{ ...toolsInput, width: 70 }} />
            <button type="button" style={btn} onClick={() => calistir({ adim: 'skor', tur: 'das28_crp', tjc: Number(tjc), sjc: Number(sjc), pga: Number(pga), crp: Number(crp), hekimKilit: true }, 'DAS28 kaydedildi.')}>Kaydet</button>
          </div>
          <div style={{ ...kucuk, marginTop: 8 }}>Tam ölçek ve BASDAI için Araçlar › DAS28 / BASDAI.</div>
        </div>
      )}

      {sekme === 'Lab' && (
        <div>
          <div style={etiket}>CRP / ESR</div>
          <div style={satir}>
            {(['crp', 'esr'] as const).map((t) => (
              <button key={t} type="button" style={{ ...ghost, background: labTur === t ? 'rgba(217,119,6,0.22)' : 'transparent' }} onClick={() => setLabTur(t)}>{t.toUpperCase()}</button>
            ))}
            <input type="number" step="0.1" value={labDeger} onChange={(e) => setLabDeger(e.target.value)} style={{ ...toolsInput, width: 100 }} />
            <button type="button" style={btn} onClick={() => calistir({ adim: 'lab', tur: labTur, deger: Number(labDeger), hekimKilit: true }, 'Lab kaydedildi.')}>Kaydet</button>
          </div>
        </div>
      )}
