...
export default function RomatolojiHome({ patientId }: { patientId: string }) {
  ...
  if (!v) return <div style={{ ...toolsCard, color: '#8FA0B5', fontSize: 12 }}>Romatoloji yükleniyor…</div>
  ...
  return (
    <div style={toolsCard} data-chapter="romatoloji">
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
        {SEKMELER.map((x) => (
          <button key={x} type="button" onClick={() => setSekme(x)} style={{ ...ghost, borderRadius: 999, background: sekme === x ? 'rgba(217,119,6,0.22)' : 'transparent', color: sekme === x ? '#FEF3C7' : '#8FA0B5' }}>{x}</button>
        ))}
      </div>
