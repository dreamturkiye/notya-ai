'use client'
/**
 * ENFEKSIYON-EXCEPTIONAL-01 — Araçlar › Enfeksiyon stüdyoları ortak kabuğu.
 * Yalnız enfeksiyon hastalıkları hekimi açar; başka branş /doktor-tools'a döner.
 */
...
export const ENF_VURGU: AracVurgu = { ana: '#0D9488', anaMetin: '#F0FDFA', yumusak: '#5EEAD4', baslik: '#2DD4BF' }
export const enfStil = aracStil(ENF_VURGU)
...
export function EnfHastaSecici({ secili, sec }: { secili: string; sec: (id: string, ad: string) => void }) {
  return <HastaSecici secili={secili} sec={sec} />
}

export default function EnfAracKabugu({ route, baslik, aciklama, children }: { route: string; baslik: string; aciklama: string; children: React.ReactNode }) {
  ...
  return (
    <AracVurguSaglayici vurgu={ENF_VURGU}>
      <div style={{ ...toolsShell, overflowX: 'hidden' }}>
        <DoktorNav />
        <div style={{ maxWidth: 1000, margin: '0 auto', padding: '24px 16px 56px', boxSizing: 'border-box' }}>
          {!izin ? (
            <div style={{ color: '#9BB0C7', fontSize: 15, padding: '12px 0' }}>{izin === null ? 'Yükleniyor…' : 'Bu araç yalnızca enfeksiyon hastalıkları için.'}</div>
          ) : (
            <>
              <div style={{ marginBottom: 18 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: ENF_VURGU.baslik, letterSpacing: '1.4px', textTransform: 'uppercase', marginBottom: 8 }}>Araçlar · Enfeksiyon Hastalıkları</div>
