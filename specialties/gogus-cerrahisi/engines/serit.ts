/**
 * GOGUS-CERRAHISI-EXCEPTIONAL-01 — Sticky vizit şeridi. SAF fonksiyon.
 * Çipler: acil bayrak · pre-op · tüp/yara · patoloji · kontrol · gecikmiş.
 */
import { gunFarki } from './gogus-cerrahisi'
...
export interface GcSeritGirdi {
  bugun: string
  preopSayi: number
  tupYaraDurum: string | null
  patolojiHazir: boolean | null
...
export function gcSeridi(g: GcSeritGirdi): GcSerit {
  ...
  if (g.riskBayraklari.length) {
    ...
    if (!g.riskHekimOnay) kirmizi.push(`Açık göğüs cerrahisi acil bayrağı (${g.riskBayraklari.join('; ')}) — hekim onayı olmadan vizit kapatılmaz`)
  } else {
  ...
  chips.push(
    g.tupYaraDurum
      ? { ad: 'Tüp/yara', deger: g.tupYaraDurum, durum: g.tupYaraDurum === 'dikkat' ? 'dikkat' : 'iyi' }
      : { ad: 'Tüp/yara', deger: '—', durum: 'yok', alt: 'izlem yok' },
  )

  chips.push(
    g.patolojiHazir === true
      ? { ad: 'Patoloji', deger: 'hazır', durum: 'dikkat', alt: 'hekim değerlendirir' }
      : g.patolojiHazir === false
        ? { ad: 'Patoloji', deger: 'bekliyor', durum: 'yok' }
