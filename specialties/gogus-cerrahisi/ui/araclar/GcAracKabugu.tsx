'use client'
/**
 * GOGUS-CERRAHISI-EXCEPTIONAL-01 — Araçlar › Göğüs Cerrahisi stüdyoları ortak kabuğu.
 * Yalnız gogus-cerrahisi hekimi açar; gogus-hastaliklari / dahiliye /doktor-tools'a döner.
 */
...
export const GC_VURGU: AracVurgu = { ana: '#B91C1C', anaMetin: '#FEF2F2', yumusak: '#FCA5A5', baslik: '#F87171' }
...
export function GcHastaSecici({ secili, sec }: { secili: string; sec: (id: string, ad: string) => void }) {
  return <HastaSecici secili={secili} sec={sec} />
}

export default function GcAracKabugu({ route, baslik, aciklama, children }: { route: string; baslik: string; aciklama: string; children: React.ReactNode }) {
    ...
    <AracVurguSaglayici vurgu={GC_VURGU}>
      <div style={{ ...toolsShell, overflowX: 'hidden' }}>
        ...
        <div style={{ maxWidth: 1000, margin: '0 auto', padding: '24px 16px 56px', boxSizing: 'border-box' }}>
          {!izin ? (
            <div style={{ color: '#9BB0C7', fontSize: 15, padding: '12px 0' }}>{izin === null ? 'Yükleniyor…' : 'Bu araç yalnızca göğüs cerrahisi için.'}</div>
          ) : (
            <>
              <div style={{ marginBottom: 18 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: GC_VURGU.baslik, letterSpacing: '1.4px', textTransform: 'uppercase', marginBottom: 8 }}>Araçlar · Göğüs Cerrahisi</div>
