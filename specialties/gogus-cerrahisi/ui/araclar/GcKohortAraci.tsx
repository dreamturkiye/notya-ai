...
export default function GcKohortAraci() {
  ...
  const yukle = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch('/api/doktor/gogus-cerrahisi/kohort', { headers: { Authorization: `Bearer ${t}` }, cache: 'no-store' })
  ...
  const gonder = async () => {
    ...
    try {
      const t = await getAccessTokenAsync()
      const r = await fetch('/api/doktor/gogus-cerrahisi/kohort', {
        method: 'POST',
  ...
  return (
    <>
      <div style={S.kutu}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 10 }}>
          {BAYRAKLAR.map((b) => (
            <button key={b} type="button" onClick={() => setSuzgec((p) => (p.includes(b) ? p.filter((x) => x !== b) : [...p, b]))}
              style={{ ...S.ghost, background: suzgec.includes(b) ? 'rgba(185,28,28,0.25)' : 'transparent' }}>
              {GC_BAYRAK_AD[b]}
            </button>
          ))}
        </div>
        ...
        <TaslakNotu>TASLAK — hatırlatma tanı / CAT / doz taşımaz. Ameliyathane HIS bu panelde yoktur.</TaslakNotu>
      </div>
    </>
  )
}
