...
export type KartAdi = 'ht' | 'dm' | 'lipid' | 'tiroid' | 'kvr' | 'ckd' | 'anemi' | 'obezite' | 'hf' | 'antikoagulan' | 'gi' | 'pulm' | 'checkup' | 'asi' | 'tarama' | 'izlem' | 'sgk' | 'ekg' | 'nodul' | 'ramazan' | 'nudge' | 'polifarmasi' | 'hedef' | 'sigara' | 'vitamin' | 'gut' | 'osteo'
...
export function kilitDogrula(kart: string, alan: string): string | null {
  const izinli: Record<string, string[]> = { ht: ['evre', 'hedef'], dm: ['hedef_hba1c', 'tip'], lipid: ['hedef_ldl'], tiroid: ['tani'], kvr: ['kategori', 'hedef_ldl'], ckd: ['evre', 'kronik'], anemi: ['plan'], obezite: ['plan'], hf: ['nyha', 'plan'], antikoagulan: ['ajan', 'hedef_inr', 'endikasyon'], gi: ['tani', 'plan'], pulm: ['tani', 'plan'], checkup: ['paket', 'rapor'], asi: ['plan'], tarama: ['plan'], izlem: ['sustur'], sgk: ['rapor'], ekg: ['rapor'], nodul: ['tarif'], ramazan: ['risk', 'plan'], nudge: ['kirilganlik', 'phq2', 'kb_teknik'], polifarmasi: ['plan'], hedef: ['kart'], sigara: ['plan'], vitamin: ['plan'], gut: ['tani', 'plan'], osteo: ['tani', 'plan'] }
