...
/** Görüntüleme yuvarlaması (saklanan değer değişmez): birim dönüşümünden gelen 119.877 mg/dL → 120; 7.9 → 7.9; 0.995 → 1. */
export function gosterimSayi(x: number): string {
  const b = Math.abs(x) >= 100 ? 0 : Math.abs(x) >= 10 ? 1 : 2
  return String(Math.round(x * 10 ** b) / 10 ** b)
}
...
export function vizitSeridi(g: SeritGirdi): VizitSeridi {
  ...
  chips.push(g.ldl ? { ad: 'LDL', deger: gosterimSayi(g.ldl.deger), durum: g.ldl.hedef != null && g.ldl.deger > g.ldl.hedef ? 'kotu' : 'iyi', alt: g.ldl.hedef != null ? `hedef <${g.ldl.hedef}` : 'hedef kilitli değil' } : { ad: 'LDL', deger: '—', durum: 'yok' })
  chips.push(g.egfr ? { ad: 'eGFR', deger: `${gosterimSayi(g.egfr.deger)}${g.egfr.evre ? ` ${g.egfr.evre}` : ''}`, durum: g.egfr.renk === 'kirmizi' ? 'kotu' : g.egfr.renk === 'turuncu' || g.egfr.renk === 'sari' ? 'dikkat' : 'iyi' } : { ad: 'eGFR', deger: '—', durum: 'yok' })
