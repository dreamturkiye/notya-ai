 ...
 * paket tarihinden SONRA gelen sonuçla "tamam" sayılır; hekim elle işaretleyebilir. Birleşik rapor: onaylı lab + onaylı belge
 * özetleri + kartların hekim kilitli özetleri. Rapor hekim kilidi olmadan "taslak" damgalı.
 */
import type { Dipnot } from './dahiliye'

export interface PaketKalem { kod: string; ad: string; keys?: string[]; belge?: string; opsiyonel?: boolean; not?: string }
...
export const PAKETLER: PaketSku[] = [
  ...
  { sku: 'ileri65', ad: 'İleri yaş check-up (65+)', uygun: (y) => y != null && y >= 65, kalemler: [...KAPSAMLI, { kod: 'dxa', ad: 'DXA (kadın ≥65 / risk)', not: 'TEMD Osteoporoz; elle işaretle', opsiyonel: true }, { kod: 'kirilganlik', ad: 'Kırılganlık + düşme taraması', not: 'Dahiliye › nudge kartı' }, { kod: 'isitme_gorme', ad: 'İşitme / görme sorgusu', not: 'elle işaretle' }] },
]
...
export interface KalemDurum { kod: string; ad: string; tamam: boolean; kaynak: 'lab' | 'belge' | 'hekim' | null; tarih: string | null; opsiyonel: boolean; not?: string }
export function paketDurumu(p: { sku: string; tarih: string; manuelTamam: string[] }, labTarihleri: Record<string, string[]>, belgeTarihleri: Record<string, string[]>): { kalemler: KalemDurum[]; tamamlanan: number; zorunluToplam: number; bitti: boolean } {
