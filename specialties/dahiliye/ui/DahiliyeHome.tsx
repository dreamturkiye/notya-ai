...
export default function DahiliyeHome({ patientId }: { patientId: string }) {
  ...
  if (!v) return <div style={{ ...toolsCard, color: CHROME_RENK.muted, fontSize: 12 }}>Dahiliye yükleniyor…</div>;
  ...
  return (
    <div style={{ ...toolsCard }} data-chapter="dahiliye">
      {v.serit && <VizitSeridiBar s={v.serit} onPlan={() => undefined} />}
      {v.wow?.w4 && <NudgeBar w4={v.wow.w4} kaynak={kaynak} refler={v.kutuphane.refler} calistir={calistir} />}
      ...
      {sekme === 'Özet' && (<div>
        ...
        {v.gorevler.map((g) => <div key={g.id} style={{ display: 'flex', gap: 8, alignItems: 'center', fontSize: 12, color: g.due && g.due < new Date().toISOString().slice(0, 10) ? '#F87171' : CHROME_RENK.ink, padding: '2px 0' }}><span style={{ flex: 1 }}>{g.ad} <span style={kucuk}>{g.due || ''} · {g.kaynak}</span></span><button type="button" onClick={() => calistir({ adim: 'gorev', gorevId: g.id, durum: 'tamam' })} style={{ ...ghost, padding: '4px 10px', fontSize: 12, minWidth: 36, minHeight: 28, flexShrink: 0 }}>✓</button></div>)}
      ...
      {sekme === 'HT' && (<div>
        ...
        {sonHt?.degerlendirme && (<div style={{ fontSize: 12, color: CHROME_RENK.ink }}>
      ...
      {sekme === 'DM' && (<div>
        ...
        {v.dm?.degerlendirme && (<div style={{ fontSize: 12, color: CHROME_RENK.ink, marginTop: 6 }}><div>HbA1c {c.hba1c?.deger ?? '—'}%{c.hba1c?.delta != null ? ` (Δ ${c.hba1c.delta > 0 ? '+' : ''}${c.hba1c.delta})` : ''} · {v.dm.degerlendirme.kontrolde == null ? 'değer yok' : v.dm.degerlendirme.kontrolde ? 'hedefte' : 'hedef dışı'} · sonraki HbA1c {v.dm.degerlendirme.sonrakiHba1cAy} ay</div>{v.dm.degerlendirme.uyarilar.map((u) => <div key={u} style={{ color: '#FBBF24' }}>⚠ {u}</div>)}{v.dm.degerlendirme.plan.map((p) => <div key={p}>• {p}</div>)}<Kaynak d={v.dm.degerlendirme.dipnotlar} acik={kaynak} refler={v.kutuphane.refler} /></div>)}
      </div>)}
      ...
      {sekme === 'Lipid' && (<div>
        ...
        {v.lipid?.degerlendirme && (<div style={{ fontSize: 12, color: CHROME_RENK.ink, marginTop: 6 }}>{v.lipid.degerlendirme.uyarilar.map((u) => <div key={u} style={{ color: '#FBBF24' }}>⚠ {u}</div>)}{v.lipid.degerlendirme.plan.map((p) => <div key={p}>• {p}</div>)}<Kaynak d={v.lipid.degerlendirme.dipnotlar} acik={kaynak} refler={v.kutuphane.refler} /></div>)}
      </div>)}
      ...
      {sekme === 'Tiroid' && (<div>
        ...
        {v.tiroid?.degerlendirme && (<div style={{ fontSize: 12, color: CHROME_RENK.ink, marginTop: 6 }}><div>{v.tiroid.degerlendirme.yorum}</div>{v.tiroid.degerlendirme.sevk.map((x) => <div key={x} style={{ color: '#FBBF24' }}>→ {x}</div>)}<Kaynak d={v.tiroid.degerlendirme.dipnotlar} acik={kaynak} refler={v.kutuphane.refler} /></div>)}
      </div>)}
      ...
      {sekme === 'Check-up' && (<div>
        ...
        <ul style={{ fontSize: 12, color: CHROME_RENK.ink, margin: 0, paddingLeft: 18 }}>{v.kutuphane.checkup.map((x) => <li key={x}>{x}</li>)}</ul>
      ...
      {sekme === 'İlaçlar' && (<div>
        ...
        <Kaynak d={v.kutuphane.ilacDipnot} acik={kaynak} refler={v.kutuphane.refler} />
        {v.ilaclar.map((i) => <div key={i.id} style={{ fontSize: 12, color: i.aktif ? CHROME_RENK.ink : CHROME_RENK.muted }}>{i.aktif ? '● ' : '○ '}{i.ilac_adi} {i.doz || ''} {i.kullanim_sikli || ''}</div>)}
      ...
      {sekme === 'Sevk' && (<div>
        ...
        <Kaynak d={v.kutuphane.kirmiziDipnot} acik={kaynak} refler={v.kutuphane.refler} />
        {v.sevkler.map((x) => <div key={x.id} style={{ fontSize: 12, color: CHROME_RENK.ink }}>→ {x.hedef}{x.not_metni ? ` — ${x.not_metni}` : ''} <span style={kucuk}>{new Date(x.created_at).toLocaleDateString('tr-TR')}</span></div>)}
      </div>)}
    </div>
  );
}
