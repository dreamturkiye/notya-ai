...
export default function DahiliyeWow({ sekme, wow, kaynak, refler, calistir }: Props) {
  ...
  if (sekme === 'KBH') {
    ...
    return (<div>
      ...
      <div style={satir}>
        <span style={kucuk}>eGFR: <b style={{ color: CHROME_RENK.ink }}>{c?.egfr ?? '—'}</b> · UACR: <b style={{ color: CHROME_RENK.ink }}>{c?.uacr ?? '—'}</b> mg/g {c?.uacrKaynak ? `(${c.uacrKaynak})` : ''}</span>
  ...
  if (sekme === 'SGK rapor') {
    ...
    return (<div>
      ...
      {sgk && (<div style={{ fontSize: 12, color: CHROME_RENK.ink, marginTop: 8 }}>
