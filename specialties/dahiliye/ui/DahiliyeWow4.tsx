      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 18 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <div style={{ background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.28)', borderRadius: 14, padding: '12px 16px', minWidth: 88, textAlign: 'center' }}>
            <div style={{ fontSize: 22, fontWeight: 800, color: CHROME_RENK.warn, fontVariantNumeric: 'tabular-nums' }}>{bayrakli || '—'}</div>
            <div style={{ fontSize: 12, color: CHROME_RENK.muted, marginTop: 2 }}>bayraklı</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gap: 0, borderRadius: 14, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.08)' }}>
        {gorunen.map((s, i) => (
          ...
          >
            ...
            <span style={{ fontSize: 14, color: CHROME_RENK.muted }}>son vizit {s.sonVizit || '—'}{s.portalVar ? '' : ' · portal yok'}</span>
          </div>
        ))}
        {v && !gorunen.length && (
          <div style={{ padding: '36px 20px', textAlign: 'center' }}>
            <div style={{ fontSize: 17, fontWeight: 700, color: CHROME_RENK.muted, marginBottom: 8 }}>Bu filtrede hasta yok</div>
