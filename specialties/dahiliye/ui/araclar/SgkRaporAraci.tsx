...
const ISARET_RENK = (t: boolean | null) => (t === true ? '#34D399' : t === false ? '#F87171' : '#FBBF24');
    ...
    <>
      ...
      {sablon === 'dm' && (
        <div style={kutu}>
          ...
          <div style={satir}><Secim etiket="Diyabet tipi" deger={dmTip} set={setDmTip} secenekler={DM_TIP} /></div>
        </div>
      )}
      ...
      <div style={{ ...kutu, borderColor: 'rgba(20,184,166,0.35)' }} aria-live="polite">
        ...
        <KopyalaButonu metin={kopyaMetni} etiket="Taslağı kopyala" />
      </div>
      ...
      <div style={kutu}>
        ...
        {sonuc.sutKontrol.map((s) => (
          <div key={s.madde} style={{ ...metin, marginTop: 6, display: 'flex', gap: 8 }}>
            <span style={{ color: ISARET_RENK(s.tamam), fontWeight: 800 }}>{ISARET(s.tamam)}</span>
            <span>{s.madde}</span>
          </div>
        ))}
