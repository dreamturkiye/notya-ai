...
export default function CocukCerrahisiHome({ patientId }: { patientId: string }) {
  ...
  if (!v) return <div style={{ ...toolsCard, color: CHROME_RENK.muted, fontSize: 12 }}>Çocuk Cerrahisi yükleniyor…</div>
  ...
  return (
    <div style={toolsCard} data-chapter="cocuk-cerrahisi">
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
        {SEKMELER.map((x) => (
          <button key={x} type="button" onClick={() => setSekme(x)} style={{ ...ghost, borderRadius: 999, background: sekme === x ? 'rgba(8,145,178,0.22)' : 'transparent', color: sekme === x ? '#CFFAFE' : CHROME_RENK.muted }}>{x}</button>
        ))}
      </div>
