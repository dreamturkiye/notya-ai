...
import { nefStil, Istatistik, TaslakNotu, Rozet } from './NefAracKabugu'
import { NEF_BAYRAK_AD, nefKohortFiltre, type NefKohortBayrak, type NefKohortSatir } from '../../engines/kohort'
import { ILAC_DOZ_UYARI_LISTESI } from '../../engines/egfr'

const S = nefStil
const BAYRAKLAR = Object.keys(NEF_BAYRAK_AD) as NefKohortBayrak[]

export default function NefKohortAraci() {
  const [satirlar, setSatirlar] = useState<NefKohortSatir[] | null>(null)
  const [toplam, setToplam] = useState(0)
  const [suzgec, setSuzgec] = useState<NefKohortBayrak[]>([])
  ...
  const yukle = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch('/api/doktor/nefroloji/kohort', { headers: { Authorization: `Bearer ${t}` }, cache: 'no-store' })
  ...
  const gorunen = nefKohortFiltre(satirlar || [], suzgec)

  const gonder = async () => {
    setDurum(''); setHata(''); setGonderiliyor(true)
    try {
      const t = await getAccessTokenAsync()
      const r = await fetch('/api/doktor/nefroloji/kohort', {
        method: 'POST',
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 10 }}>
          {BAYRAKLAR.map((b) => (
            <button key={b} type="button" onClick={() => setSuzgec((p) => (p.includes(b) ? p.filter((x) => x !== b) : [...p, b]))}
              style={{ ...S.ghost, background: suzgec.includes(b) ? 'rgba(6,182,212,0.25)' : 'transparent' }}>
              {NEF_BAYRAK_AD[b]}
            </button>
          ))}
        </div>
        ...
        {gorunen.map((s) => (
          <label key={s.patientId} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div>
              ...
              <div style={S.kucuk}>
                {s.bayraklar.map((b) => <Rozet key={b} ton={b === 'risk_acik' ? 'kirmizi' : 'uyari'}>{NEF_BAYRAK_AD[b]}</Rozet>)}
