  ...
  return (
    <div style={toolsCard} data-chapter="nefroloji">
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
        {SEKMELER.map((x) => (
          <button key={x} type="button" onClick={() => setSekme(x)} style={{ ...ghost, borderRadius: 999, background: sekme === x ? 'rgba(6,182,212,0.22)' : 'transparent', color: sekme === x ? '#CFFAFE' : '#8FA0B5' }}>{x}</button>
        ))}
      </div>
