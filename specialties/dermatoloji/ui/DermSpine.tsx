...
export function DermSpine({ patientId }: { patientId: string }) {
  ...
  if (!v) return <div style={{ ...toolsCard, color: '#8FA0B5', fontSize: 12 }}>Derm spine yükleniyor…</div>;
  const lez = v.lezyonlar.find((l) => l.id === s('lezyonId')) || v.lezyonlar[0];

  return (
    <div style={{ ...toolsCard, marginBottom: 12 }} data-chapter="derm-spine">
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>{SEKME.filter((x) => x !== 'Kozmetik' || kozmetikAcik).map((x) => <button key={x} type="button" onClick={() => setSekme(x)} style={{ ...ghost, background: sekme === x ? 'rgba(15,155,142,0.2)' : 'transparent', color: sekme === x ? '#2DD4BF' : '#8FA0B5', borderRadius: 999 }}>{x}{x === 'Lezyon değerlendirme' && v.lezyonlar.some((l) => l.acil) ? ' ⚠' : ''}</button>)}{!kozmetikAcik && <button type="button" onClick={() => setKozmetikAcik(true)} style={{ ...ghost, borderRadius: 999, color: '#64748B', fontSize: 10 }}>+ Kozmetik (isteğe bağlı)</button>}</div>
      {mesaj && <div style={{ fontSize: 12, color: /amadı|eksik|geçersiz|Hata/.test(mesaj) ? '#F87171' : '#2DD4BF', marginBottom: 8 }}>{mesaj}</div>}
      ...
      {sekme === 'Lezyon değerlendirme' && (<div>
        ...
        {v.lezyonlar.length > 0 && (<>
          ...
          {lez?.degerlendirme && <div style={{ fontSize: 12, color: '#EDF1F7', marginTop: 6 }}><span style={{ fontWeight: 800, color: lez.acil ? '#F87171' : '#2DD4BF' }}>{lez.acil ? '⚠ MELANOM ŞÜPHESİ (acil bayrak)' : 'Şüphe yok'}</span> · ABCDE {lez.degerlendirme.abcdePuan}/5{lez.size_mm ? ` · ${lez.size_mm} mm` : ''}{lez.resmi_tani ? ` · Resmi tanı: ${lez.resmi_tani}` : ''}{lez.patoloji_sonuc ? ` · Patoloji: ${lez.patoloji_sonuc}` : ''}<ul style={{ ...kucuk, margin: '4px 0 0', paddingLeft: 18 }}>{lez.degerlendirme.oneri.map((o) => <li key={o}>{o}</li>)}</ul></div>}
      ...
      {sekme === 'İşlemler' && (<div>
        ...
        {v.islemler.map((i) => <div key={i.id} style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '5px 0', fontSize: 12, color: '#EDF1F7' }}><b>{i.tarih}</b> · {v.kutuphane.islemler.find((x) => x.kod === i.tur)?.ad || i.tur}{i.onam_id ? ' · onam ✓' : ''}{i.patoloji_sonuc ? <span style={kucuk}> · patoloji: {i.patoloji_sonuc}</span> : (['punch', 'shave', 'eksizyon'].includes(i.tur) && <span style={satir}><input value={s('pat_' + i.id)} onChange={(e) => set('pat_' + i.id, e.target.value)} placeholder="patoloji sonucu" style={{ ...toolsInput, minWidth: 220 }} /><button type="button" onClick={() => calistir({ adim: 'islem_patoloji', islemId: i.id, patolojiSonuc: s('pat_' + i.id) }, 'Patoloji aynı lezyona bağlandı.')} style={ghost}>Bağla</button></span>)}<div style={kucuk}>Yara bakımı: {i.yara_bakimi.join(' · ')}</div></div>)}
      </div>)}
      ...
      {sekme === 'İlaç güvenliği' && (<div>
        ...
        {v.ilacGuvenlik.map((i) => <div key={i.id} style={{ fontSize: 12, color: '#EDF1F7', padding: '4px 0', borderTop: '1px solid rgba(255,255,255,0.06)' }}>{i.ilac} · başlangıç {i.baslangic || '—'}{i.aylik_due ? ` · aylık β-hCG ${i.aylik_due}` : ''}{i.kapisi?.uyari?.length ? <span style={{ color: '#FBBF24' }}> · {i.kapisi.uyari.join('; ')}</span> : null}</div>)}
