    ...
    <div style={{ ...toolsCard, marginBottom: 12 }} data-chapter="derm-spine">
      ...
      {sekme === 'Lezyon değerlendirme' && (<div>
        ...
        {v.lezyonlar.length > 0 && (<>
          ...
          {lez?.degerlendirme && <div style={{ fontSize: 12, color: CHROME_RENK.ink, marginTop: 6 }}><span style={{ fontWeight: 800, color: lez.acil ? '#F87171' : '#2DD4BF' }}>{lez.acil ? '⚠ MELANOM ŞÜPHESİ (acil bayrak)' : 'Şüphe yok'}</span> · ABCDE {lez.degerlendirme.abcdePuan}/5{lez.size_mm ? ` · ${lez.size_mm} mm` : ''}{lez.resmi_tani ? ` · Resmi tanı: ${lez.resmi_tani}` : ''}{lez.patoloji_sonuc ? ` · Patoloji: ${lez.patoloji_sonuc}` : ''}<ul style={{ ...kucuk, margin: '4px 0 0', paddingLeft: 18 }}>{lez.degerlendirme.oneri.map((o) => <li key={o}>{o}</li>)}</ul></div>}
