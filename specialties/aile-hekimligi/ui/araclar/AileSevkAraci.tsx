...
import { aileStil, AileHastaSecici, Onay, KopyalaButonu, Istatistik, TaslakNotu, useUrlHasta } from './AileAracKabugu'
import { acilTara, hekimOnayiGerekliMi, ACIL_KODLARI, ACIL_KONTROL_LISTESI, HASTA_ACIL_METNI, type AcilKod } from '../../engines/acil'
import { ACIL_YONLENDIRME_METNI } from '../../engines/aile'

export default function AileSevkAraci() {
  ...
  const ozet = [
    bayraklar.length ? `Sevk/acil bayrakları: ${bayraklar.map((b) => b.ad).join('; ')}` : 'Sevk/acil bayrağı saptanmadı (hekim değerlendirmesi esas)',
  ...
  return (
    <>
      <div style={{ ...aileStil.kutu, borderColor: 'rgba(248,113,113,0.45)', background: 'rgba(248,113,113,0.08)' }}>{ACIL_YONLENDIRME_METNI}</div>
      <div style={aileStil.kutu}>
        ...
        <div style={{ ...aileStil.etiket, marginTop: 12 }}>Hekim işareti</div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {ACIL_KODLARI.map((k) => (
            <Onay key={k.kod} ad={k.ad} deger={isaretli.includes(k.kod)} set={(b) => setIsaretli((p) => (b ? [...p, k.kod] : p.filter((x) => x !== k.kod)))} />
          ))}
        </div>
        <div style={{ ...aileStil.etiket, marginTop: 12 }}>Hastanın ifadesi (isteğe bağlı)</div>
        <textarea value={metin} onChange={(e) => setMetin(e.target.value)} rows={3} aria-label="Hastanın ifadesi" style={{ ...aileStil.input, width: '100%', marginTop: 8, resize: 'vertical' }} />
      </div>
      <div style={aileStil.kutu}>
        <div style={aileStil.etiket}>Triyaj sonucu</div>
        ...
        {bayraklar.map((b) => (
          <div key={b.kod} style={{ ...aileStil.metin, marginBottom: 6 }}>
            <b>{b.ad}</b> — {b.eylem}
          </div>
        ))}
        {!bayraklar.length && <div style={aileStil.kucuk}>Henüz bayrak yok — hekim değerlendirmesi esas.</div>}
        <div style={{ ...aileStil.etiket, marginTop: 12 }}>Kontrol listesi</div>
        {ACIL_KONTROL_LISTESI.map((x, i) => (
          <Onay key={x} ad={x} deger={liste.includes(i)} set={(b) => setListe((p) => (b ? [...p, i] : p.filter((y) => y !== i)))} />
        ))}
        <div style={{ marginTop: 10 }}>
          <input value={eylem} onChange={(e) => setEylem(e.target.value)} placeholder="Hekim eylemi / sevk notu" style={aileStil.input} />
        </div>
        {onayGerek && (
          <div style={{ marginTop: 10 }}>
        ...
        <TaslakNotu>Sevk/acil bayrakları karar desteğidir; tanı yazmaz. Açık “hemen/aynı gün” bayrakta hekim onayı olmadan kayıt yok (409).</TaslakNotu>
        ...
        <div style={{ ...aileStil.kucuk, marginTop: 10 }}>{HASTA_ACIL_METNI}</div>
      </div>
    </>
  )
}
