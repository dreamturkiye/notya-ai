...
import { aileStil, Istatistik, TaslakNotu, Rozet } from './AileAracKabugu'
import { AILE_BAYRAK_AD, aileKohortFiltre, type AileKohortBayrak, type AileKohortSatir } from '../../engines/kohort'

const S = aileStil
const BAYRAKLAR = Object.keys(AILE_BAYRAK_AD) as AileKohortBayrak[]

export default function AileKohortAraci() {
  const [satirlar, setSatirlar] = useState<AileKohortSatir[] | null>(null)
  const [toplam, setToplam] = useState(0)
  const [suzgec, setSuzgec] = useState<AileKohortBayrak[]>([])
  ...
  const yukle = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch('/api/doktor/aile-hekimligi/kohort', { headers: { Authorization: `Bearer ${t}` }, cache: 'no-store' })
  ...
  const gorunen = aileKohortFiltre(satirlar || [], suzgec)

  const gonder = async () => {
    setDurum(''); setHata(''); setGonderiliyor(true)
    try {
      const t = await getAccessTokenAsync()
      const r = await fetch('/api/doktor/aile-hekimligi/kohort', {
        method: 'POST',
    ...
    <>
      <div style={S.kutu}>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 10 }}>
          ...
          <Istatistik deger={satirlar == null ? '…' : gorunen.filter((s) => s.bayraklar.includes('risk_acik')).length} etiket="Açık sevk/acil bayrağı" ton={gorunen.some((s) => s.bayraklar.includes('risk_acik')) ? 'kirmizi' : 'notr'} />
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 10 }}>
          {BAYRAKLAR.map((b) => (
            <button key={b} type="button" onClick={() => setSuzgec((p) => (p.includes(b) ? p.filter((x) => x !== b) : [...p, b]))}
              style={{ ...S.ghost, background: suzgec.includes(b) ? 'rgba(21,128,61,0.25)' : 'transparent' }}>
              {AILE_BAYRAK_AD[b]}
            </button>
          ))}
        </div>
        ...
        {gorunen.map((s) => (
          <label key={s.patientId} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div>
              ...
              <div style={S.kucuk}>
                {s.bayraklar.map((b) => <Rozet key={b} ton={b === 'risk_acik' ? 'kirmizi' : 'uyari'}>{AILE_BAYRAK_AD[b]}</Rozet>)}
        ...
        <TaslakNotu>Hatırlatma tanı, skor, ilaç adı ve doz yazılmaz. Açık sevk/acil bayrağında “sizi arayacağız” dili kullanılır.</TaslakNotu>
      </div>
    </>
  )
}
