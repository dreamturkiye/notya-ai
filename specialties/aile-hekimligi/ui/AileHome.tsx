...
export default function AileHome({ patientId }: { patientId: string }) {
  ...
  if (!v) return <div style={{ ...toolsCard, color: '#8FA0B5', fontSize: 12 }}>Aile Hekimliği yükleniyor…</div>
  ...
  return (
    <div style={toolsCard} data-chapter="aile-hekimligi">
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
        {SEKMELER.map((x) => (
          <button key={x} type="button" onClick={() => setSekme(x)} style={{ ...ghost, borderRadius: 999, background: sekme === x ? 'rgba(21,128,61,0.22)' : 'transparent', color: sekme === x ? '#BBF7D0' : '#8FA0B5' }}>{x}</button>
        ))}
      </div>
