...
export default function OrtopediHome({ patientId }: { patientId: string }) {
  ...
  if (!v) return <div style={{ ...toolsCard, color: '#8FA0B5', fontSize: 12 }}>Ortopedi yükleniyor…</div>
  ...
  return (
    <div style={toolsCard} data-chapter="ortopedi">
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
        {SEKMELER.map((x) => (
          <button key={x} type="button" onClick={() => setSekme(x)} style={{ ...ghost, borderRadius: 999, background: sekme === x ? 'rgba(217,119,6,0.22)' : 'transparent', color: sekme === x ? '#FDE68A' : '#8FA0B5' }}>{x}</button>
        ))}
      </div>
