...
export default function OrtoKohortAraci() {
  const [satirlar, setSatirlar] = useState<OrtoKohortSatir[] | null>(null)
  const [toplam, setToplam] = useState(0)
  const [suzgec, setSuzgec] = useState<OrtoKohortBayrak[]>([])
  ...
  const yukle = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch('/api/doktor/ortopedi/kohort', { headers: { Authorization: `Bearer ${t}` }, cache: 'no-store' })
  ...
  const gorunen = ortoKohortFiltre(satirlar || [], suzgec)

  const gonder = async () => {
    setDurum(''); setHata(''); setGonderiliyor(true)
    try {
      const t = await getAccessTokenAsync()
      const r = await fetch('/api/doktor/ortopedi/kohort', {
        method: 'POST',
      ...
      <div style={S.kutu}>
        ...
        {gorunen.map((s) => (
          <label key={s.patientId} style={{ ...S.metin, display: 'flex', gap: 8, padding: '6px 0', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <span style={S.kucuk}>{s.bayraklar.map((b) => ORTO_BAYRAK_AD[b]).join(' · ')}</span>
