...
import { bcStil, Istatistik, TaslakNotu, Rozet } from './BcAracKabugu'
import { BC_BAYRAK_AD, bcKohortFiltre, type BcKohortBayrak, type BcKohortSatir } from '../../engines/kohort'

const S = bcStil
const BAYRAKLAR = Object.keys(BC_BAYRAK_AD) as BcKohortBayrak[]

export default function BcKohortAraci() {
  const [satirlar, setSatirlar] = useState<BcKohortSatir[] | null>(null)
  const [toplam, setToplam] = useState(0)
  const [suzgec, setSuzgec] = useState<BcKohortBayrak[]>([])
  ...
  const yukle = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch('/api/doktor/beyin-cerrahisi/kohort', { headers: { Authorization: `Bearer ${t}` }, cache: 'no-store' })
  ...
  const gorunen = bcKohortFiltre(satirlar || [], suzgec)

  const gonder = async () => {
    setDurum(''); setHata(''); setGonderiliyor(true)
    try {
      const t = await getAccessTokenAsync()
      const r = await fetch('/api/doktor/beyin-cerrahisi/kohort', {
        method: 'POST',
  ...
  return (
    <>
      <div style={S.kutu}>
        ...
        {gorunen.map((s) => (
          <label key={s.patientId} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div>
              ...
              <div style={S.kucuk}>
                {s.bayraklar.map((b) => <Rozet key={b} ton={b === 'risk_acik' ? 'kirmizi' : 'uyari'}>{BC_BAYRAK_AD[b]}</Rozet>)}
        ...
        <TaslakNotu>TASLAK — hatırlatma tanı / AED doz taşımaz. Nöroloji Migren/İnme kohortu değildir.</TaslakNotu>
      </div>
    </>
  )
}
