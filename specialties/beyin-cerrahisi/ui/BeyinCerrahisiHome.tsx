...
export default function BeyinCerrahisiHome({ patientId }: { patientId: string }) {
  ...
  if (!v) return <div style={{ ...toolsCard, color: '#8FA0B5', fontSize: 12 }}>Beyin Cerrahisi yükleniyor…</div>
  ...
  return (
    <div style={toolsCard} data-chapter="beyin-cerrahisi">
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
        {SEKMELER.map((x) => (
          <button key={x} type="button" onClick={() => setSekme(x)} style={{ ...ghost, borderRadius: 999, background: sekme === x ? 'rgba(124,58,237,0.22)' : 'transparent', color: sekme === x ? '#EDE9FE' : '#8FA0B5' }}>{x}</button>
        ))}
      </div>
