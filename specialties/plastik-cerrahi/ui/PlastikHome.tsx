...
export default function PlastikHome({ patientId }: { patientId: string }) {
  ...
  if (!v) return <div style={{ ...toolsCard, color: CHROME_RENK.muted, fontSize: 12 }}>Plastik yükleniyor…</div>
  ...
  return (
    <div style={toolsCard} data-chapter="plastik-cerrahi">
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
        {SEKMELER.map((x) => (
          <button key={x} type="button" onClick={() => setSekme(x)} style={{ ...ghost, borderRadius: 999, background: sekme === x ? 'rgba(190,24,93,0.22)' : 'transparent', color: sekme === x ? '#FCE7F3' : CHROME_RENK.muted }}>{x}</button>
        ))}
      </div>
