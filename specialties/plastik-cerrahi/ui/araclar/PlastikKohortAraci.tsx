...
import { plastikStil, Istatistik, TaslakNotu, Rozet } from './PlastikAracKabugu'
import { PLASTIK_BAYRAK_AD, plastikKohortFiltre, type PlastikKohortBayrak, type PlastikKohortSatir } from '../../engines/kohort'

const S = plastikStil
const BAYRAKLAR = Object.keys(PLASTIK_BAYRAK_AD) as PlastikKohortBayrak[]

export default function PlastikKohortAraci() {
  const [satirlar, setSatirlar] = useState<PlastikKohortSatir[] | null>(null)
  const [toplam, setToplam] = useState(0)
  const [suzgec, setSuzgec] = useState<PlastikKohortBayrak[]>([])
  ...
  const yukle = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch('/api/doktor/plastik-cerrahi/kohort', { headers: { Authorization: `Bearer ${t}` }, cache: 'no-store' })
  ...
  const gorunen = plastikKohortFiltre(satirlar || [], suzgec)

  const gonder = async () => {
    setDurum(''); setHata(''); setGonderiliyor(true)
    try {
      const t = await getAccessTokenAsync()
      const r = await fetch('/api/doktor/plastik-cerrahi/kohort', {
        method: 'POST',
  ...
  return (
    <>
      <div style={S.kutu}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 10 }}>
          {BAYRAKLAR.map((b) => (
            <button key={b} type="button" onClick={() => setSuzgec((p) => (p.includes(b) ? p.filter((x) => x !== b) : [...p, b]))}
              style={{ ...S.ghost, background: suzgec.includes(b) ? 'rgba(190,24,93,0.25)' : 'transparent' }}>
              {PLASTIK_BAYRAK_AD[b]}
            </button>
          ))}
        </div>
        ...
        {gorunen.map((s) => (
          <label key={s.patientId} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div>
              ...
              <div style={S.kucuk}>
                {s.bayraklar.map((b) => <Rozet key={b} ton={b === 'risk_acik' ? 'kirmizi' : 'uyari'}>{PLASTIK_BAYRAK_AD[b]}</Rozet>)}
        ...
        <TaslakNotu>TASLAK — hatırlatma tanı / doz taşımaz. Foto ve yara bayrakları hekim görevinden gelir.</TaslakNotu>
      </div>
    </>
  )
}
