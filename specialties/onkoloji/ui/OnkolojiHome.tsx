...
export default function OnkolojiHome({ patientId }: { patientId: string }) {
  ...
  if (!v) return <div style={{ ...toolsCard, color: '#8FA0B5', fontSize: 12 }}>Onkoloji yükleniyor…</div>
  ...
  return (
    <div style={toolsCard} data-chapter="onkoloji">
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
        {SEKMELER.map((x) => (
          <button key={x} type="button" onClick={() => setSekme(x)} style={{ ...ghost, borderRadius: 999, background: sekme === x ? 'rgba(220,38,38,0.22)' : 'transparent', color: sekme === x ? '#FEE2E2' : '#8FA0B5' }}>{x}</button>
        ))}
      </div>
