'use client'
/**
 * ONKOLOJI-EXCEPTIONAL-01 — Araçlar › Onkoloji stüdyoları ortak kabuğu.
 * Yalnız onkoloji hekimi açar; başka branş (dahiliye dahil) /doktor-tools'a döner.
 */
...
export const ONKO_VURGU: AracVurgu = { ana: '#DC2626', anaMetin: '#FEF2F2', yumusak: '#FCA5A5', baslik: '#F87171' }
...
export function OnkoHastaSecici({ secili, sec }: { secili: string; sec: (id: string, ad: string) => void }) {
  return <HastaSecici secili={secili} sec={sec} />
}

export default function OnkoAracKabugu({ route, baslik, aciklama, children }: { route: string; baslik: string; aciklama: string; children: React.ReactNode }) {
  ...
  return (
    <AracVurguSaglayici vurgu={ONKO_VURGU}>
      <div style={{ ...toolsShell, overflowX: 'hidden' }}>
        <DoktorNav />
        <div style={{ maxWidth: 1000, margin: '0 auto', padding: '24px 16px 56px', boxSizing: 'border-box' }}>
          {!izin ? (
            <div style={{ color: '#9BB0C7', fontSize: 15, padding: '12px 0' }}>{izin === null ? 'Yükleniyor…' : 'Bu araç yalnızca onkoloji için.'}</div>
          ) : (
            <>
              <div style={{ marginBottom: 18 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: ONKO_VURGU.baslik, letterSpacing: '1.4px', textTransform: 'uppercase', marginBottom: 8 }}>Araçlar · Onkoloji</div>
