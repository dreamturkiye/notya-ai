...
export default function SporHekimligiHome({ patientId }: { patientId: string }) {
  ...
  if (!v) return <div style={{ ...toolsCard, color: '#8FA0B5', fontSize: 12 }}>Spor Hekimliği yükleniyor…</div>
  ...
  return (
    <div style={toolsCard} data-chapter="spor-hekimligi">
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
        {SEKMELER.map((x) => (
          <button key={x} type="button" onClick={() => setSekme(x)} style={{ ...ghost, borderRadius: 999, background: sekme === x ? 'rgba(180,83,9,0.22)' : 'transparent', color: sekme === x ? '#FCD34D' : '#8FA0B5' }}>{x}</button>
        ))}
      </div>
