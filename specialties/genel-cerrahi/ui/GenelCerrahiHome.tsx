...
export default function GenelCerrahiHome({ patientId }: { patientId: string }) {
  ...
  if (!v) return <div style={{ ...toolsCard, color: '#8FA0B5', fontSize: 12 }}>Genel Cerrahi yükleniyor…</div>
  ...
  return (
    <div style={toolsCard} data-chapter="genel-cerrahi">
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
        {SEKMELER.map((x) => (
          <button key={x} type="button" onClick={() => setSekme(x)} style={{ ...ghost, borderRadius: 999, background: sekme === x ? 'rgba(239,68,68,0.22)' : 'transparent', color: sekme === x ? '#FEE2E2' : '#8FA0B5' }}>{x}</button>
        ))}
      </div>
