/**
 * GENEL-CERRAHI-EXCEPTIONAL-01 — Yara / dren izlem. SAF fonksiyon.
 * Tarih + hekim alanları. Enfeksiyon tanısı / antibiyotik dozu YOK.
 */
...
export function yaraYasakIceriyorMu(metin: string): boolean {
  return /\d+\s*(mg|mcg)\b|antibiyotik dozu|tanı kilit|apse tanısı|yara enfeksiyonu tanısı/i.test(metin)
}
...
export function yaraSkorla(ham: unknown): YaraSonuc {
  ...
  const dipnot: Dipnot = { ref: 'TCD', not: 'Yara/dren izlem karar desteğidir; enfeksiyon tanısı hekimde' }
  if (!kart.tarih) {
    return { tamamMi: false, kart, ozet: 'İzlem tarihi gerekli.', dipnot }
  }
  if (kart.not && yaraYasakIceriyorMu(kart.not)) {
  ...
  return {
    tamamMi: true,
    kart,
    ozet: `${parcalar.join(' · ')}. Yara/dren izlem karar desteğidir; enfeksiyon tanısı ve antibiyotik dozu hekimde.`,
    dipnot,
  }
}
