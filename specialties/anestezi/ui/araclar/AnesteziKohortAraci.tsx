...
import { anesteziStil, Istatistik, TaslakNotu, Rozet } from './AnesteziAracKabugu'
import { ANESTEZI_BAYRAK_AD, anesteziKohortFiltre, type AnesteziKohortBayrak, type AnesteziKohortSatir } from '../../engines/kohort'

const S = anesteziStil
const BAYRAKLAR = Object.keys(ANESTEZI_BAYRAK_AD) as AnesteziKohortBayrak[]

export default function AnesteziKohortAraci() {
  const [satirlar, setSatirlar] = useState<AnesteziKohortSatir[] | null>(null)
  const [toplam, setToplam] = useState(0)
  const [suzgec, setSuzgec] = useState<AnesteziKohortBayrak[]>([])
  ...
  const yukle = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch('/api/doktor/anestezi/kohort', { headers: { Authorization: `Bearer ${t}` }, cache: 'no-store' })
  ...
  const gorunen = anesteziKohortFiltre(satirlar || [], suzgec)

  const gonder = async () => {
    setDurum(''); setHata(''); setGonderiliyor(true)
    try {
      const t = await getAccessTokenAsync()
      const r = await fetch('/api/doktor/anestezi/kohort', {
        method: 'POST',
    ...
    <>
      <div style={S.kutu}>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 10 }}>
          ...
          <Istatistik deger={satirlar == null ? '…' : gorunen.filter((s) => s.bayraklar.includes('alerji_ilac')).length} etiket="Alerji/ilaç" ton="uyari" />
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 10 }}>
          {BAYRAKLAR.map((b) => (
            <button key={b} type="button" onClick={() => setSuzgec((p) => (p.includes(b) ? p.filter((x) => x !== b) : [...p, b]))}
              style={{ ...S.ghost, background: suzgec.includes(b) ? 'rgba(139,92,246,0.25)' : 'transparent' }}>
              {ANESTEZI_BAYRAK_AD[b]}
            </button>
          ))}
        </div>
        ...
        {gorunen.map((s) => (
          <label key={s.patientId} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div>
              ...
              <div style={S.kucuk}>
                {s.bayraklar.map((b) => <Rozet key={b} ton={b === 'risk_acik' ? 'kirmizi' : 'uyari'}>{ANESTEZI_BAYRAK_AD[b]}</Rozet>)}
        ...
        <TaslakNotu>TASLAK — hatırlatma tanı / ilaç dozu taşımaz. Genel cerrahi pre-op kohortu değildir.</TaslakNotu>
      </div>
    </>
  )
}
