...
const ADIMLAR = ['serit', 'olcum', 'olcum_nota', 'kopya', 'glokom', 'dr', 'dr_sevk_kapat', 'enjeksiyon', 'sgk_kapi', 'sgkrapor', 'sgkrapor_kilit', 'katarakt', 'goruntu_okuma', 'kontrol', 'pediatrik', 'onsegment', 'acil', 'intake_nota', 'gorev']
...
describe('(a) göz prompts exist and are complete', () => {
  ...
  it('every tools.ts step exists in the göz API', { skip: !fs.existsSync(apiYolu) && 'app/api/doktor/goz/route.ts henüz yok' }, () => {
    const api = kaynak('app/api/doktor/goz/route.ts')
