/**
 * Zod-shaped validators living inside this specialty folder.
 *
 * The npm `zod` package is not in package.json. Adding it would edit a shared file
 * (package.json / package-lock.json), which this sandbox forbids. Call sites use the
 * same `z.object / z.enum / safeParse` API (copied per chapter — no cross-chapter import, no-bleed).
 * is possible when the shell allows that shared change.
 */
