...
export function Secim({ deger, set, secenekler, bos, etiket }: { deger: string; set: (x: string) => void; secenekler: Array<[string, string]>; bos?: string; etiket?: string }) {
  return (
    <select aria-label={etiket} value={deger} onChange={(e) => set(e.target.value)} style={{ ...gozStil.input, width: 'auto', minWidth: 120, minHeight: 44 }}>
...
export function GozHastaSecici({ secili, sec }: { secili: string; sec: (id: string, ad: string) => void }) {
  ...
  return (
    <div>
      <div style={gozStil.satir}>
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Hasta ara (isteğe bağlı)" aria-label="Hasta ara" style={{ ...gozStil.input, flex: '1 1 200px', width: 'auto' }} />
        <select aria-label="Hasta seç" value={secili} onChange={(e) => { const h = (liste || []).find((x) => x.id === e.target.value); sec(e.target.value, h?.label || ''); }} style={{ ...gozStil.input, flex: '1 1 220px', width: 'auto', minHeight: 44 }}>
      ...
      {hata && <div style={{ ...gozStil.hata, marginTop: 6 }}>{hata}</div>}
    </div>
  );
}
        ...
        {!izin ? (
          ...
          <>
            <div style={{ marginBottom: 18 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#14B8A6', letterSpacing: '1.4px', textTransform: 'uppercase', marginBottom: 8 }}>Araçlar · Göz</div>
