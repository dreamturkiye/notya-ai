...
import { ftrStil, Istatistik, TaslakNotu, Rozet } from './FtrAracKabugu'
import { FTR_BAYRAK_AD, ftrKohortFiltre, type FtrKohortBayrak, type FtrKohortSatir } from '../../engines/kohort'

const S = ftrStil
const BAYRAKLAR = Object.keys(FTR_BAYRAK_AD) as FtrKohortBayrak[]

export default function FtrKohortAraci() {
  const [satirlar, setSatirlar] = useState<FtrKohortSatir[] | null>(null)
  const [toplam, setToplam] = useState(0)
  const [suzgec, setSuzgec] = useState<FtrKohortBayrak[]>([])
  ...
  const yukle = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch('/api/doktor/fizik-tedavi/kohort', { headers: { Authorization: `Bearer ${t}` }, cache: 'no-store' })
  ...
  const gorunen = ftrKohortFiltre(satirlar || [], suzgec)

  const gonder = async () => {
    setDurum(''); setHata(''); setGonderiliyor(true)
    try {
      const t = await getAccessTokenAsync()
      const r = await fetch('/api/doktor/fizik-tedavi/kohort', {
        method: 'POST',
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 10 }}>
          {BAYRAKLAR.map((b) => (
            <button key={b} type="button" onClick={() => setSuzgec((p) => (p.includes(b) ? p.filter((x) => x !== b) : [...p, b]))}
              style={{ ...S.ghost, background: suzgec.includes(b) ? 'rgba(22,163,74,0.25)' : 'transparent' }}>
              {FTR_BAYRAK_AD[b]}
            </button>
          ))}
        </div>
        ...
        {gorunen.map((s) => (
          <label key={s.patientId} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div>
              ...
              <div style={S.kucuk}>
                {s.bayraklar.map((b) => <Rozet key={b} ton={b === 'risk_acik' ? 'kirmizi' : 'uyari'}>{FTR_BAYRAK_AD[b]}</Rozet>)}
