...
export default function FtrHome({ patientId }: { patientId: string }) {
  ...
  if (!v) return <div style={{ ...toolsCard, color: CHROME_RENK.muted, fontSize: 12 }}>FTR yükleniyor…</div>
  ...
  return (
    <div style={toolsCard} data-chapter="fizik-tedavi">
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
        {SEKMELER.map((x) => (
          <button key={x} type="button" onClick={() => setSekme(x)} style={{ ...ghost, borderRadius: 999, background: sekme === x ? 'rgba(22,163,74,0.22)' : 'transparent', color: sekme === x ? '#BBF7D0' : CHROME_RENK.muted }}>{x}</button>
        ))}
      </div>
