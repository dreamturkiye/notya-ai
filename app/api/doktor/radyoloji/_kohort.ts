/**
 * RADYOLOJI-EXCEPTIONAL-01 — kohort verisi + hasta-güvenli hatırlatma.
 */
...
import {
  radyoKohortSatirlari, radyoRecallMesaji, type RadyoKohortBayrak, type RadyoKohortGirdi,
} from '@/specialties/radyoloji/engines/kohort'

export type Sb = Awaited<ReturnType<typeof doktorOturum>> extends infer T ? (T extends { supabase: infer S } ? S : never) : never

const KAYNAK_TABLOLARI = ['hasta_radyoloji', 'radyo_kuyruk', 'radyo_rapor', 'radyo_kritik', 'radyo_acil', 'radyo_gorevleri'] as const

export async function radyoKohortVerisi(sb: Sb, doctorId: string, bugun: string, sadece?: string[]) {
  ...
  const [pQ, bQ, rQ, gQ, kQ, sQ, tQ] = await Promise.all([
    sb.from('patients').select('id, name_encrypted').eq('doctor_id', doctorId).in('id', ids),
    sb.from('hasta_radyoloji').select('patient_id, next_kontrol, kuyruk, notes').eq('doctor_id', doctorId).in('patient_id', ids),
    sb.from('radyo_acil').select('patient_id, bayraklar, hekim_onay, tarih').eq('doctor_id', doctorId).in('patient_id', ids).order('tarih', { ascending: false }).limit(3000),
    sb.from('radyo_gorevleri').select('patient_id, kod, due').eq('doctor_id', doctorId).eq('durum', 'acik').in('patient_id', ids).limit(5000),
    sb.from('radyo_kritik').select('patient_id, bayraklar, hekim_onay, tarih').eq('doctor_id', doctorId).in('patient_id', ids).order('tarih', { ascending: false }).limit(3000),
  ...
  const risk = grup(rQ.data), gorev = grup(gQ.data), kritik = grup(kQ.data)
  const portal = new Set((tQ.data || []).map((r) => String(r.patient_id)))

  const girdi: RadyoKohortGirdi[] = ids.filter((id) => ad.has(id)).map((id) => {
    const b = bolum.get(id)
    const notes = (b?.notes && typeof b.notes === 'object' ? b.notes : {}) as { belge?: { sonraki?: string } }
    const kuyruk = (b?.kuyruk && typeof b.kuyruk === 'object' ? b.kuyruk : null) as { durum?: string } | null
    const acikKritik = [
      ...(risk.get(id) || []).filter((r) => !r.hekim_onay).flatMap((r) => ((r.bayraklar as string[] | null) || []).map(String)),
      ...(kritik.get(id) || []).filter((r) => !r.hekim_onay).flatMap((r) => ((r.bayraklar as string[] | null) || []).map(String)),
    ]
    return {
      patientId: id,
      ad: ad.get(id)!,
      acikKritikBayraklari: acikKritik,
      ...
      belgeBekliyor: !!(notes.belge?.sonraki && notes.belge.sonraki < bugun),
      kuyrukAktif: !!(kuyruk && kuyruk.durum && kuyruk.durum !== 'arsiv' && kuyruk.durum !== 'rapor_hazir'),
    }
  })
  return { satirlar: radyoKohortSatirlari(girdi, bugun), toplamHasta: girdi.length }
}

export async function radyoHatirlatmaGonder(
  sb: Sb, doctorId: string, patientId: string, bayraklar: RadyoKohortBayrak[], bugun: string,
): Promise<'gonderildi' | 'yakin' | 'hata'> {
  const m = radyoRecallMesaji(bayraklar)
  ...
  const { data: acik } = await sb.from('radyo_gorevleri').select('id')
    .eq('doctor_id', doctorId).eq('patient_id', patientId).eq('kod', 'hatirlatma_takip').eq('durum', 'acik').maybeSingle()
  if (!acik) await sb.from('radyo_gorevleri').insert({ patient_id: patientId, doctor_id: doctorId, kod: 'hatirlatma_takip', ad: 'Hatırlatma gönderildi — randevu dönüşünü takip edin', due, kaynak: 'hatirlatma' })
  return 'gonderildi'
}
