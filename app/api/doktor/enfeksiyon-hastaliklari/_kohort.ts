/**
 * ENFEKSIYON-EXCEPTIONAL-01 — kohort verisi + hasta-güvenli hatırlatma.
 */
...
import {
  enfKohortSatirlari, enfRecallMesaji, type EnfKohortBayrak, type EnfKohortGirdi,
} from '@/specialties/enfeksiyon-hastaliklari/engines/kohort'

export type Sb = Awaited<ReturnType<typeof doktorOturum>> extends infer T ? (T extends { supabase: infer S } ? S : never) : never

const KAYNAK_TABLOLARI = ['hasta_enfeksiyon', 'enfeksiyon_izolasyon', 'enfeksiyon_atb', 'enfeksiyon_viral', 'enfeksiyon_acil', 'enfeksiyon_gorevleri'] as const

export async function enfKohortVerisi(sb: Sb, doctorId: string, bugun: string, sadece?: string[]) {
  ...
  const [pQ, bQ, rQ, gQ, sQ, tQ] = await Promise.all([
    ...
    sb.from('hasta_enfeksiyon').select('patient_id, next_kontrol').eq('doctor_id', doctorId).in('patient_id', ids),
    sb.from('enfeksiyon_acil').select('patient_id, bayraklar, hekim_onay, tarih').eq('doctor_id', doctorId).in('patient_id', ids).order('tarih', { ascending: false }).limit(3000),
    sb.from('enfeksiyon_gorevleri').select('patient_id, kod, due').eq('doctor_id', doctorId).eq('durum', 'acik').in('patient_id', ids).limit(5000),
  ...
  const girdi: EnfKohortGirdi[] = ids.filter((id) => ad.has(id)).map((id) => ({
    patientId: id,
    ad: ad.get(id)!,
  ...
  return { satirlar: enfKohortSatirlari(girdi, bugun), toplamHasta: girdi.length }
}

export async function enfHatirlatmaGonder(
  sb: Sb, doctorId: string, patientId: string, bayraklar: EnfKohortBayrak[], bugun: string,
): Promise<'gonderildi' | 'yakin' | 'hata'> {
  const m = enfRecallMesaji(bayraklar)
  ...
  const { data: acik } = await sb.from('enfeksiyon_gorevleri').select('id')
    .eq('doctor_id', doctorId).eq('patient_id', patientId).eq('kod', 'hatirlatma_takip').eq('durum', 'acik').maybeSingle()
  if (!acik) await sb.from('enfeksiyon_gorevleri').insert({ patient_id: patientId, doctor_id: doctorId, kod: 'hatirlatma_takip', ad: 'Hatırlatma gönderildi — randevu dönüşünü takip edin', due, kaynak: 'hatirlatma' })
  return 'gonderildi'
}
