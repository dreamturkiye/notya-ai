/**
 * ENFEKSIYON-EXCEPTIONAL-01 — Kohort GET/POST.
 */
import { NextRequest, NextResponse } from 'next/server'
import { doktorOturum } from '@/lib/doktor/serverAuth'
import { enfKohortVerisi, enfHatirlatmaGonder } from '../_kohort'
import type { EnfKohortBayrak } from '@/specialties/enfeksiyon-hastaliklari/engines/kohort'
...
export async function GET(req: NextRequest) {
  ...
  const { satirlar, toplamHasta } = await enfKohortVerisi(sb, user.id, bugun)
...
export async function POST(req: NextRequest) {
  ...
  const bugun = new Date().toISOString().slice(0, 10)
  const { satirlar } = await enfKohortVerisi(sb, user.id, bugun, ids)
  const izinli = new Map(satirlar.map((s) => [s.patientId, s.bayraklar as EnfKohortBayrak[]]))
  let gonderilen = 0, atlanan = 0
  for (const id of ids) {
    const bayraklar = izinli.get(id)
    if (!bayraklar?.length) { atlanan++; continue }
    const r = await enfHatirlatmaGonder(sb, user.id, id, bayraklar, bugun)
