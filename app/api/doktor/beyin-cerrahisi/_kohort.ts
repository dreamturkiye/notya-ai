/**
 * BEYIN-CERRAHISI-EXCEPTIONAL-01 — kohort verisi + hasta-güvenli hatırlatma.
 */
...
import {
  bcKohortSatirlari, bcRecallMesaji, type BcKohortBayrak, type BcKohortGirdi,
} from '@/specialties/beyin-cerrahisi/engines/kohort'

export type Sb = Awaited<ReturnType<typeof doktorOturum>> extends infer T ? (T extends { supabase: infer S } ? S : never) : never

const KAYNAK_TABLOLARI = ['hasta_beyin_cerrahisi', 'bc_postop', 'bc_bilinc', 'bc_acil', 'bc_gorevleri'] as const

export async function bcKohortVerisi(sb: Sb, doctorId: string, bugun: string, sadece?: string[]) {
  ...
  const [pQ, bQ, rQ, gQ, sQ, tQ] = await Promise.all([
    ...
    sb.from('hasta_beyin_cerrahisi').select('patient_id, next_kontrol, notes').eq('doctor_id', doctorId).in('patient_id', ids),
    sb.from('bc_acil').select('patient_id, bayraklar, hekim_onay, tarih').eq('doctor_id', doctorId).in('patient_id', ids).order('tarih', { ascending: false }).limit(3000),
    sb.from('bc_gorevleri').select('patient_id, kod, due').eq('doctor_id', doctorId).eq('durum', 'acik').in('patient_id', ids).limit(5000),
  ...
  const girdi: BcKohortGirdi[] = ids.filter((id) => ad.has(id)).map((id) => {
  ...
  return { satirlar: bcKohortSatirlari(girdi, bugun), toplamHasta: girdi.length }
}

export async function bcHatirlatmaGonder(
  sb: Sb, doctorId: string, patientId: string, bayraklar: BcKohortBayrak[], bugun: string,
): Promise<'gonderildi' | 'yakin' | 'hata'> {
  const m = bcRecallMesaji(bayraklar)
  ...
  const { data: acik } = await sb.from('bc_gorevleri').select('id')
    .eq('doctor_id', doctorId).eq('patient_id', patientId).eq('kod', 'hatirlatma_takip').eq('durum', 'acik').maybeSingle()
  if (!acik) await sb.from('bc_gorevleri').insert({ patient_id: patientId, doctor_id: doctorId, kod: 'hatirlatma_takip', ad: 'Hatırlatma gönderildi — randevu dönüşünü takip edin', due, kaynak: 'hatirlatma' })
  return 'gonderildi'
}
