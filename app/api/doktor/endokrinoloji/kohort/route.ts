/**
 * ENDOKRINOLOJI-EXCEPTIONAL-01 — Kohort GET/POST.
 */
...
export async function GET(req: NextRequest) {
  ...
  const { satirlar, toplamHasta } = await endoKohortVerisi(sb, user.id, bugun)
...
export async function POST(req: NextRequest) {
  ...
  const bugun = new Date().toISOString().slice(0, 10)
  const { satirlar } = await endoKohortVerisi(sb, user.id, bugun, ids)
  const izinli = new Map(satirlar.map((s) => [s.patientId, s.bayraklar as EndoKohortBayrak[]]))
  let gonderilen = 0, atlanan = 0
  for (const id of ids) {
    const bayraklar = izinli.get(id)
    if (!bayraklar?.length) { atlanan++; continue }
    const r = await endoHatirlatmaGonder(sb, user.id, id, bayraklar, bugun)
