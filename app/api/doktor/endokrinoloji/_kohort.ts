/**
 * ENDOKRINOLOJI-EXCEPTIONAL-01 — kohort verisi + hasta-güvenli hatırlatma.
 */
...
const KAYNAK_TABLOLARI = ['hasta_endokrinoloji', 'endo_lab', 'endo_acil', 'endo_gorevleri'] as const

export async function endoKohortVerisi(sb: Sb, doctorId: string, bugun: string, sadece?: string[]) {
  ...
  const [pQ, bQ, lQ, rQ, gQ, sQ, tQ] = await Promise.all([
    sb.from('patients').select('id, name_encrypted').eq('doctor_id', doctorId).in('id', ids),
    sb.from('hasta_endokrinoloji').select('patient_id, next_kontrol').eq('doctor_id', doctorId).in('patient_id', ids),
  ...
  const bolum = ilk(bQ.data), lab = ilk(lQ.data), sonVizit = ilk(sQ.data)
  const risk = grup(rQ.data), gorev = grup(gQ.data)
  const portal = new Set((tQ.data || []).map((r) => String(r.patient_id)))

  const girdi: EndoKohortGirdi[] = ids.filter((id) => ad.has(id)).map((id) => {
    const hb = lab.get(id)
    const bant = hb ? labSkorla('hba1c', hb.deger == null ? null : Number(hb.deger)).bant : null
    return {
      patientId: id,
      ad: ad.get(id)!,
      sonHba1cBant: bant,
  ...
  return { satirlar: endoKohortSatirlari(girdi, bugun), toplamHasta: girdi.length }
}

export async function endoHatirlatmaGonder(
  sb: Sb, doctorId: string, patientId: string, bayraklar: EndoKohortBayrak[], bugun: string,
): Promise<'gonderildi' | 'yakin' | 'hata'> {
  const m = endoRecallMesaji(bayraklar)
  ...
  const { data: acik } = await sb.from('endo_gorevleri').select('id')
    .eq('doctor_id', doctorId).eq('patient_id', patientId).eq('kod', 'hatirlatma_takip').eq('durum', 'acik').maybeSingle()
  if (!acik) await sb.from('endo_gorevleri').insert({ patient_id: patientId, doctor_id: doctorId, kod: 'hatirlatma_takip', ad: 'Hatırlatma gönderildi — randevu dönüşünü takip edin', due, kaynak: 'hatirlatma' })
  return 'gonderildi'
}
