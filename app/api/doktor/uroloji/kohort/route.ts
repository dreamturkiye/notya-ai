/**
 * UROLOJI-EXCEPTIONAL-01 — Araçlar › Üroloji kohort paneli API.
 */
import { NextRequest, NextResponse } from 'next/server'
import { doktorOturum } from '@/lib/doktor/serverAuth'
import { uroKohortVerisi, uroHatirlatmaGonder } from '../_kohort'
...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json(await uroKohortVerisi(oturum.supabase, oturum.user.id, bugun()))
}
...
export async function POST(req: NextRequest) {
  ...
  const T = bugun()
  const { satirlar } = await uroKohortVerisi(sb, user.id, T, ids)
  let gonderilen = 0, atlanan = 0
  for (const s of satirlar) {
    const r = await uroHatirlatmaGonder(sb, user.id, s.patientId, s.bayraklar, T)
