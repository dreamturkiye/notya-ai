/**
 * UROLOJI-EXCEPTIONAL-01 — Üroloji kohort verisi + hasta-güvenli hatırlatma.
 */
...
import {
  uroKohortSatirlari,
  uroRecallMesaji,
  type UroKohortBayrak,
  type UroKohortGirdi,
} from '@/specialties/uroloji/engines/kohort'

export type Sb = Awaited<ReturnType<typeof doktorOturum>> extends infer T ? (T extends { supabase: infer S } ? S : never) : never

const KAYNAK_TABLOLARI = ['hasta_uro', 'uro_ipss', 'uro_psa', 'uro_risk', 'uro_gorevleri'] as const

export async function uroKohortVerisi(sb: Sb, doctorId: string, bugun: string, sadece?: string[]) {
  ...
  const [pQ, bQ, iQ, psaQ, rQ, gQ, sQ, tQ] = await Promise.all([
    sb.from('patients').select('id, name_encrypted').eq('doctor_id', doctorId).in('id', ids),
    sb.from('hasta_uro').select('patient_id, next_kontrol').eq('doctor_id', doctorId).in('patient_id', ids),
    sb.from('uro_ipss').select('patient_id, bant, tarih').eq('doctor_id', doctorId).in('patient_id', ids).order('tarih', { ascending: false }).limit(3000),
    sb.from('uro_psa').select('patient_id, tarih').eq('doctor_id', doctorId).in('patient_id', ids).order('tarih', { ascending: false }).limit(3000),
    sb.from('uro_risk').select('patient_id, bayraklar, hekim_onay, tarih').eq('doctor_id', doctorId).in('patient_id', ids).order('tarih', { ascending: false }).limit(3000),
    sb.from('uro_gorevleri').select('patient_id, kod, due').eq('doctor_id', doctorId).eq('durum', 'acik').in('patient_id', ids).limit(5000),
  ...
  const bolum = ilk(bQ.data), ipss = ilk(iQ.data), psa = ilk(psaQ.data), sonVizit = ilk(sQ.data)
  const risk = grup(rQ.data), gorev = grup(gQ.data)
  const portal = new Set((tQ.data || []).map((r) => String(r.patient_id)))

  const girdi: UroKohortGirdi[] = ids.filter((id) => ad.has(id)).map((id) => ({
    patientId: id,
    ad: ad.get(id)!,
    sonPsaTarihi: psa.get(id)?.tarih ? String(psa.get(id)!.tarih).slice(0, 10) : null,
    sonIpssBant: ipss.get(id)?.bant ? String(ipss.get(id)!.bant) : null,
  ...
  return { satirlar: uroKohortSatirlari(girdi, bugun), toplamHasta: girdi.length }
}

export async function uroHatirlatmaGonder(
  sb: Sb,
  doctorId: string,
  patientId: string,
  ...
  const m = uroRecallMesaji(bayraklar)
  ...
  const { data: acik } = await sb.from('uro_gorevleri').select('id')
    .eq('doctor_id', doctorId).eq('patient_id', patientId).eq('kod', 'hatirlatma_takip').eq('durum', 'acik').maybeSingle()
  if (!acik) await sb.from('uro_gorevleri').insert({ patient_id: patientId, doctor_id: doctorId, kod: 'hatirlatma_takip', ad: 'Hatırlatma gönderildi — randevu dönüşünü takip edin', due, kaynak: 'hatirlatma' })
  return 'gonderildi'
}
