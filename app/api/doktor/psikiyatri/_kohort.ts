/**
 * PSIK-EXCEPTIONAL-01 — psikiyatri kohort verisi + hasta-güvenli hatırlatma (route.ts yalnız handler export edebilir).
 * Göz/derm kohort deseniyle aynı: hasta kimlikleri YALNIZ doctor_id = hekim olan psik_* satırlarından türer,
 * ad yalnız kimliği doğrulanmış hekime çözülür (HASTA-İZOLASYON).
 *
 * Ruh sağlığı verisi en hassas sınıftır: hatırlatma metni tanı, ölçek adı, skor veya ilaç adı TAŞIMAZ
 * (specialties/psikiyatri/engines/kohort.ts · psikRecallMesaji).
 */
...
import {
  psikKohortSatirlari,
  psikRecallMesaji,
  type PsikKohortBayrak,
  type PsikKohortGirdi,
} from '@/specialties/psikiyatri/engines/kohort'

export type Sb = Awaited<ReturnType<typeof doktorOturum>> extends infer T ? (T extends { supabase: infer S } ? S : never) : never

const KAYNAK_TABLOLARI = ['hasta_psik', 'psik_olcek', 'psik_risk', 'psik_gorevleri'] as const

export async function psikKohortVerisi(sb: Sb, doctorId: string, bugun: string, sadece?: string[]) {
  ...
  const [pQ, bQ, oQ, rQ, gQ, sQ, tQ] = await Promise.all([
    ...
    sb.from('hasta_psik').select('patient_id, next_kontrol').eq('doctor_id', doctorId).in('patient_id', ids),
    sb.from('psik_olcek').select('patient_id, tip, skor, maddeler, tarih').eq('doctor_id', doctorId).eq('tip', 'phq9').in('patient_id', ids).order('tarih', { ascending: false }).limit(3000),
    sb.from('psik_risk').select('patient_id, bayraklar, hekim_onay, tarih').eq('doctor_id', doctorId).in('patient_id', ids).order('tarih', { ascending: false }).limit(3000),
    sb.from('psik_gorevleri').select('patient_id, kod, due').eq('doctor_id', doctorId).eq('durum', 'acik').in('patient_id', ids).limit(5000),
  ...
  const bolum = ilk(bQ.data), phq = ilk(oQ.data), sonVizit = ilk(sQ.data)
  const risk = grup(rQ.data), gorev = grup(gQ.data)
  const portal = new Set((tQ.data || []).map((r) => String(r.patient_id)))

  const girdi: PsikKohortGirdi[] = ids.filter((id) => ad.has(id)).map((id) => {
    const p = phq.get(id)
    return {
      patientId: id,
      ad: ad.get(id)!,
      phq9: p ? Number(p.skor) : null,
      phq9Madde9: !!(p?.maddeler as { ozkiyim_madde9?: boolean } | null)?.ozkiyim_madde9,
  ...
  return { satirlar: psikKohortSatirlari(girdi, bugun), toplamHasta: girdi.length }
}

/**
 * Tek hastaya hatırlatma. Çağıran, patientId'nin hekime ait olduğunu ÖNCE doğrulamış olmalı (kohort filtresi).
 * 7 gün içinde aynı konu gönderildiyse atlar; gönderince dönüş takibi için açık görev bırakır.
 */
export async function psikHatirlatmaGonder(
  sb: Sb,
  doctorId: string,
  patientId: string,
  ...
  const m = psikRecallMesaji(bayraklar)
  ...
  const { data: acik } = await sb.from('psik_gorevleri').select('id')
    .eq('doctor_id', doctorId).eq('patient_id', patientId).eq('kod', 'hatirlatma_takip').eq('durum', 'acik').maybeSingle()
  if (!acik) await sb.from('psik_gorevleri').insert({ patient_id: patientId, doctor_id: doctorId, kod: 'hatirlatma_takip', ad: 'Hatırlatma gönderildi — randevu dönüşünü takip edin', due, kaynak: 'hatirlatma' })
  return 'gonderildi'
}
