/**
 * KARDIO-EXCEPTIONAL-01 — Kardiyoloji kohort verisi + hasta-güvenli hatırlatma.
 */
...
import {
  kardioKohortSatirlari,
  kardioRecallMesaji,
  type KardioKohortBayrak,
  type KardioKohortGirdi,
} from '@/specialties/kardiyoloji/engines/kohort'

export type Sb = Awaited<ReturnType<typeof doktorOturum>> extends infer T ? (T extends { supabase: infer S } ? S : never) : never

const KAYNAK_TABLOLARI = ['hasta_kardiyoloji', 'kardio_score2', 'kardio_izlem', 'kardio_risk', 'kardio_gorevleri'] as const

export async function kardioKohortVerisi(sb: Sb, doctorId: string, bugun: string, sadece?: string[]) {
  ...
  const [pQ, bQ, sQ, rQ, gQ, sessQ, tQ] = await Promise.all([
    sb.from('patients').select('id, name_encrypted').eq('doctor_id', doctorId).in('id', ids),
    sb.from('hasta_kardiyoloji').select('patient_id, next_kontrol').eq('doctor_id', doctorId).in('patient_id', ids),
    sb.from('kardio_score2').select('patient_id, risk_pct, maddeler, tarih').eq('doctor_id', doctorId).in('patient_id', ids).order('tarih', { ascending: false }).limit(3000),
    sb.from('kardio_risk').select('patient_id, bayraklar, hekim_onay, tarih').eq('doctor_id', doctorId).in('patient_id', ids).order('tarih', { ascending: false }).limit(3000),
    sb.from('kardio_gorevleri').select('patient_id, kod, due').eq('doctor_id', doctorId).eq('durum', 'acik').in('patient_id', ids).limit(5000),
  ...
  const bolum = ilk(bQ.data), score = ilk(sQ.data), sonVizit = ilk(sessQ.data)
  const risk = grup(rQ.data), gorev = grup(gQ.data)
  const portal = new Set((tQ.data || []).map((r) => String(r.patient_id)))

  const girdi: KardioKohortGirdi[] = ids.filter((id) => ad.has(id)).map((id) => {
    const sc = score.get(id) as { risk_pct?: number | null; maddeler?: { kova?: string } } | undefined
    return {
      patientId: id,
      ad: ad.get(id)!,
      sonScore2Pct: sc?.risk_pct == null ? null : Number(sc.risk_pct),
      sonScore2Kova: sc?.maddeler?.kova || null,
  ...
  return { satirlar: kardioKohortSatirlari(girdi, bugun), toplamHasta: girdi.length }
}

export async function kardioHatirlatmaGonder(
  sb: Sb,
  doctorId: string,
  patientId: string,
  ...
  const m = kardioRecallMesaji(bayraklar)
  ...
  await sb.from('kardio_gorevleri').insert({
    patient_id: patientId, doctor_id: doctorId, kod: 'hatirlatma_donus', ad: 'Hatırlatma dönüş takibi', due: bugun, kaynak: 'kohort',
  })
  return 'gonderildi'
}
