/**
 * GENEL-CERRAHI-EXCEPTIONAL-01 — kohort verisi + hasta-güvenli hatırlatma.
 */
...
import {
  gcKohortSatirlari, gcRecallMesaji, type GcKohortBayrak, type GcKohortGirdi,
} from '@/specialties/genel-cerrahi/engines/kohort'
import { PREOP_MADDELER } from '@/specialties/genel-cerrahi/engines/preop'

export type Sb = Awaited<ReturnType<typeof doktorOturum>> extends infer T ? (T extends { supabase: infer S } ? S : never) : never

const KAYNAK_TABLOLARI = ['hasta_genel_cerrahi', 'gc_preop', 'gc_yara_dren', 'gc_patoloji', 'gc_acil', 'gc_gorevleri'] as const
...
export async function gcKohortVerisi(sb: Sb, doctorId: string, bugun: string, sadece?: string[]) {
  ...
  const [pQ, bQ, rQ, gQ, sQ, tQ] = await Promise.all([
    ...
    sb.from('hasta_genel_cerrahi').select('patient_id, next_kontrol, preop, patoloji').eq('doctor_id', doctorId).in('patient_id', ids),
  ...
  const girdi: GcKohortGirdi[] = ids.filter((id) => ad.has(id)).map((id) => {
    const b = bolum.get(id)
    const preop = (b?.preop && typeof b.preop === 'object' ? b.preop : null) as { tamamlanan?: string[]; ameliyatTarihi?: string } | null
    const patoloji = (b?.patoloji && typeof b.patoloji === 'object' ? b.patoloji : null) as { durum?: string } | null
    const tamam = Array.isArray(preop?.tamamlanan) ? preop!.tamamlanan!.length : 0
    return {
      patientId: id,
      ad: ad.get(id)!,
      ...
      preopEksik: !!preop && tamam < PREOP_MADDELER.length,
      ...
      patolojiBekliyor: patoloji?.durum === 'bekleniyor',
    }
  })
  return { satirlar: gcKohortSatirlari(girdi, bugun), toplamHasta: girdi.length }
}

export async function gcHatirlatmaGonder(
  sb: Sb, doctorId: string, patientId: string, bayraklar: GcKohortBayrak[], bugun: string,
): Promise<'gonderildi' | 'yakin' | 'hata'> {
  const m = gcRecallMesaji(bayraklar)
