/**
 * KBB-EXCEPTIONAL-01 — KBB kohort verisi + hasta-güvenli hatırlatma (route.ts yalnız handler export edebilir).
 * Psikiyatri / göz / derm kohort deseniyle aynı: hasta kimlikleri YALNIZ doctor_id = hekim olan kbb_* satırlarından
 * türer, ad yalnız kimliği doğrulanmış hekime çözülür (HASTA-İZOLASYON).
 *
 * Hatırlatma metni tanı, dB değeri, kayıp bandı veya ilaç adı TAŞIMAZ
 * (specialties/kulak-burun-bogaz/engines/kohort.ts · kbbRecallMesaji).
 */
...
import {
  kbbKohortSatirlari,
  kbbRecallMesaji,
  type KbbKohortBayrak,
  type KbbKohortGirdi,
} from '@/specialties/kulak-burun-bogaz/engines/kohort'

export type Sb = Awaited<ReturnType<typeof doktorOturum>> extends infer T ? (T extends { supabase: infer S } ? S : never) : never

const KAYNAK_TABLOLARI = ['hasta_kbb', 'kbb_odyometri', 'kbb_risk', 'kbb_gorevleri'] as const

export async function kbbKohortVerisi(sb: Sb, doctorId: string, bugun: string, sadece?: string[]) {
  ...
  const [pQ, bQ, oQ, rQ, gQ, sQ, tQ] = await Promise.all([
    ...
    sb.from('hasta_kbb').select('patient_id, next_kontrol').eq('doctor_id', doctorId).in('patient_id', ids),
    sb.from('kbb_odyometri').select('patient_id, tarih').eq('doctor_id', doctorId).in('patient_id', ids).order('tarih', { ascending: false }).limit(3000),
    sb.from('kbb_risk').select('patient_id, bayraklar, hekim_onay, tarih').eq('doctor_id', doctorId).in('patient_id', ids).order('tarih', { ascending: false }).limit(3000),
    sb.from('kbb_gorevleri').select('patient_id, kod, due').eq('doctor_id', doctorId).eq('durum', 'acik').in('patient_id', ids).limit(5000),
  ...
  const bolum = ilk(bQ.data), odyo = ilk(oQ.data), sonVizit = ilk(sQ.data)
  const risk = grup(rQ.data), gorev = grup(gQ.data)
  const portal = new Set((tQ.data || []).map((r) => String(r.patient_id)))

  const girdi: KbbKohortGirdi[] = ids.filter((id) => ad.has(id)).map((id) => ({
    patientId: id,
    ad: ad.get(id)!,
    sonOdyoTarihi: odyo.get(id)?.tarih ? String(odyo.get(id)!.tarih).slice(0, 10) : null,
  ...
  return { satirlar: kbbKohortSatirlari(girdi, bugun), toplamHasta: girdi.length }
}
 ...
 */
export async function kbbHatirlatmaGonder(
  sb: Sb,
  doctorId: string,
  patientId: string,
  ...
  const m = kbbRecallMesaji(bayraklar)
  ...
  const { data: acik } = await sb.from('kbb_gorevleri').select('id')
    .eq('doctor_id', doctorId).eq('patient_id', patientId).eq('kod', 'hatirlatma_takip').eq('durum', 'acik').maybeSingle()
  if (!acik) await sb.from('kbb_gorevleri').insert({ patient_id: patientId, doctor_id: doctorId, kod: 'hatirlatma_takip', ad: 'Hatırlatma gönderildi — randevu dönüşünü takip edin', due, kaynak: 'hatirlatma' })
  return 'gonderildi'
}
