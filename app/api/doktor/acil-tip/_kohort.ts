/**
 * ACIL-TIP-EXCEPTIONAL-01 — kohort verisi + hasta-güvenli hatırlatma.
 */
...
import {
  atKohortSatirlari, atRecallMesaji, type AtKohortBayrak, type AtKohortGirdi,
} from '@/specialties/acil-tip/engines/kohort'

export type Sb = Awaited<ReturnType<typeof doktorOturum>> extends infer T ? (T extends { supabase: infer S } ? S : never) : never

const KAYNAK_TABLOLARI = ['hasta_acil_tip', 'at_esi', 'at_kritik_yol', 'at_sevk', 'at_risk', 'at_gorevleri'] as const

export async function atKohortVerisi(sb: Sb, doctorId: string, bugun: string, sadece?: string[]) {
  ...
  const [pQ, bQ, rQ, gQ, sQ, tQ] = await Promise.all([
    ...
    sb.from('hasta_acil_tip').select('patient_id, next_kontrol, esi, kritik_yol, sevk').eq('doctor_id', doctorId).in('patient_id', ids),
    sb.from('at_risk').select('patient_id, bayraklar, hekim_onay, tarih').eq('doctor_id', doctorId).in('patient_id', ids).order('tarih', { ascending: false }).limit(3000),
    sb.from('at_gorevleri').select('patient_id, kod, due').eq('doctor_id', doctorId).eq('durum', 'acik').in('patient_id', ids).limit(5000),
  ...
  const girdi: AtKohortGirdi[] = ids.filter((id) => ad.has(id)).map((id) => {
    const b = bolum.get(id) as { next_kontrol?: string | null; esi?: { seviye?: number } | null; kritik_yol?: { yollar?: string[] } | null; sevk?: { hedef?: string } | null } | undefined
    const esi = b?.esi && typeof b.esi === 'object' ? b.esi : null
    const kritik = b?.kritik_yol && typeof b.kritik_yol === 'object' ? b.kritik_yol : null
    const sevk = b?.sevk && typeof b.sevk === 'object' ? b.sevk : null
    return {
      patientId: id,
      ad: ad.get(id)!,
      ...
      esiSeviye: typeof esi?.seviye === 'number' ? esi.seviye : null,
      kritikYolVar: Array.isArray(kritik?.yollar) && kritik!.yollar!.length > 0,
      sevkVar: !!sevk?.hedef,
    }
  })
  return { satirlar: atKohortSatirlari(girdi, bugun), toplamHasta: girdi.length }
}

export async function atHatirlatmaGonder(
  sb: Sb, doctorId: string, patientId: string, bayraklar: AtKohortBayrak[], bugun: string,
): Promise<'gonderildi' | 'yakin' | 'hata'> {
  const m = atRecallMesaji(bayraklar)
  ...
  const { data: acik } = await sb.from('at_gorevleri').select('id')
    .eq('doctor_id', doctorId).eq('patient_id', patientId).eq('kod', 'hatirlatma_takip').eq('durum', 'acik').maybeSingle()
  if (!acik) await sb.from('at_gorevleri').insert({ patient_id: patientId, doctor_id: doctorId, kod: 'hatirlatma_takip', ad: 'Hatırlatma gönderildi — randevu dönüşünü takip edin', due, kaynak: 'hatirlatma' })
  return 'gonderildi'
}
