/**
 * ONKOLOJI-EXCEPTIONAL-01 — kohort verisi + hasta-güvenli hatırlatma.
 */
...
import {
  onkoKohortSatirlari, onkoRecallMesaji, type OnkoKohortBayrak, type OnkoKohortGirdi,
} from '@/specialties/onkoloji/engines/kohort'
import { kurNormalize } from '@/specialties/onkoloji/engines/kur'

export type Sb = Awaited<ReturnType<typeof doktorOturum>> extends infer T ? (T extends { supabase: infer S } ? S : never) : never

const KAYNAK_TABLOLARI = ['hasta_onkoloji', 'onko_kur', 'onko_acil', 'onko_gorevleri'] as const

export async function onkoKohortVerisi(sb: Sb, doctorId: string, bugun: string, sadece?: string[]) {
  ...
  const [pQ, bQ, rQ, gQ, sQ, tQ] = await Promise.all([
    ...
    sb.from('hasta_onkoloji').select('patient_id, next_kontrol, kur, notes').eq('doctor_id', doctorId).in('patient_id', ids),
    sb.from('onko_acil').select('patient_id, bayraklar, hekim_onay, tarih').eq('doctor_id', doctorId).in('patient_id', ids).order('tarih', { ascending: false }).limit(3000),
    sb.from('onko_gorevleri').select('patient_id, kod, due').eq('doctor_id', doctorId).eq('durum', 'acik').in('patient_id', ids).limit(5000),
  ...
  const girdi: OnkoKohortGirdi[] = ids.filter((id) => ad.has(id)).map((id) => {
    const b = bolum.get(id)
    const kart = kurNormalize(b?.kur)
    const notes = (b?.notes && typeof b.notes === 'object' ? b.notes : {}) as { goruntu?: { sonraki?: string } }
    return {
      patientId: id,
      ad: ad.get(id)!,
      ...
      goruntuBekliyor: !!(notes.goruntu?.sonraki && notes.goruntu.sonraki < bugun),
    }
  })
  return { satirlar: onkoKohortSatirlari(girdi, bugun), toplamHasta: girdi.length }
}

export async function onkoHatirlatmaGonder(
  sb: Sb, doctorId: string, patientId: string, bayraklar: OnkoKohortBayrak[], bugun: string,
): Promise<'gonderildi' | 'yakin' | 'hata'> {
  const m = onkoRecallMesaji(bayraklar)
  ...
  const { data: acik } = await sb.from('onko_gorevleri').select('id')
    .eq('doctor_id', doctorId).eq('patient_id', patientId).eq('kod', 'hatirlatma_takip').eq('durum', 'acik').maybeSingle()
  if (!acik) await sb.from('onko_gorevleri').insert({ patient_id: patientId, doctor_id: doctorId, kod: 'hatirlatma_takip', ad: 'Hatırlatma gönderildi — randevu dönüşünü takip edin', due, kaynak: 'hatirlatma' })
  return 'gonderildi'
}
