/**
 * ONKOLOJI-EXCEPTIONAL-01 — Kohort GET/POST.
 */
import { NextRequest, NextResponse } from 'next/server'
import { doktorOturum } from '@/lib/doktor/serverAuth'
import { onkoKohortVerisi, onkoHatirlatmaGonder } from '../_kohort'
import type { OnkoKohortBayrak } from '@/specialties/onkoloji/engines/kohort'
...
export async function GET(req: NextRequest) {
  ...
  const { satirlar, toplamHasta } = await onkoKohortVerisi(sb, user.id, bugun)
...
export async function POST(req: NextRequest) {
  ...
  const bugun = new Date().toISOString().slice(0, 10)
  const { satirlar } = await onkoKohortVerisi(sb, user.id, bugun, ids)
  const izinli = new Map(satirlar.map((s) => [s.patientId, s.bayraklar as OnkoKohortBayrak[]]))
  let gonderilen = 0, atlanan = 0
  for (const id of ids) {
    const bayraklar = izinli.get(id)
    if (!bayraklar?.length) { atlanan++; continue }
    const r = await onkoHatirlatmaGonder(sb, user.id, id, bayraklar, bugun)
