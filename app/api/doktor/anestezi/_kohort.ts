/**
 * ANESTEZI-EXCEPTIONAL-01 — kohort verisi + hasta-güvenli hatırlatma.
 */
...
const KAYNAK_TABLOLARI = ['hasta_anestezi', 'anestezi_asa', 'anestezi_hava_yolu', 'anestezi_agri', 'anestezi_acil', 'anestezi_gorevleri'] as const

export async function anesteziKohortVerisi(sb: Sb, doctorId: string, bugun: string, sadece?: string[]) {
  ...
  const girdi: AnesteziKohortGirdi[] = ids.filter((id) => ad.has(id)).map((id) => {
    const b = bolum.get(id)
    const notes = (b?.notes && typeof b.notes === 'object' ? b.notes : {}) as { alerjiIlac?: boolean }
    return {
      patientId: id,
      ad: ad.get(id)!,
      ...
      alerjiIlacBayrak: !!notes.alerjiIlac,
    }
  })
  return { satirlar: anesteziKohortSatirlari(girdi, bugun), toplamHasta: girdi.length }
}

export async function anesteziHatirlatmaGonder(
  sb: Sb, doctorId: string, patientId: string, bayraklar: AnesteziKohortBayrak[], bugun: string,
): Promise<'gonderildi' | 'yakin' | 'hata'> {
  const m = anesteziRecallMesaji(bayraklar)
  ...
  const { data: acik } = await sb.from('anestezi_gorevleri').select('id')
    .eq('doctor_id', doctorId).eq('patient_id', patientId).eq('kod', 'hatirlatma_takip').eq('durum', 'acik').maybeSingle()
  if (!acik) await sb.from('anestezi_gorevleri').insert({ patient_id: patientId, doctor_id: doctorId, kod: 'hatirlatma_takip', ad: 'Hatırlatma gönderildi — randevu dönüşünü takip edin', due, kaynak: 'hatirlatma' })
  return 'gonderildi'
}
