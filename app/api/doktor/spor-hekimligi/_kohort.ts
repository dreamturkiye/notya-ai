/**
 * SPOR-HEKIMLIGI-EXCEPTIONAL-01 — Spor kohort verisi + hasta-güvenli hatırlatma.
 */
...
import {
  sporKohortSatirlari,
  sporRecallMesaji,
  type SporKohortBayrak,
  type SporKohortGirdi,
} from '@/specialties/spor-hekimligi/engines/kohort'

export type Sb = Awaited<ReturnType<typeof doktorOturum>> extends infer T ? (T extends { supabase: infer S } ? S : never) : never

const KAYNAK_TABLOLARI = ['hasta_spor', 'spor_rtp', 'spor_sakatlik', 'spor_risk', 'spor_gorevleri'] as const

export async function sporKohortVerisi(sb: Sb, doctorId: string, bugun: string, sadece?: string[]) {
  ...
  const [pQ, bQ, rQ, sQ, riskQ, gQ, sesQ, tQ] = await Promise.all([
    sb.from('patients').select('id, name_encrypted').eq('doctor_id', doctorId).in('id', ids),
    sb.from('hasta_spor').select('patient_id, next_kontrol').eq('doctor_id', doctorId).in('patient_id', ids),
    sb.from('spor_rtp').select('patient_id, basamak, tarih').eq('doctor_id', doctorId).in('patient_id', ids).order('tarih', { ascending: false }).limit(3000),
    sb.from('spor_sakatlik').select('patient_id, durum, yuklenme_uyari, tarih').eq('doctor_id', doctorId).in('patient_id', ids).order('tarih', { ascending: false }).limit(3000),
    sb.from('spor_risk').select('patient_id, bayraklar, hekim_onay, tarih').eq('doctor_id', doctorId).in('patient_id', ids).order('tarih', { ascending: false }).limit(3000),
    sb.from('spor_gorevleri').select('patient_id, kod, due').eq('doctor_id', doctorId).eq('durum', 'acik').in('patient_id', ids).limit(5000),
  ...
  const bolum = ilk(bQ.data), rtp = ilk(rQ.data), sonVizit = ilk(sesQ.data)
  const sak = grup(sQ.data), risk = grup(riskQ.data), gorev = grup(gQ.data)
  const portal = new Set((tQ.data || []).map((r) => String(r.patient_id)))

  const girdi: SporKohortGirdi[] = ids.filter((id) => ad.has(id)).map((id) => {
    const sakRows = sak.get(id) || []
    const aktif = sakRows.some((s) => s.durum === 'aktif' || s.durum === 'iyilesiyor')
    const yuk = sakRows.some((s) => s.yuklenme_uyari && (s.durum === 'aktif' || s.durum === 'iyilesiyor'))
    return {
      patientId: id,
      ad: ad.get(id)!,
      sonRtpBasamak: rtp.get(id)?.basamak != null ? Number(rtp.get(id)!.basamak) : null,
      sonRtpTarihi: rtp.get(id)?.tarih ? String(rtp.get(id)!.tarih).slice(0, 10) : null,
      aktifSakatlik: aktif,
      yuklenmeUyari: yuk,
  ...
  return { satirlar: sporKohortSatirlari(girdi, bugun), toplamHasta: girdi.length }
}

export async function sporHatirlatmaGonder(
  sb: Sb,
  doctorId: string,
  patientId: string,
  ...
  const m = sporRecallMesaji(bayraklar)
  ...
  const { data: acik } = await sb.from('spor_gorevleri').select('id')
    .eq('doctor_id', doctorId).eq('patient_id', patientId).eq('kod', 'hatirlatma_takip').eq('durum', 'acik').maybeSingle()
  if (!acik) await sb.from('spor_gorevleri').insert({ patient_id: patientId, doctor_id: doctorId, kod: 'hatirlatma_takip', ad: 'Hatırlatma gönderildi — randevu dönüşünü takip edin', due, kaynak: 'hatirlatma' })
  return 'gonderildi'
}
