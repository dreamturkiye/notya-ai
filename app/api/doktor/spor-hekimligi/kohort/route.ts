/**
 * SPOR-HEKIMLIGI-EXCEPTIONAL-01 — Araçlar › Spor kohort paneli API.
 */
import { NextRequest, NextResponse } from 'next/server'
import { doktorOturum } from '@/lib/doktor/serverAuth'
import { sporKohortVerisi, sporHatirlatmaGonder } from '../_kohort'
...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json(await sporKohortVerisi(oturum.supabase, oturum.user.id, bugun()))
}
...
export async function POST(req: NextRequest) {
  ...
  const T = bugun()
  const { satirlar } = await sporKohortVerisi(sb, user.id, T, ids)
  let gonderilen = 0, atlanan = 0
  for (const s of satirlar) {
    const r = await sporHatirlatmaGonder(sb, user.id, s.patientId, s.bayraklar, T)
