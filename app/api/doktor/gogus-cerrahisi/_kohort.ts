/**
 * GOGUS-CERRAHISI-EXCEPTIONAL-01 — kohort verisi + hasta-güvenli hatırlatma.
 */
...
} from '@/specialties/gogus-cerrahisi/engines/kohort'
import { patolojiNormalize } from '@/specialties/gogus-cerrahisi/engines/patoloji'
...
export async function gcKohortVerisi(sb: Sb, doctorId: string, bugun: string, sadece?: string[]) {
  ...
  const girdi: GcKohortGirdi[] = ids.filter((id) => ad.has(id)).map((id) => {
    ...
    const tup = (b?.tup_yara && typeof b.tup_yara === 'object' ? b.tup_yara : null) as { durum?: string } | null
    const pat = patolojiNormalize(b?.patoloji)
    return {
      patientId: id,
      ad: ad.get(id)!,
      ...
      tupYaraAktif: !!(tup?.durum && tup.durum !== 'cikarildi'),
      patolojiBekliyor: !!(b?.patoloji && !pat.hazir),
