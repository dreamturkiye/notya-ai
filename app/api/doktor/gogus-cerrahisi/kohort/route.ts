/**
 * GOGUS-CERRAHISI-EXCEPTIONAL-01 — Kohort GET/POST.
 */
import { NextRequest, NextResponse } from 'next/server'
import { doktorOturum } from '@/lib/doktor/serverAuth'
import { gcKohortVerisi, gcHatirlatmaGonder } from '../_kohort'
import type { GcKohortBayrak } from '@/specialties/gogus-cerrahisi/engines/kohort'
...
export async function GET(req: NextRequest) {
  ...
  const { satirlar, toplamHasta } = await gcKohortVerisi(sb, user.id, bugun)
...
export async function POST(req: NextRequest) {
  ...
  const bugun = new Date().toISOString().slice(0, 10)
  const { satirlar } = await gcKohortVerisi(sb, user.id, bugun, ids)
  const izinli = new Map(satirlar.map((s) => [s.patientId, s.bayraklar as GcKohortBayrak[]]))
  let gonderilen = 0, atlanan = 0
  for (const id of ids) {
    const bayraklar = izinli.get(id)
    if (!bayraklar?.length) { atlanan++; continue }
    const r = await gcHatirlatmaGonder(sb, user.id, id, bayraklar, bugun)
