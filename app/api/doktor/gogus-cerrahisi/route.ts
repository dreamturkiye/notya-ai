/**
 * GOGUS-CERRAHISI-EXCEPTIONAL-01 — Göğüs Cerrahisi API. Ayaktan muayenehane akışı.
 * HASTA-İZOLASYON: her istek hastaSahibiMi kapısından geçer.
 * GET  ?patientId= → şerit, preop, tüp/yara, patoloji, acil, görevler
 * POST adim: preop | tup_yara | patoloji | risk | gorev | kontrol
 */
import { NextRequest, NextResponse } from 'next/server'
import { doktorOturum } from '@/lib/doktor/serverAuth'
import { hastaSahibiMi } from '@/lib/doktor/hastaSahipligi'
import { gununNotunaEkle } from '@/lib/doktor/gununNotunaEkle'
import { preopSkorla } from '@/specialties/gogus-cerrahisi/engines/preop'
import { tupYaraSkorla } from '@/specialties/gogus-cerrahisi/engines/tupYara'
import { patolojiSkorla, patolojiNormalize } from '@/specialties/gogus-cerrahisi/engines/patoloji'
import {
  acilTara, hekimOnayiGerekliMi, ACIL_KODLARI, ACIL_KONTROL_LISTESI, type AcilKod,
} from '@/specialties/gogus-cerrahisi/engines/acil'
import { gcSeridi } from '@/specialties/gogus-cerrahisi/engines/serit'
import {
  HEKIM_KILIT_METNI, ACIL_YONLENDIRME_METNI, KAPSAM_NOTU, ISO_GUN,
} from '@/specialties/gogus-cerrahisi/engines/gogus-cerrahisi'
...
async function bolumKaydi(sb: Sb, doctorId: string, patientId: string) {
  const { data } = await sb.from('hasta_gogus_cerrahisi').select('id, next_kontrol, preop, tup_yara, patoloji, notes').eq('patient_id', patientId).eq('doctor_id', doctorId).maybeSingle()
  if (data) return data
  const { data: yeni } = await sb.from('hasta_gogus_cerrahisi').insert({ patient_id: patientId, doctor_id: doctorId }).select('id, next_kontrol, preop, tup_yara, patoloji, notes').maybeSingle()
  return yeni || null
}
...
export async function POST(req: NextRequest) {
  ...
  if (adim === 'preop') {
    ...
    await sb.from('hasta_gogus_cerrahisi').update({ preop: { secilen: s.secilen, tarih: T }, updated_at: new Date().toISOString() }).eq('id', String(kayit?.id || '')).eq('doctor_id', user.id).eq('patient_id', patientId)
  ...
  if (adim === 'tup_yara') {
    ...
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    await sb.from('hasta_gogus_cerrahisi').update({ tup_yara: s.kart, updated_at: new Date().toISOString() }).eq('id', String(kayit?.id || '')).eq('doctor_id', user.id).eq('patient_id', patientId)
    await gorevEkle(sb, user.id, patientId, s.gorevOnerileri.map((g) => ({ ...g, kaynak: 'tup_yara' })))
    const rn = await gununNotunaEkle(sb, user.id, patientId, s.ozet)
  ...
  if (adim === 'patoloji') {
    ...
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    await sb.from('hasta_gogus_cerrahisi').update({ patoloji: s.kart, updated_at: new Date().toISOString() }).eq('id', String(kayit?.id || '')).eq('doctor_id', user.id).eq('patient_id', patientId)
    await gorevEkle(sb, user.id, patientId, s.gorevOnerileri.map((g) => ({ ...g, kaynak: 'patoloji' })))
    const rn = await gununNotunaEkle(sb, user.id, patientId, s.ozet)
    return NextResponse.json({ ok: true, kart: s.kart, notId: rn.eklendi ? rn.notId : null })
  }
  ...
  if (adim === 'risk') {
    ...
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    if (bayraklar.length) {
      const rn = await gununNotunaEkle(sb, user.id, patientId, `⚑ Göğüs cerrahisi acil bayrak: ${bayraklar.map((x) => x.ad).join('; ')} — hekim onayı verildi${eylem ? `: ${eylem}` : ''}. ${ACIL_YONLENDIRME_METNI}`)
  ...
  if (adim === 'kontrol') {
    ...
    const kayit = await bolumKaydi(sb, user.id, patientId)
    const { error } = await sb.from('hasta_gogus_cerrahisi')
...
export async function GET(req: NextRequest) {
  ...
  const tup = (bolum.data?.tup_yara && typeof bolum.data.tup_yara === 'object' ? bolum.data.tup_yara : null) as { durum?: string } | null
  const pat = patolojiNormalize(bolum.data?.patoloji)

  const planlar: Array<{ kaynak: string; madde: string }> = []
  if (preopSayi) planlar.push({ kaynak: 'Pre-op', madde: `${preopSayi} madde` })
  if (tup?.durum) planlar.push({ kaynak: 'Tüp/yara', madde: String(tup.durum) })
  if (pat.hazir) planlar.push({ kaynak: 'Patoloji', madde: 'rapor hazır' })
  else if (pat.raporHazirTarihi || pat.ornekTarihi) planlar.push({ kaynak: 'Patoloji', madde: 'bekleniyor' })

  const serit = gcSeridi({
    bugun: T,
    preopSayi,
    tupYaraDurum: tup?.durum ? String(tup.durum) : null,
    patolojiHazir: bolum.data?.patoloji ? pat.hazir : null,
  ...
  return NextResponse.json({
    ...
    bolum: {
      ...
      tupYara: bolum.data?.tup_yara || null,
