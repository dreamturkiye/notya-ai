/**
 * PLASTIK-CERRAHI-EXCEPTIONAL-01 — kohort verisi + hasta-güvenli hatırlatma.
 */
...
import {
  plastikKohortSatirlari, plastikRecallMesaji, type PlastikKohortBayrak, type PlastikKohortGirdi,
} from '@/specialties/plastik-cerrahi/engines/kohort'

export type Sb = Awaited<ReturnType<typeof doktorOturum>> extends infer T ? (T extends { supabase: infer S } ? S : never) : never

const KAYNAK_TABLOLARI = ['hasta_plastik', 'plastik_yara', 'plastik_foto', 'plastik_acil', 'plastik_gorevleri'] as const

export async function plastikKohortVerisi(sb: Sb, doctorId: string, bugun: string, sadece?: string[]) {
  ...
  const [pQ, bQ, rQ, gQ, sQ, tQ] = await Promise.all([
    ...
    sb.from('hasta_plastik').select('patient_id, next_kontrol, foto').eq('doctor_id', doctorId).in('patient_id', ids),
    sb.from('plastik_acil').select('patient_id, bayraklar, hekim_onay, tarih').eq('doctor_id', doctorId).in('patient_id', ids).order('tarih', { ascending: false }).limit(3000),
    sb.from('plastik_gorevleri').select('patient_id, kod, due').eq('doctor_id', doctorId).eq('durum', 'acik').in('patient_id', ids).limit(5000),
  ...
  const girdi: PlastikKohortGirdi[] = ids.filter((id) => ad.has(id)).map((id) => {
    const b = bolum.get(id)
    const foto = (b?.foto && typeof b.foto === 'object' ? b.foto : {}) as { sonraki?: string }
    return {
      patientId: id,
      ad: ad.get(id)!,
      ...
      fotoBekliyor: !!(foto.sonraki && foto.sonraki < bugun),
    }
  })
  return { satirlar: plastikKohortSatirlari(girdi, bugun), toplamHasta: girdi.length }
}

export async function plastikHatirlatmaGonder(
  sb: Sb, doctorId: string, patientId: string, bayraklar: PlastikKohortBayrak[], bugun: string,
): Promise<'gonderildi' | 'yakin' | 'hata'> {
  const m = plastikRecallMesaji(bayraklar)
  ...
  const { data: acik } = await sb.from('plastik_gorevleri').select('id')
    .eq('doctor_id', doctorId).eq('patient_id', patientId).eq('kod', 'hatirlatma_takip').eq('durum', 'acik').maybeSingle()
  if (!acik) await sb.from('plastik_gorevleri').insert({ patient_id: patientId, doctor_id: doctorId, kod: 'hatirlatma_takip', ad: 'Hatırlatma gönderildi — randevu dönüşünü takip edin', due, kaynak: 'hatirlatma' })
  return 'gonderildi'
}
