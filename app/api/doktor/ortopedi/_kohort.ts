/**
 * ORTOPEDI-EXCEPTIONAL-01 — Ortopedi kohort verisi + hasta-güvenli hatırlatma.
 */
...
import {
  ortoKohortSatirlari,
  ortoRecallMesaji,
  type OrtoKohortBayrak,
  type OrtoKohortGirdi,
} from '@/specialties/ortopedi/engines/kohort'

export type Sb = Awaited<ReturnType<typeof doktorOturum>> extends infer T ? (T extends { supabase: infer S } ? S : never) : never

const KAYNAK_TABLOLARI = ['hasta_ortopedi', 'orto_kirik_alci', 'orto_vas', 'orto_risk', 'orto_gorevleri'] as const

export async function ortoKohortVerisi(sb: Sb, doctorId: string, bugun: string, sadece?: string[]) {
  ...
  const [pQ, bQ, vQ, rQ, gQ, sQ, tQ] = await Promise.all([
    sb.from('patients').select('id, name_encrypted').eq('doctor_id', doctorId).in('id', ids),
    sb.from('hasta_ortopedi').select('patient_id, next_kontrol').eq('doctor_id', doctorId).in('patient_id', ids),
    sb.from('orto_vas').select('patient_id, bant, tarih').eq('doctor_id', doctorId).in('patient_id', ids).order('tarih', { ascending: false }).limit(3000),
    sb.from('orto_risk').select('patient_id, bayraklar, hekim_onay, tarih').eq('doctor_id', doctorId).in('patient_id', ids).order('tarih', { ascending: false }).limit(3000),
    sb.from('orto_gorevleri').select('patient_id, kod, due').eq('doctor_id', doctorId).eq('durum', 'acik').in('patient_id', ids).limit(5000),
  ...
  const bolum = ilk(bQ.data), vas = ilk(vQ.data), sonVizit = ilk(sQ.data)
  const risk = grup(rQ.data), gorev = grup(gQ.data)
  const portal = new Set((tQ.data || []).map((r) => String(r.patient_id)))

  const girdi: OrtoKohortGirdi[] = ids.filter((id) => ad.has(id)).map((id) => ({
    patientId: id,
    ad: ad.get(id)!,
    sonVasBant: vas.get(id)?.bant ? String(vas.get(id)!.bant) : null,
  ...
  return { satirlar: ortoKohortSatirlari(girdi, bugun), toplamHasta: girdi.length }
}

export async function ortoHatirlatmaGonder(
  sb: Sb,
  doctorId: string,
  patientId: string,
  ...
  const m = ortoRecallMesaji(bayraklar)
  ...
  const { data: acik } = await sb.from('orto_gorevleri').select('id')
    .eq('doctor_id', doctorId).eq('patient_id', patientId).eq('kod', 'hatirlatma_takip').eq('durum', 'acik').maybeSingle()
  if (!acik) await sb.from('orto_gorevleri').insert({ patient_id: patientId, doctor_id: doctorId, kod: 'hatirlatma_takip', ad: 'Hatırlatma gönderildi — randevu dönüşünü takip edin', due, kaynak: 'hatirlatma' })
  return 'gonderildi'
}
