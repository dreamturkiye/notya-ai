/**
 * ORTOPEDI-EXCEPTIONAL-01 — Araçlar › Ortopedi kohort paneli API.
 */
import { NextRequest, NextResponse } from 'next/server'
import { doktorOturum } from '@/lib/doktor/serverAuth'
import { ortoKohortVerisi, ortoHatirlatmaGonder } from '../_kohort'
...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json(await ortoKohortVerisi(oturum.supabase, oturum.user.id, bugun()))
}
...
export async function POST(req: NextRequest) {
  ...
  const T = bugun()
  const { satirlar } = await ortoKohortVerisi(sb, user.id, T, ids)
  let gonderilen = 0, atlanan = 0
  for (const s of satirlar) {
    const r = await ortoHatirlatmaGonder(sb, user.id, s.patientId, s.bayraklar, T)
