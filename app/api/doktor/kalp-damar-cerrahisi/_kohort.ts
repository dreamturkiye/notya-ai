/**
 * KALP-DAMAR-CERRAHISI-EXCEPTIONAL-01 — kohort verisi + hasta-güvenli hatırlatma.
 */
...
const KAYNAK_TABLOLARI = ['hasta_kalp_damar_cerrahisi', 'kdc_preop', 'kdc_greft_yara', 'kdc_antikoag', 'kdc_acil', 'kdc_gorevleri'] as const

export async function kdcKohortVerisi(sb: Sb, doctorId: string, bugun: string, sadece?: string[]) {
  ...
  const girdi: KdcKohortGirdi[] = ids.filter((id) => ad.has(id)).map((id) => {
    const b = bolum.get(id)
    const preop = (b?.preop && typeof b.preop === 'object' ? b.preop : null) as { secilen?: string[] } | null
    const greft = (b?.greft_yara && typeof b.greft_yara === 'object' ? b.greft_yara : null) as { durum?: string } | null
    const ak = antikoagNormalize(b?.antikoag)
    return {
      patientId: id,
      ad: ad.get(id)!,
      ...
      preopSayi: Array.isArray(preop?.secilen) ? preop!.secilen!.length : 0,
      greftYaraAktif: !!(greft?.durum && greft.durum !== 'kapandi'),
      antikoagVade: !!(ak.sonrakiKontrol || ak.labVadesi),
  ...
  return { satirlar: kdcKohortSatirlari(girdi, bugun), toplamHasta: girdi.length }
}

export async function kdcHatirlatmaGonder(
  sb: Sb, doctorId: string, patientId: string, bayraklar: KdcKohortBayrak[], bugun: string,
): Promise<'gonderildi' | 'yakin' | 'hata'> {
  const m = kdcRecallMesaji(bayraklar)
  ...
  const { data: acik } = await sb.from('kdc_gorevleri').select('id')
    .eq('doctor_id', doctorId).eq('patient_id', patientId).eq('kod', 'hatirlatma_takip').eq('durum', 'acik').maybeSingle()
  if (!acik) await sb.from('kdc_gorevleri').insert({ patient_id: patientId, doctor_id: doctorId, kod: 'hatirlatma_takip', ad: 'Hatırlatma gönderildi — randevu dönüşünü takip edin', due, kaynak: 'hatirlatma' })
  return 'gonderildi'
}
