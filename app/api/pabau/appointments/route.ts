...
/**
 * NOTYA-PABAU-01 — the clinic's future Pabau appointments, proxied.
 * Documented endpoint: GET https://api.oauth.pabau.com/{api_key}/appointments
 * (returns all future appointments; a single one via /appointments/{id}).
 */
...
export async function GET(req: NextRequest) {
  ...
  const r = await pabauGet(apiKey, id ? `appointments/${encodeURIComponent(id)}` : "appointments")
  if (!r.ok) return NextResponse.json({ success: false, error: "Pabau randevu verisi alınamadı." }, { status: 502 })
