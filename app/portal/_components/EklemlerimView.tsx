'use client'
/**
 * ORTOPEDI-EXCEPTIONAL-01 — Sağlığım › Eklemlerim (hasta yüzü).
 * YASAK: tanı, VAS sayı, skor, doz, kaynama.
 */
import Link from 'next/link'
import type { PortalEklem } from '@/lib/portal/types'
import { EmptyState, SectionHeader, SoftPanel } from './ui'

function uzunTarih(iso: string): string {
  const d = new Date(iso.length === 10 ? `${iso}T12:00:00` : iso)
  return isNaN(d.getTime()) ? iso : d.toLocaleDateString('tr-TR', { day: 'numeric', month: 'long', year: 'numeric' })
}

function Baslik({ children }: { children: React.ReactNode }) {
  return (
    <div className="sg-goz-baslik">
      <h2>{children}</h2>
    </div>
  )
}

const ALT_BASLIK = 'Doktorunuzun belirlediği kontrol ve hatırlatma tarihleri. Yorum ve plan doktorunuzdadır.'

export function EklemlerimView({ eklem, basePath }: { eklem: PortalEklem | null; basePath: string }) {
  if (!eklem) {
    return (
      <div className="sg-fade">
        <SectionHeader title="Eklemlerim" subtitle={ALT_BASLIK} />
        <EmptyState art="takip" title="Henüz paylaşılan takip yok" body="Doktorunuz kontrol tarihi veya hatırlatma kaydettiğinde burada görünür." />
        <Link href={basePath} className="sg-back-link" style={{ margin: '0 20px' }}>← Özet</Link>
      </div>
    )
  }

  const bos = !eklem.sonrakiKontrol && !eklem.hatirlatmalar.length && !eklem.izlemHatirlatma.length && !eklem.islemHatirlatma.length

  return (
    <div className="sg-fade">
      <SectionHeader title="Eklemlerim" subtitle={ALT_BASLIK} />
      <Link href={basePath} className="sg-back-link" style={{ margin: '0 20px 16px' }}>← Özet</Link>

      {bos && (
        <EmptyState art="takip" title="Henüz paylaşılan takip yok" body="Doktorunuz kontrol tarihi veya hatırlatma kaydettiğinde burada görünür." />
      )}

      {eklem.sonrakiKontrol && (
        <SoftPanel className="sg-goz-panel">
          <Baslik>Yaklaşan kontrol</Baslik>
          <div className="sg-goz-tarih">{uzunTarih(eklem.sonrakiKontrol.tarih)}</div>
          <div className="sg-goz-meta">{eklem.sonrakiKontrol.neden}</div>
        </SoftPanel>
      )}

      {eklem.izlemHatirlatma.length > 0 && (
        <SoftPanel className="sg-goz-panel">
          <Baslik>Alçı / hareket takip</Baslik>
          <ul style={{ margin: 0, paddingLeft: 18 }}>
            {eklem.izlemHatirlatma.map((h, i) => (
              <li key={i}>{h.ad}{h.due ? ` · ${uzunTarih(h.due)}` : ''}</li>
            ))}
          </ul>
          <div className="sg-goz-meta" style={{ marginTop: 8 }}>
            Sonuçları doktorunuz değerlendirir ve muayenede sizinle konuşur.
          </div>
        </SoftPanel>
      )}

      {eklem.islemHatirlatma.length > 0 && (
        <SoftPanel className="sg-goz-panel">
          <Baslik>Görüntüleme ve işlem randevuları</Baslik>
          <ul style={{ margin: 0, paddingLeft: 18 }}>
            {eklem.islemHatirlatma.map((h, i) => (
              <li key={i}>{h.ad}{h.due ? ` · ${uzunTarih(h.due)}` : ''}</li>
            ))}
          </ul>
        </SoftPanel>
      )}

      {eklem.hatirlatmalar.length > 0 && (
        <SoftPanel className="sg-goz-panel">
          <Baslik>Tüm hatırlatmalar</Baslik>
          <ul style={{ margin: 0, paddingLeft: 18 }}>
            {eklem.hatirlatmalar.map((h, i) => (
              <li key={i}>
                {h.ad}{h.due ? ` · ${uzunTarih(h.due)}` : ''}
                {h.durum === 'gecikti' && <span className="sg-goz-meta"> · tarihi geçti</span>}
                {h.durum === 'yaklasiyor' && <span className="sg-goz-meta"> · yaklaşıyor</span>}
              </li>
            ))}
          </ul>
        </SoftPanel>
      )}

      {eklem.bakimIpuclari.length > 0 && (
        <SoftPanel className="sg-goz-panel">
          <Baslik>Genel öneriler</Baslik>
          <ul style={{ margin: 0, paddingLeft: 18 }}>
            {eklem.bakimIpuclari.map((x, i) => <li key={i}>{x}</li>)}
          </ul>
        </SoftPanel>
      )}

      <p className="sg-goz-meta" style={{ margin: '16px 20px' }}>{eklem.not}</p>
    </div>
  )
}
