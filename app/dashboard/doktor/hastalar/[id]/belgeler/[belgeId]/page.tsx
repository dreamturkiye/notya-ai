    ...
    <div style={toolsShell}>
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '14px 12px' }}>
        ...
        <div className="notya-grid-yigin" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1fr)', gap: 14 }}>
          ...
          <div>
            <div style={{ ...toolsCard, marginBottom: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8, marginBottom: 8 }}>
                <div style={{ fontSize: 15, fontWeight: 800, color: CHROME_RENK.ink }}>{doc?.fileName || 'Belge'}</div>
              ...
              {modalite === 'fundus' && (
                <label style={{ display: 'flex', gap: 8, alignItems: 'flex-start', marginTop: 8, fontSize: 12, color: CHROME_RENK.muted, cursor: 'pointer' }}>
              ...
              <div style={{ marginTop: 10, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                ...
                {mesaj && <span style={{ fontSize: 12, color: durum === 'hata' ? '#a45b3e' : CHROME_RENK.pine }}>{mesaj}</span>}
              </div>
            </div>
          ...
          {/* RIGHT: report */}
          <div>
            {!rapor && <div style={{ ...toolsCard, color: CHROME_RENK.muted, fontSize: 13 }}>
              <div style={{ fontWeight: 700, color: CHROME_RENK.ink, marginBottom: 6 }}>Henüz taslak yok</div>
              ...
              <>
                ...
                {rapor.bulgular.length > 0 && (
                  <div style={{ ...toolsCard, marginBottom: 10 }}>
                    ...
                    <ul style={{ margin: 0, paddingLeft: 18, fontSize: 13, color: CHROME_RENK.ink }}>{rapor.bulgular.map((b, i) => <li key={i}>{b}</li>)}</ul>
                  </div>
                )}

                <div style={{ ...toolsCard, marginBottom: 10 }}>
                  <div style={etiket}>Olası tanılar <span style={{ fontWeight: 400, color: CHROME_RENK.muted }}>· güven üst sınırı %{analiz.fusion?.capPct ?? 70}</span></div>
                  {rapor.tanilar.length === 0 && <div style={{ fontSize: 12, color: CHROME_RENK.muted }}>Tanı önerisi yok.</div>}
                  {rapor.tanilar.map((t, i) => (
                    <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '6px 0', borderTop: i ? '1px solid #F6F0E4' : 'none' }}>
                      ...
                      <div style={{ flex: 1, fontSize: 13, color: CHROME_RENK.ink }}>
                        <div style={{ fontWeight: 700 }}>{t.ad} {t.icd10 && <span style={{ color: CHROME_RENK.muted, fontWeight: 400 }}>({t.icd10})</span>}</div>
                        {t.destek.length > 0 && <div style={{ fontSize: 11, color: CHROME_RENK.pine }}>destek: {t.destek.join(', ')}</div>}
                ...
                <div style={{ ...toolsCard, marginBottom: 10, position: 'sticky', bottom: 12, zIndex: 30, boxShadow: '0 -8px 24px rgba(0,0,0,0.45)' }}>
                  <div style={etiket}>Resmi tanı (hekim) <span style={{ fontWeight: 400, color: CHROME_RENK.muted }}>· her satır bir tanı; ICD-10 parantez içinde</span></div>
                  ...
                  {onayMesaj && <div style={{ marginTop: 8, fontSize: 12, fontWeight: 700, color: /eklendi|kilitlendi|onaylandı/i.test(onayMesaj) ? CHROME_RENK.pine : '#a45b3e' }}>{onayMesaj}</div>}
                  ...
                  {analiz.durum === 'muayene_onaylandi' && <div style={{ marginTop: 6, fontSize: 12, color: CHROME_RENK.pine }}>Muayene onaylandı — rapor kilitli. {analiz.note_id && <a href={`/dashboard/doktor/notlar/${analiz.note_id}`} style={{ color: CHROME_RENK.pine }}>Notu aç →</a>}</div>}
                  {analiz.durum === 'onaylandi' && analiz.note_id && <div style={{ marginTop: 6, fontSize: 12, color: CHROME_RENK.pine }}>Objektif&apos;e eklendi. <a href={`/dashboard/doktor/notlar/${analiz.note_id}`} style={{ color: CHROME_RENK.pine }}>Notu aç →</a> · <a href={hastaBelgelerHref(patientId)} style={{ color: CHROME_RENK.pine }}>← Belgeler</a></div>}
                </div>
                ...
                {kural.goruntuOkumaKoprusu && ['fundus', 'oct', 'dis_goz'].includes(analiz.modality_final) && analiz.durum !== 'kalite_dusuk' && (
                  <div style={{ ...toolsCard, marginBottom: 10, fontSize: 13, color: CHROME_RENK.ink }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                      ...
                      {kopruMesaj && <span style={{ fontSize: 12, color: /gönderildi/.test(kopruMesaj) ? CHROME_RENK.pine : '#a45b3e' }}>{kopruMesaj} {/gönderildi/.test(kopruMesaj) && <a href={hastaDosyaHref(patientId, 'goz')} style={{ color: CHROME_RENK.pine }}>Göz sekmesini aç →</a>}</span>}
                    </div>
                    <div style={{ fontSize: 11, color: CHROME_RENK.muted, marginTop: 6 }}>Tek alan fundus güven üst sınırı %70. DR evresi aktarılmaz — evre DR kartında hekim kilidi.</div>
                  </div>
                )}
