...
type Panel = { id: string; lab_adi: string | null; numune_tarihi: string | null; kaynaklar: string[]; kalite: string; kimlik_uyari: { eslesme: boolean | null; ipucu: string | null } | null; tablo_onayli: boolean; durum: string };
...
const flagRenk: Record<string, string> = { H: '#F87171', L: '#60A5FA', critical: '#EF4444', normal: '#2DD4BF', unknown: '#64748B' };
                    ...
                    <tbody>
                      {satirlar.map((s) => (
                        <tr key={s.id} style={{ background: s.dogrulanacak ? 'rgba(251,191,36,0.08)' : s.kritik ? 'rgba(239,68,68,0.08)' : 'transparent' }} title={s.dogrulama_notu || undefined}>
                          ...
                          <td style={{ ...hücre, color: flagRenk[s.flag], fontWeight: 800 }}>{s.flag === 'critical' ? 'KRİTİK' : s.flag === 'H' ? '↑ H' : s.flag === 'L' ? '↓ L' : s.flag === 'normal' ? 'N' : '?'}</td>
