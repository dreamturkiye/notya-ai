...
const flagRenk: Record<string, string> = { H: '#a45b3e', L: '#4A5C8A', critical: '#a45b3e', normal: '#2f4334', unknown: '#8b7d70', sinir: '#B4832F', pozitif_suphe: '#a45b3e', yetersiz_ornek: '#B4832F', tekrar: '#B4832F' };
...
const KaynakDip = ({ d, acik }: { d?: Dip[] | null; acik: boolean }) => (!acik || !d?.length ? null : <div style={{ fontSize: 11, color: '#8b7d70', marginTop: 4, borderLeft: '2px solid rgba(47,67,52,0.4)', paddingLeft: 6 }}>{d.map((x, i) => <div key={i}><b>{x.ref}</b> — {x.not} <span style={{ opacity: 0.7 }}>({(REF_ACIKLAMA as Record<string, string>)[x.ref] || x.ref})</span></div>)}</div>);
const hücre: React.CSSProperties = { padding: '4px 6px', fontSize: 12, borderBottom: '1px solid #F6F0E4', verticalAlign: 'top' };
const girdi: React.CSSProperties = { width: '100%', background: 'transparent', border: '1px solid transparent', color: '#3b2e24', fontSize: 12, padding: '2px 4px', borderRadius: 4 };
  ...
  return (
    <div style={toolsShell}>
      <div style={{ maxWidth: 1240, margin: '0 auto', padding: '14px 12px' }}>
        ...
        {panel?.panel_type === 'enabiz_gecmis' && <div style={{ background: '#E4F3F1', border: '1px solid rgba(47,67,52,0.4)', color: '#2f4334', borderRadius: 10, padding: '10px 14px', fontSize: 13, marginBottom: 10 }}>e-Nabız geçmiş içe aktarma (yalnız yüklenen PDF — canlı e-Nabız çekimi yok). Her satır kendi basılı tarihini taşır; tarihsiz satır onaylı seriye girmez. Kimlik kontrolü ve Onayla kapısı her lab belgesiyle aynıdır.</div>}
          ...
          <div>
            <div style={toolsCard}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8 }}>
                <div style={{ fontSize: 15, fontWeight: 800, color: '#3b2e24' }}>{doc?.fileName || 'Laboratuvar belgesi'}</div>
              ...
              {doc && /csv|excel|spreadsheet/.test(doc.fileType) && <div style={{ fontSize: 12, color: '#8b7d70', marginTop: 8 }}>Tablo dosyası — satırlar sağda.</div>}
              <div style={{ marginTop: 10, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                ...
                {panel && <span style={{ fontSize: 11, color: '#8b7d70' }}>{panel.lab_adi || ''}{panel.numune_tarihi ? ` · numune ${new Date(panel.numune_tarihi).toLocaleDateString('tr-TR')}` : ''} · kaynak: {panel.kaynaklar?.join(' + ') || '—'} · kalite {panel.kalite}</span>}
              </div>
              {mesaj && <div style={{ marginTop: 8, fontSize: 12, color: /Hata|amadı|eşleşmiyor/.test(mesaj) ? '#a45b3e' : '#2f4334' }}>{mesaj}</div>}
            </div>
          </div>

          <div>
            {!panel && <div style={{ ...toolsCard, color: '#8b7d70', fontSize: 13 }}>{durum === 'cikariyor' ? 'Tablo çıkarılıyor…' : 'Laboratuvar tablosu hazırlanıyor. Birkaç saniye içinde burada görünür; gerekirse soldan "Tabloyu çıkar"a basın.'}</div>}
            {panel && (
              <div style={{ ...toolsCard, marginBottom: 10 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', flexWrap: 'wrap', gap: 8 }}>
                  <div style={etiket}>Tablo <span style={{ fontWeight: 400, color: '#8b7d70' }}>· {satirlar.length} parametre · {yuksek} yüksek · {dusuk} düşük{kritikSatirlar.length ? ` · ${kritikSatirlar.length} kritik` : ''}{eslesmeyen ? ` · ${eslesmeyen} eşleşmedi` : ''} · hücreye tıklayıp düzeltin</span></div>
                ...
                {!analiz && (
                  <div style={{ marginTop: 8, padding: '8px 10px', borderRadius: 8, background: '#E4F3F1', border: '1px solid rgba(47,67,52,0.4)', fontSize: 12, color: '#2f4334', lineHeight: 1.45 }}>
                ...
                <div style={{ overflowX: 'auto', marginTop: 6 }}>
                  <table style={{ width: '100%', minWidth: 560, borderCollapse: 'collapse' }}>
                    <thead><tr style={{ color: '#8b7d70', fontSize: 11 }}>{['Test', 'Sonuç', 'Birim', 'Ref', 'Flag', 'Önceki', 'Δ', 'Trend'].map((h) => <th key={h} style={{ ...hücre, textAlign: 'left', fontWeight: 600 }}>{h}</th>)}</tr></thead>
                    <tbody>
                      {satirlar.map((s) => (
                        <tr key={s.id} style={{ background: s.dogrulanacak ? '#FBF3DE' : s.kritik ? '#FBEAE3' : 'transparent' }} title={s.dogrulama_notu || undefined}>
                          <td style={hücre}>
                            ...
                            <div style={{ fontSize: 10, color: s.canonical_key ? '#8b7d70' : '#B4832F' }}>
                            ...
                            {panel?.panel_type === 'enabiz_gecmis' && <input type="date" title="e-Nabız satır tarihi (basılı) — tarihsiz satır onaylı seriye girmez" defaultValue={s.numune_tarihi || ''} disabled={kilitli} onBlur={(e) => e.target.value && e.target.value !== (s.numune_tarihi || '') && hücreKaydet(s, 'numune_tarihi', e.target.value)} style={{ ...girdi, fontSize: 10, width: 120, color: s.numune_tarihi ? '#8b7d70' : '#B4832F' }} />}
                          </td>
                          ...
                          <td style={{ ...hücre, color: '#8b7d70' }}>{s.prior_value != null ? `${s.prior_value}${s.prior_date ? ` (${new Date(s.prior_date).toLocaleDateString('tr-TR')})` : ''}` : '—'}</td>
                          <td style={{ ...hücre, color: '#3b2e24' }}>{s.delta_pct != null ? `${s.delta_pct > 0 ? '+' : ''}${s.delta_pct}%` : '—'}</td>
                          <td style={{ ...hücre, color: s.trend === 'new_abn' ? '#a45b3e' : s.trend === 'new_normal' ? '#2f4334' : '#8b7d70' }}>{trendTr[s.trend] || s.trend}{s.dogrulanacak ? ' · doğrulanacak' : ''}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {lab && analiz && (
              <>
                ...
                <div style={{ ...toolsCard, marginBottom: 10 }}>
                  <div style={etiket}>Özet ({persona}) <span style={{ fontWeight: 400, color: '#8b7d70' }}>· düzenlenebilir</span></div>
                  ...
                  <div style={{ marginTop: 6, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    {kayitMesaj && /özet|Özet|oturum|Taslak|Kaydedilemedi|kaydedildi/i.test(kayitMesaj) && (
                      <span style={{ fontSize: 12, fontWeight: 700, color: /kaydedildi/i.test(kayitMesaj) ? '#2f4334' : /kaydediliyor/i.test(kayitMesaj) ? '#8b7d70' : '#a45b3e' }}>{kayitMesaj}</span>
                    )}
                  </div>
                </div>
                {[['Yeni bozulanlar', lab.yeni_bozulanlar, '#a45b3e'], ['Düzelenler', lab.duzelenler, '#2f4334'], ['Kronik sapma', lab.kronik, '#B4832F']].map(([baslik, liste, renk]) => (liste as string[]).length ? (
                  <div key={String(baslik)} style={{ ...toolsCard, marginBottom: 10 }}><div style={{ ...etiket, color: String(renk) }}>{String(baslik)}</div><ul style={{ margin: 0, paddingLeft: 18, fontSize: 13, color: '#3b2e24' }}>{(liste as string[]).map((x, i) => <li key={i}>{x}</li>)}</ul></div>
                ) : null)}
                <div style={{ ...toolsCard, marginBottom: 10 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8 }}><div style={etiket}>Olası tanılar <span style={{ fontWeight: 400, color: '#8b7d70' }}>· güven üst sınırı %{analiz.fusion?.capPct ?? 70}</span></div>{lab.kaynak && <button type="button" onClick={() => setKaynakAcik((x) => !x)} style={{ ...btnGhost, padding: '3px 9px', fontSize: 11, color: kaynakAcik ? '#2f4334' : '#8b7d70' }}>Kaynak</button>}</div>
                  {lab.tanilar.length === 0 && <div style={{ fontSize: 12, color: '#8b7d70' }}>Tanı önerisi yok.</div>}
                  {lab.tanilar.map((t, i) => (
                    <div key={i} style={{ display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'flex-start', padding: '6px 0', borderTop: i ? '1px solid #F6F0E4' : 'none' }}>
                      <div style={{ minWidth: 60, textAlign: 'center' }}><div style={{ fontSize: 18, fontWeight: 800, color: t.guven_bant === 'yüksek' ? '#2f4334' : t.guven_bant === 'orta' ? '#B4832F' : '#8b7d70' }}>%{t.guven_pct}</div><div style={{ fontSize: 10, color: '#8b7d70' }}>{t.guven_bant}</div></div>
                      <div style={{ flex: '1 1 180px', minWidth: 0, fontSize: 13, color: '#3b2e24' }}><div style={{ fontWeight: 700 }}>{t.ad} {t.icd10 && <span style={{ color: '#8b7d70', fontWeight: 400 }}>({t.icd10})</span>}</div>{t.destek.length > 0 && <div style={{ fontSize: 11, color: '#2f4334' }}>destek: {t.destek.join(', ')}</div>}<KaynakDip d={lab.kaynak?.tanilar[i] ? [lab.kaynak.tanilar[i]] : null} acik={kaynakAcik} /></div>
                ...
                <div style={{ ...toolsCard, marginBottom: 10 }}>
                  ...
                  {lab.klinik_iliski && <div style={{ fontSize: 12, color: '#8b7d70', marginTop: 6 }}>Asistan: {lab.klinik_iliski}</div>}
                  ...
                  {kayitMesaj && <div style={{ marginTop: 8, fontSize: 12, fontWeight: 700, color: /kaydedildi|kilitlendi|alındı/i.test(kayitMesaj) ? '#2f4334' : /kaydediliyor|kilitleniyor/i.test(kayitMesaj) ? '#8b7d70' : '#a45b3e' }}>{kayitMesaj}</div>}
                  ...
                  {analiz.note_id && <div style={{ marginTop: 6, fontSize: 12, color: '#2f4334' }}>{kilitli ? 'Muayene onaylandı — kilitli.' : "Objektif'e eklendi."} <a href={`/dashboard/doktor/notlar/${analiz.note_id}`} style={{ color: '#2f4334' }}>Notu aç →</a></div>}
                </div>
                {(lab.oneri || lab.recete_ipucu) && <div style={{ ...toolsCard, marginBottom: 10, fontSize: 13, color: '#3b2e24', whiteSpace: 'pre-wrap' }}><div style={etiket}>Öneri</div>{lab.oneri}{lab.recete_ipucu ? `\nReçete ipucu (yalnız öneri): ${lab.recete_ipucu}` : ''}<KaynakDip d={lab.kaynak?.oneri} acik={kaynakAcik} /></div>}
                {lab.sinirlar.length > 0 && <div style={{ ...toolsCard, fontSize: 12, color: '#8b7d70' }}><div style={etiket}>Sınırlar</div><ul style={{ margin: 0, paddingLeft: 18 }}>{lab.sinirlar.map((s, i) => <li key={i}>{s}</li>)}</ul>{analiz.fusion?.duzeltmeler?.length ? <div style={{ marginTop: 6, color: '#8b7d70' }}>Sistem düzeltmeleri: {analiz.fusion.duzeltmeler.join('; ')}</div> : null}</div>}
              </>
            )}
          </div>
        </div>
